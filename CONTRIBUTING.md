...
这是一个以阅读和翻译协作为主的个人资料库。贡献包括翻译、独立审校、索引维护、图片归档
和抓取/校验工具改进。所有变更必须通过 Pull Request 提交，由仓库所有者审核后合并。

## 先选贡献类型

| 你想做什么 | 先看哪里 | 交付物 |
|---|---|---|
| 翻译一篇英文资料 | [翻译工作流](#三翻译工作流) | 英文原文、中文配对、审校记录、校验结果 |
| 修正文义或术语 | [独立审校](#独立审校) | 最小范围的中文修改和前后对照说明 |
| 修复目录、链接或排序 | [导航与目录维护](#四导航与目录维护) | 工具/配置改动、重新生成的索引、链接检查结果 |
| 归档图片、快照或 WWDC 幻灯片 | [图片归档](#五图片归档) / [WWDC 幻灯片工程](#六wwdc-幻灯片工程) | 来源记录、哈希/清单、可复现的验证结果 |
| 改进脚本或校验器 | [提交前检查](#七提交前检查) | 测试、变更说明和不改变既有资料的证明 |

一次 PR 只选择一类主要目标。发现另一个问题时，请先记录下来，不要把不相关的翻译、归档
和工具重构混在同一个 PR 中。

## 一、准备与分支

先确认工作树干净，再从最新 `main` 创建独立分支：
...
默认 PR 的目标分支必须是 `main`。只有在确实需要堆叠时才使用其他基线，并且必须同时满足：

1. 上游基线已经有一个单独的 PR；
2. PR 描述写明 `base → head` 的合并顺序；
3. 下游分支不追加与当前任务无关的提交；
4. 上游 PR 合并后，把下游 PR 改基到 `main`，重新检查完整 diff。

如果一个分支已经混入翻译、归档或其他历史提交，不要直接把它改基到 `main`。请从 `main`
新建干净分支，只挑选本任务需要的提交（例如使用 `git cherry-pick`），再开新的 PR。

### 最小贡献闭环

1. 在 `meta/PROJECT_STATUS.md`、`meta/NEXT_STEPS.md` 和当前计划中确认任务仍在范围内。
2. 记录要处理的相对路径；如果是导航或工具改动，先说明输入、输出和不变的文件。
3. 从 `main` 建分支，完成修改，并保留英文原文与中文译文的相对路径配对。
4. 由没有参与初译的独立上下文完成审校，记录已处理的疑点。
5. 运行本任务对应的结构校验、导航生成和链接检查。
6. 查看 `git diff --check`、`git status --short` 和 staged diff，只暂存本任务文件。
7. 创建 PR，填写仓库提供的检查清单，等待审核；不要自行合并。

## 二、通用边界

- 只能提交目标为 `main` 的 PR，不得直接向 `main` 推送或自行合并；堆叠 PR 例外必须遵守上面的分支规则。
- 一个 PR 只处理一个明确任务；翻译批次应保持可独立审查。
- 不删除英文原文，不改变原文与译文的相对路径配对。
- 不提交 API Key、`.env`、缓存、临时视频、下载中间文件或失败日志。
- 不改变仓库可见性；第三方博客和 Apple 材料的归档授权并不等于再发布授权。
...
GitHub 的“保护主分支”提示是仓库设置建议，不会自动替代翻译审校，也不是 PR 合并失败的
诊断结果。建议至少启用：

- 要求通过 Pull Request 合并，禁止直接推送到 `main`；
- 至少 1 位审查者批准后才能合并；
- 新提交后使旧批准失效（避免审校过时）；
- 在仓库真正提供稳定 CI 检查前，不要把不存在的检查名称设为必需项。

启用规则前先确认维护者仍能审批和合并；规则过严会把仓库所有者锁在自己的合并流程外。
