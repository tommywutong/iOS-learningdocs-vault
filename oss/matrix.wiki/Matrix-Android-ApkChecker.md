...
**Matrix**是微信终端自研和正在使用的一套APM（Application Performance Management）系统。
...
![globle](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/global.png)

示例apk中包含的文件按类型统计如下图所示：
![file-type](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/file-type.png)

对于示例apk，我们使用Matrix-ApkChecker进行了全面检查，主要发现以下几个问题：

- png文件（不包括**.9.png**）未经压缩，可以考虑一定程度的压缩
![uncompress_file](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/uncompress_file.png)

- 存在一些冗余的文件，文件内容相同的文件应该只保留一份
![duplicated_file](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/duplicated_file.png)

- 存在无用资源，包括未使用的系统support包中的资源、第三方资源包中的无用资源以及示例app定义的资源
![unused_resources](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/unused_resources.png)

- 存在无用的assets资源，应该删除
![unused_assets](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/unused_assets.png)

针对上述Matrix-ApkChecker检测出来的问题，做如下针对性的优化：

- 首先删除冗余文件
> *res/drawable-xxxhdpi* 目下存在与 *res/drawable* 目录内容相同的文件，删除 *res/drawable* 目录下的 *icon.png* 以及 *round.png*。 删除之后，可以看到示例apk中png文件缩小了**23.89** KB 。
![ret-sub-duplicate](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/ret-sub-duplicate.png)


- 将png文件转换成**webp**格式
> 从示例输出中可以看到，示例apk的 **minSdkVersion** 是18，android对于API >= 18的版本已经支持透明的webp。使用**Android Studio**自带的webp转换功能，选择无损压缩，将部分png文件（不含 *.9.png* ）转成webp之后，示例apk的大小缩小了 **7.03** KB
![ret-convert-file](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/ret-convert-file.png)


- 删除无用的assets文件
> 将assets/music目录下的 *.mp3* 文件删除，示例apk的大小缩减了 **69.39** KB
![ret-sub-unused-assets](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/ret-sub-unused-assets.png)


- 删除无用资源
> 可以看到删除之后，apk中无用资源大大减少，同时示例apk中arsc文件大小缩减了 **36.99** KB
![ret-sub-unused-resource](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/ret-sub-unused-resource.png)
...
![total-work-flow](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/total-work-flow.png)
...
> Matrix-ApkChecker支持json格式和html格式的输出结果，默认的输出结果包含了最详尽的信息，开发者可以通过自定义输出结果的Formater来过滤精简输出信息。
只需要以下三步就可以实现自定义的输出结果格式：
	1.继承TaskJsonResult或者TaskHtmlResult来精简自定义每个Task的输出信息
![mmtaskjsonresult](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/mmtaskjsonresult.png)
	2.继承TaskResultRegistry并在其中注册自定义输出格式的名称和实现类
![taskresultregistry](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/taskresultregistry.png)
...
- 版本追踪
从下图可以直观看到微信多个版本的apk大小变化趋势。
![chart-line](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/chart-line.png)

- 版本issue
针对每个版本提出可以优化的issue，如下图所示：
![chart-issue](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/chart-issue.png)

- 版本详情

可以进一步查看某个版本apk的详情。下图显示了该版本各类型文件的占比情况：
![chart-pie](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/chart-pie.png)
对于该版本可能存在的问题，也会给出对应的suggesstion ：
![chart-suggests](https://github.com/Tencent/matrix/blob/dev/assets/img/wiki/apk-checker/chart-suggests.png)
