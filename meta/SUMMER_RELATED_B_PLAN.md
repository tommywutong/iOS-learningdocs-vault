...
用户明确恢复此前剩余的 19 篇，但只允许处理这 19 篇，不能重跑完整 407 篇。对应 `zh/`
路径中的结构草稿在人工复核时发现段落拼接、重复、漏译和代码围栏污染，不能视为完成。
本轮使用 Codex 逐篇修订；在逐篇通过严格校验和语言复核前，B 类验收数仍为 388 / 407。

### 2026-08-01 收尾完成

剩余 19 篇已全部使用 Codex 完成人工修订和语言复核，没有调用 DeepSeek，也没有恢复完整
分片或旧 `run-id`。19 / 19 均通过 `check_pair(..., strict_identifiers=True)`；B 类冻结
白名单现已验收 **407 / 407**。此前的结构草稿已修订为最终成品，不再属于当前交付阻塞。
