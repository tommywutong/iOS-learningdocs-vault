...
**归档主体和暑期学习定向翻译均已完成，等待仓库所有者审核本轮 PR。** Claude 会话中断前留下的
36 篇 Apple 文档译文已恢复；随后 PR #2–#16 又新增并审校了 319 篇 core 译文。DeepSeek
流水线新增的 218 篇译文（Apple 207、WWDC 11）已经审核并通过 PR #17 合入 `main`，
合并提交为 `09f2f8563`。新的 `summer-all-r01` 又完成 69 篇定向译文（博客 41、Apple
24、WWDC 4），已通过独立审校和全仓机械校验，尚未由用户合并。旧大批次曾因 DeepSeek
API 返回 402 暂停；用户现已改变范围，因此即使充值也不得恢复旧的 `core-r04-all`。
...
| 英文博客 | 2,263 | 102 | 2,161 | 24,510,419 |
| **合计** | **6,187** | **1,110** | **5,077** | — |

这里的 1,110 只统计“英文文件旁边存在同路径中文译文”，不包含 681 篇原生中文博客。
