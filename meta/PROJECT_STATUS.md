...
用户随后补充要求：暑期计划点名、且与 iOS 学习直接相关的英文单页快照也必须翻译。
`summer-snapshots-r01` 已完成 28 / 28，并归档计划内 WWDC18/415、WWDC19/423 幻灯片；
相关改动通过 PR #24 提交；至此当前 129 篇已取得原文的定向白名单全部完成。另有 22 个
单页 URL 受 robots 或原站
失效限制，仍需用户手动保存或等待作者原页恢复，不能宣称已经抓齐。
...
| 来源 | 已归档 | 当前中文状态 |
|---|---:|---:|
| Apple 现行文档 | 95,634 页，其中成篇文章 3,746 篇 | 1,070 篇成篇译文；1,091 个中文配对文件 |
| WWDC 逐字稿 | 178 场 | 105 场译文 |
| 英文技术博客 | 2,263 篇 | 355 篇译文 |
| 原生中文博客 | 681 篇 | 原文即中文，不计入翻译进度 |
| 学习计划单篇快照 | 108 个目标，成功 86 个 | 56 篇中文原文；30 篇英文中 28 篇已有译文 |
| Apple / Swift 开源资料 | 25 个仓库 | 保持原文 |
| 图片附件 | 2,806 个，约 0.78 GB | 计划内快照图片 64 个已本地化 |
| WWDC 幻灯片 | 3 场、586 张 WebP | Session 415、416、423 |

`python3 tools/translate_plan.py status` 当前统计的可翻译语料为：

| 来源 | 需译 | 已译 | 待译 | 待译字符 |
|---|---:|---:|---:|---:|
| Apple 现行文档 | 3,746 | 1,070 | 2,676 | 13,669,437 |
| WWDC | 178 | 105 | 73 | 2,020,501 |
| 英文博客 | 2,263 | 355 | 1,908 | 21,489,604 |
| 学习计划英文快照 | 30 | 28 | 2 | 146,554 |
| **合计** | **6,217** | **1,558** | **4,659** | — |

这里的 1,558 只统计“英文文件存在对应中文译文”，不包含 681 篇原生中文博客。快照剩余
2 篇是明确排除的 W3C XML 完整规范和 Open Data Structures，不是当前 iOS 计划欠账。
...
4. 翻译暑期计划点名、且服务于 iOS 学习的英文网页快照；
5. 不再追求全部 core 或全部英文博客完成。
...
### 2.5 2026-07-28 学习计划英文快照

`summer-snapshots-r01` 完成 28 篇、488,119 字节英文网页快照：

- 88 次 API 调用，983,717 输入 Token、635,728 输出 Token；
- 估算费用 0.5212 美元；
- 初译与独立审校使用隔离调用，28 / 28 通过结构校验；
- 代码字符串和围栏语言由确定性步骤恢复，不接受模型对代码的擅自修改；
- 77 个远程图片引用中 64 个已本地化，13 个因资源域名 robots 禁止而保留原链接；
- W3C XML 1.0 完整规范和 Open Data Structures 首页明确排除。

译文保存在 `blogs/snapshots-zh/<域名>/`，与 `blogs/snapshots/<域名>/` 原文保持同路径。
...
- WWDC18 Session 415：285 页，20,373,434 字节 WebP；
- WWDC19 Session 423：134 页，8,267,356 字节 WebP。

扫描结果共有 56 场能找到官方 PDF，目前完成计划相关的 3 场。其余 53 场不属于当前计划，
不批量归档；另有 122 场没有官方 PDF，不从视频抽帧。仓库必须继续保持私有。

### 2.7 2026-07-28 core 合并批次
...
成功抓取的 30 篇英文快照中，28 篇 iOS 计划相关内容已经翻译；其余 2 篇是明确排除的通用
规范或入口页。
