...
**归档主体、暑期严格白名单和高价值博客补强均已完成。** PR #20 已将
`summer-all-r01` 的 69 篇定向译文合入 `main`，合并提交为 `a817fcd43`。随后
`summer-b1-r01` 又完成 32 篇逐篇筛选的高价值博客；最早恢复的 36 篇 Claude 译文也已
全部完成独立审校，其中 35 篇发生实质修改。旧大批次曾因 DeepSeek API 返回 402 暂停；
用户现已改变范围，因此即使充值也不得恢复旧的 `core-r04-all`。
