...
> 基线日期：2026-07-29
> 当前范围：暑期直接白名单已完成；新增执行暑期强相关 B 类，不扩展到通用 C 类
...
`summer-related-b-full-r01` 完成 385 篇；连同烟雾样本，B 类当前完成
**388 / 407**。累计估算 6.8095 美元。剩余 19 篇均为复杂技术博客，停止时 9 篇失败、
10 篇停在审校状态。准确清单见
[`SUMMER_RELATED_B_REMAINING.md`](SUMMER_RELATED_B_REMAINING.md)。

用户已于 2026-07-29 决定停止继续消耗。不得自动恢复该 `run-id`，不得为追求
407 / 407 继续调用模型；只有用户以后明确重启这 19 篇时才重新评估。
...
原 129 篇暑期直接白名单已经完成；B 类停在 388 / 407：

1. 当前不再调用 DeepSeek，也不继续修复剩余 19 篇；
2. 保留本地断点和失败原因，已完成译文照常进入仓库；
3. 不扩展 C 类，也不恢复宽泛 `core`；
4. 若未来明确重启，只处理剩余清单，不重跑已完成的 388 篇。

读者导航重整已经独立完成，不属于剩余 19 篇正文翻译任务：
