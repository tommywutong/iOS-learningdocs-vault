...
| P2 | core Apple 文档 + WWDC | 待办 | 剩余 2,056 篇全部通过三道质量关 |
...
当前剩余 2,056 篇、12,412,033 字符。使用 8 个互不重叠分片，每片约 130,000 字符：

```bash
python3 tools/shard.py --shards 8 --budget 130000 --scope core
```

按这个预算，大约需要 12 轮。每一轮都按以下流水线执行：
...
剩余 34,816,160 个源文件字符。考虑初译、中文输出、独立审校、重读原文、失败重试和公共
