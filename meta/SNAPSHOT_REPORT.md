...
> 由 `tools/snapshot.py` 生成于 2026-07-27 02:41。
...
| 计划位置 | URL | 详情 |
|---|---|---|
| 第五周：RunLoop、AutoreleasePool、响应者链与生命周期 / Day 2｜先会 RunLoop，再理解常驻线程与卡顿监测（对应 W4-03） | https://ai-chan.top/code/Runloop%E4%B8%8E%E5%8D%A1%E9%A1%BF%E7%9B%91%E6%8E%A7/ | network HTTP 502, 0 字节 |
| 第二周：weak、属性关键字与 Block / Day 2｜weak 按“写入—读取—销毁”三段学（对应 W2-01～W2-06） | https://blog.csdn.net/u013378438/article/details/82790332 | http-521 2134 字节，HTTP 521 |
| 第三周：Runtime 行为与 Cocoa 对象通信 / Day 5｜KVC 提供间接访问，KVO 在它的约定上插入通知（对应 W3-09） | https://blog.csdn.net/zhoupengju/article/details/53129436 | http-521 2175 字节，HTTP 521 |
| 第三周：Runtime 行为与 Cocoa 对象通信 / Day 6｜最后才比较对象通信模式（对应 W3-01、W6-07） | https://blog.csdn.net/weixin_38633659/article/details/149066468 | http-521 2208 字节，HTTP 521 |
| 第五周：RunLoop、AutoreleasePool、响应者链与生命周期 / Day 3｜AutoreleasePool 放回事件循环中理解（对应 W4-01） | https://blog.csdn.net/Deft_MKJing/article/details/82947706 | http-521 2184 字节，HTTP 521 |
| 第八阶段：持久化、序列化、源码、架构与网络串联（建议 10 天） / Day 5｜用同一组问题读 YYModel，才有可比性（对应 W6-04） | https://blog.csdn.net/game3108/article/details/52388089 | http-521 2162 字节，HTTP 521 |
| 第八阶段：持久化、序列化、源码、架构与网络串联（建议 10 天） / Day 5｜用同一组问题读 YYModel，才有可比性（对应 W6-04） | https://blog.csdn.net/Lu_Ca/article/details/114532423 | http-521 2106 字节，HTTP 521 |
| 第八阶段：持久化、序列化、源码、架构与网络串联（建议 10 天） / Day 8｜架构是前七天代码的职责重排（对应 W6-06、W6-07） | https://blog.csdn.net/weixin_46818265/article/details/142442895 | http-521 2169 字节，HTTP 521 |
| 第二周：weak、属性关键字与 Block / Day 5｜循环引用是 Day 4 所有权图的推论（对应 W2-14、W2-16） | https://bytes.vokal.io/objc-block-capture-weakself/ | network HTTP 502, 0 字节 |
| 第五周：RunLoop、AutoreleasePool、响应者链与生命周期 / Day 3｜AutoreleasePool 放回事件循环中理解（对应 W4-01） | https://devyang.space/2019/05/05/autoreleasepool/ | network HTTP 502, 0 字节 |
| 第三周：Runtime 行为与 Cocoa 对象通信 / Day 5｜KVC 提供间接访问，KVO 在它的约定上插入通知（对应 W3-09） | https://huberyyang.com/2018/04/13/KVO%E5%AE%9E%E7%8E%B0%E5%8E%9F%E7%90%86/ | http-404 244061 字节，HTTP 404 |
| 第一周：对象、类与所有权的地基 / Day 1｜先分清“地址空间”，不要一上来背 isa（对应 W1-10） | https://juejin.cn/post/6963188936508178469 | js-challenge WAF / JS 挑战页（2397 字节） |
| 第四周：线程、GCD、Operation 与锁 / Day 6｜锁的学习方式是“按约束选择”（对应 W3-11） | https://juejin.cn/post/7070416276564213791 | js-challenge WAF / JS 挑战页（2397 字节） |
| 第六周：UIKit 渲染、UITableView 与性能 / Day 5｜集合先学稳定语义，再看某版本实现（对应 W6-10） | https://www.laoqingcai.com/ios-nsmutablearray/ | http-403 5194 字节，HTTP 403 |
| 第四周：线程、GCD、Operation 与锁 / Day 2｜了解原始线程，目的是理解上层抽象（对应 W3-03、W3-04） | http://stevenwuzheng.com/archives/runloop%E5%92%8C%E7%BA%BF%E7%A8%8B%E6%9C%89%E4%BB%80%E4%B9%88%E5%85%B3%E7%B3%BB | http-404 ConnectError: [SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: Hostname mismatch, certificate is not valid for 'stevenwuzheng.com'. (_ssl.c:1032) |
| 第七周：编译、链接、Mach-O、dyld 与 App 启动 / Day 5｜把 Runtime 初始化放进 App 冷启动（对应 W6-08） | https://tech.meituan.com/2018/12/06/waimai-ios-optimizing-startup.html | network HTTP 502, 0 字节 |
| 第二周：weak、属性关键字与 Block / Day 2｜weak 按“写入—读取—销毁”三段学（对应 W2-01～W2-06） | https://www.uiimage.com/post/blog/ios/sidetables/ | network HTTP 502, 0 字节 |
| 第八阶段：持久化、序列化、源码、架构与网络串联（建议 10 天） / Day 3｜序列化先比较需求，再看二进制细节（对应 W5-09） | https://www.w3.org/TR/xml/ | http-403 HTTP 403, 5848 字节 |
| 第五周：RunLoop、AutoreleasePool、响应者链与生命周期 / Day 6｜最后补坐标系，因为它依赖视图层级（对应 W3-12、W5-08） | https://zhangbuhuai.com/post/layer-geometry-in-ios.html | http-403 HTTP 403, 548 字节 |
...
| 计划位置 | URL | 详情 |
|---|---|---|
| 第六周：UIKit 渲染、UITableView 与性能 / Day 2｜离屏渲染先学定义，再看触发条件（对应 W5-02） | https://blog.fearcat.in/a?ID=01750-5926f776-644b-465e-8d4d-d6c7b854e533 | 原站已消失：blog.fearcat.in 现在是广告导流页，整页只有一张追踪像素和一条「Do Not Sell or Share My Personal Information」，没有正文 |
| 第二周：weak、属性关键字与 Block / Day 3｜先看 Block 是什么，再谈捕获（对应 W2-12、W2-13、W2-15） | https://www.informit.com/articles/article.aspx?p=1749597&seqNum=12 | 原文已下线：站点把 article.aspx?p=… 重定向到了「Articles」全站文章列表页，抓到的是 890 条书摘的目录，不是《Advanced Mac OS X Programming》那一节 |

### 原站已下线（域名停放 / 广告页）（1 条）

| 计划位置 | URL | 详情 |
|---|---|---|
| 第四周：线程、GCD、Operation 与锁 / Day 2｜了解原始线程，目的是理解上层抽象（对应 W3-03、W3-04） | https://bujige.net/blog/iOS-Complete-learning-pthread-and-NSThread.html | 正文命中「域名交易」，原文已不存在，需换镜像源 |
...
| 2617 | 454 | 4 | cnblogs.com | [【OC底层】isMemberOfClass、isKindOfClass原理分](https://www.cnblogs.com/xgao/p/11277935.html) | **真短文** — isMemberOfClass / isKindOfClass 的实例方法与类方法对照，4 个代码块占了正文大半，散文本来就少 |
| 2741 | 930 | 4 | jianshu.com | [iOS：重识Transform和frame](https://www.jianshu.com/p/e1fec2f92c63) | **真短文** — Transform 与 frame 的关系，4 个代码块 + 配图完整 |
| 3537 | 468 | 5 | 0daybug.com | [isKindOfClass & isMemberOfClass 的分析](https://www.0daybug.com/posts/9972ffa7/index.html) | **真短文** — isKindOfClass/isMemberOfClass 源码对照，5 个代码块完整 |
| 3609 | 4 | 0 | opendatastructures.org | [Open Data Structures](https://opendatastructures.org/) | **真短文** — 计划链接的是这本开放教科书的首页（简介 + 各语言版本入口），不是章节 |
| 3792 | 0 | 2 | shakuro.com | [NSOperation and NSOperationQueue To Im](https://shakuro.com/blog/nsoperation-and-nsoperationqueue-to-improve-concurrency-in-ios) | **真短文** — 英文入门短文，2 个代码块，结构完整 |
...
判为中文的 41 篇里，**散文部分**（剔掉围栏代码块）的中文字符占比 < 15% 算异常——正文只剩英文样板的典型症状。不剔代码块的话，源码解析类的中文长文会被大面积误报。

无异常。

中文篇的散文中文字符数：中位 2012，最小 197，最大 37007。
英文篇字符数：中位 16434，最小 1905，最大 49782。
...
| 第六周：UIKit 渲染、UITableView 与性能 / Day 5｜集合先学稳定语义，再看某版本实现（对应 W6-10） | [NSMutableArray原理揭露](http://blog.joyingx.me/2015/05/03/NSMutableArray%20%E5%8E%9F%E7%90%86%E6%8F%AD%E9%9C%B2/) | blog.joyingx.me | zh | 19300 | `blogs/snapshots/blog.joyingx.me/nsmutablearray原理揭露.md` |
