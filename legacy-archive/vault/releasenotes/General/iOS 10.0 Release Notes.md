...
source_url: https://developer.apple.com/library/archive/releasenotes/General/RN-iOSSDK-10.0/index.html
archived_at: '2026-07-18T02:54:41.346012Z'
---
...
- [Introduction](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqge3tknbqfvbuqmjnirxw45cmnfxgwrlmmvwwk3tujfcf6mi)
- [Bug Reporting](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqge3tknbqfvbuqmjnirxw45cmnfxgwrlmmvwwk3tujfcf6mq)
- [Notes and Known Issues](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqge3tknbqfvbuqmjnknltc)

### Introduction

iOS 10 SDK provides support for developing iOS apps. It’s packaged with a complete set of Xcode tools, compilers, and frameworks for creating apps for iOS and OS X. These tools include the Xcode IDE and the Instruments analysis tool, among many others.

With this software you can develop apps for iPhone, iPad, or iPod touch running iOS 10. You can also test your apps using the included Simulator, which supports iOS 10. iOS 10 SDK requires a Mac computer running OS X v10.10.3 (Yosemite) or later.
...
For issues not mentioned in [Notes and Known Issues](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqge3tknbqfvbuqmjnknltc), please file bugs through the Apple Developer website [https://developer.apple.com/bug-reporting/](https://developer.apple.com/bug-reporting/). Additionally, you may discuss these issues and iOS 10 SDK in the Apple Developer Forums: [http://devforums.apple.com](http://devforums.apple.com/). To get more information about iCloud for Developers, go to [http://developer.apple.com/icloud](https://developer.apple.com/icloud).

### Notes and Known Issues

The following items relate to using iOS 10.0 SDK to develop code.

### AVFoundation

### Note

The names of the optional [AVCapturePhotoCaptureDelegate](https://developer.apple.com/documentation/avfoundation/avcapturephotocapturedelegate) methods `didFinishProcessingPhoto` and `didFinishProcessingRawPhoto` were changed to [captureOutput:didFinishProcessingPhotoSampleBuffer:previewPhotoSampleBuffer:resolvedSettings:bracketSettings:error:](https://developer.apple.com/documentation/avfoundation/avcapturephotocapturedelegate/1778647-photooutput) and [captureOutput:didFinishProcessingPhotoSampleBuffer:previewPhotoSampleBuffer:resolvedSettings:bracketSettings:error:](https://developer.apple.com/documentation/avfoundation/avcapturephotocapturedelegate/1778647-photooutput), respectively. Code that uses the previous names (such as the WWDC version of the AVCamManual sample project) will continue to compile, but the older delegate callbacks will not execute.
...
Barometric pressure data from [CMAltimeter](https://developer.apple.com/documentation/coremotion/cmaltimeter) is not currently reported on iPad Air 2, iPad mini 4, and iPad Pro. The `CMAltimeter` availability check (that is, [isRelativeAltitudeAvailable](https://developer.apple.com/documentation/coremotion/cmaltimeter/1616170-isrelativealtitudeavailable)) returns `NO`, and the update handlers [startRelativeAltitudeUpdatesToQueue:withHandler:](https://developer.apple.com/documentation/coremotion/cmaltimeter/1616004-startrelativealtitudeupdatestoqu) and [stopRelativeAltitudeUpdates](https://developer.apple.com/documentation/coremotion/cmaltimeter/1615947-stoprelativealtitudeupdates) do not respond to changes in relative altitude.

### Music

- Recently Played in For You may not refresh instantly.
- [MPMediaPickerController](https://developer.apple.com/documentation/mediaplayer/mpmediapickercontroller) may not display as expected.
...
- Editing Up Next while connected to a car head-unit may unexpectedly disrupt playback.
...
__Workaround:__ Toggle Notes off, then on, in Settings > General > iCloud.
...
The first time you debug a widget (that is, a Today app extension) on a device, it does not show as a possible extension.
