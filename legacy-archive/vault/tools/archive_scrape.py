#!/usr/bin/env python3
"""补抓 Apple Documentation Archive 中漏收的文档，产出与本仓库字节级同构的 Markdown。
...
输入 doc/ARCHIVE_GAP_PHASE2_INPUT.json，产物一律写 .staging/，只有人工复核后才移入仓库。
...
OLD_VAULT = ROOT                              # 当前仓库同时是格式参照物
GAP_INPUT = ROOT / "doc" / "ARCHIVE_GAP_PHASE2_INPUT.json"
...
def load_gap(types=None):
    docs = json.loads(GAP_INPUT.read_text())["documents"]
    out = []
    for d in docs:
