...
source_url: https://developer.apple.com/library/archive/samplecode/qtbroadcast/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:53:05.185354Z'
---
...
extern QTSPresentation  gPresentation;
...
void QTApp_Idle (WindowReference theWindow)
{
    ...
    // grant the broadcaster some processor time
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_GET_SETTINGS:
            if (gPresentation != kQTSInvalidPresentation)
                myErr = QTSPresSettingsDialog(gPresentation, kQTSAllStreams, 0, NULL, 0L);
...
#pragma unused(theMenu)
    ...
    QTFrame_SetMenuItemState(myMenu, IDM_GET_SETTINGS, gPresentation == kQTSInvalidPresentation ? kDisableMenuItem : kEnableMenuItem);
}
...
Boolean QTApp_HandleEvent (EventRecord *theEvent)
{
    ...
    // does the event belong to the monitor dialog box?
    if (IsDialogEvent(theEvent)) {
        if (DialogSelect(theEvent, &myDialog, &myItemHit)) {
            QTBC_HandleMonitorWindowEvents(myDialog, myItemHit);
            myEventHandled = true;
        }
    }
