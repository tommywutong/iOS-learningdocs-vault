...
source_url: https://developer.apple.com/library/archive/samplecode/sndequalizer/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:51.798384Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [sndequalizer](sndequalizer.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](sndequalizer.md)
...
extern DialogPtr        gSndEqDialog;
extern Boolean          gSndEqDialogIsVis;
extern SndListHandle    gSndEqResourceHandle;
//extern SndChannelPtr  gSndEqSndChannelPtr;
extern UInt8            gLevels[kSndEqNumBands];
...
void QTApp_Idle (WindowReference theWindow)
{
    ...
    if (myWindowObject != NULL) {
        ...
        // update the sound equalizer display for the front movie window
        if ((**myWindowObject).fMovie != NULL)
            SndEq_UpdateMovieLevels(myWindowObject);
    }
...
void QTApp_Draw (WindowReference theWindow)
{
    ...
    // if the window is the graphic equalizer window, redraw it
    if ((DialogPtr)theWindow == gSndEqDialog)
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_SHOWHIDE_EQ:
            SndEq_ToggleDialog();
            myIsHandled = true;
            break;

        case IDM_OPEN_SND_RESOURCE:
            gSndEqResourceHandle = SndEq_OpenSoundFile();
            myIsHandled = true;
            break;

        case IDM_PLAY_SND_RESOURCE:
            if (gSndEqResourceHandle != NULL)
                SndEq_PlaySoundResource(gSndEqResourceHandle);
            myIsHandled = true;
            break;
...
#pragma unused(theMenu)
    ...
    if (gSndEqDialog == NULL) {
        // no equalizer display dialog box
        QTFrame_SetMenuItemState(myMenu, IDM_SHOWHIDE_EQ, kDisableMenuItem);
        QTFrame_SetMenuItemState(myMenu, IDM_OPEN_SND_RESOURCE, kDisableMenuItem);
        QTFrame_SetMenuItemState(myMenu, IDM_PLAY_SND_RESOURCE, kDisableMenuItem);
    } else {
        // display dialog box exists
        QTFrame_SetMenuItemState(myMenu, IDM_SHOWHIDE_EQ, kEnableMenuItem);
        QTFrame_SetMenuItemState(myMenu, IDM_OPEN_SND_RESOURCE, kEnableMenuItem);

        if (gSndEqDialogIsVis)
            QTFrame_SetMenuItemLabel(myMenu, IDM_SHOWHIDE_EQ, "Hide &Equalizer");
        else
            QTFrame_SetMenuItemLabel(myMenu, IDM_SHOWHIDE_EQ, "Show &Equalizer");

        if (gSndEqResourceHandle == NULL)
            QTFrame_SetMenuItemState(myMenu, IDM_PLAY_SND_RESOURCE, kDisableMenuItem);
        else
            QTFrame_SetMenuItemState(myMenu, IDM_PLAY_SND_RESOURCE, kEnableMenuItem);
    }
}
...
Boolean QTApp_HandleEvent (EventRecord *theEvent)
{
    ...
    // on Windows, a movie window needs to be open before this code gets triggered
...
void QTApp_SetupWindowObject (WindowObject theWindowObject)
{
    if (theWindowObject != NULL)
        (**theWindowObject).fAppData = (Handle)SndEq_InitWindowData(theWindowObject);
}
...
{
    SndEq_DumpWindowData(theWindowObject);
