...
source_url: https://developer.apple.com/library/archive/samplecode/qtgraphics/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:49.362743Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [qtgraphics](qtgraphics.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](qtgraphics.md)
...
#pragma unused(theMenu)
    ...
    UInt16                      myIndex;
    Boolean                     myIsImage = false;

#if TARGET_OS_WIN32
    myMenu = theMenu;
#endif
#if TARGET_OS_MAC
    myMenu = GetMenuHandle(kImageMenuResID);
#endif
    ...
    QTFrame_SetMenuItemState(myMenu, IDM_HALF_SIZE, myIsImage ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_NORMAL_SIZE, myIsImage ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_DOUBLE_SIZE, myIsImage ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_FILL_SCREEN, kDisableMenuItem);                                // exercise for the reader
    QTFrame_SetMenuItemState(myMenu, IDM_ROTATE_LEFT, myIsImage ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_ROTATE_RIGHT, myIsImage ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_FLIP_HORIZ, myIsImage ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_FLIP_VERT, myIsImage ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_EXPORT_IMAGE, myIsImage ? kEnableMenuItem : kDisableMenuItem);

    //////////
    //
    // set check mark on current size menu item
    //
    //////////

    for (myIndex = IDM_HALF_SIZE; myIndex <= IDM_FILL_SCREEN; myIndex++) {
        if (myAppData != NULL)
            QTFrame_SetMenuItemCheck(myMenu, myIndex, (**myAppData).fCurrentSize == myIndex);
        else
            QTFrame_SetMenuItemCheck(myMenu, myIndex, false);
    }
}
...
{
    if (theWindowObject != NULL)
        if ((**theWindowObject).fAppData != NULL)
            DisposeHandle((**theWindowObject).fAppData);
