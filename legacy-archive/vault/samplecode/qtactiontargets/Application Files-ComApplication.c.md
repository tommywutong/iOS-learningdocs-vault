...
source_url: https://developer.apple.com/library/archive/samplecode/qtactiontargets/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:53:10.896602Z'
---
...
void QTApp_Init (UInt32 theStartPhase)
{
    ...
    if (theStartPhase & kInitAppPhase_BeforeCreateFrameWindow) {
        // check to make sure that QuickTime wired sprites are available;
        // we depend on these features
        if (!QTUtils_HasWiredSprites())
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_MAKE_TWIN_SPRITES_MOVIE:
            QTTarg_CreateTwinSpritesMovie();
            myIsHandled = true;
            break;

        case IDM_ADD_TEXT_CONTROL_TO_MOVIE:
            if (myMovie != NULL) {
                QTTarg_AddTextToggleButtonTrack(myMovie);
        ...
        case IDM_SET_MOVIE_TARGET_NAME:
            // prompt the user for the desired target name
            myString = QTTarg_GetStringFromUser(kMovieNameResIndex);
            if (myString != NULL) {
                QTUtils_SetMovieTargetName(myMovie, myString);
                free(myString);
                (**myWindowObject).fIsDirty = true;
            }
            myIsHandled = true;
        ...
        case IDM_GET_MOVIE_TARGET_NAME:
            myString = QTUtils_GetMovieTargetName(myMovie);
            if (myString != NULL) {
                // display name to user
                myPString = QTUtils_ConvertCToPascalString(myString);
                QTTarg_ShowStringToUser(myPString);
        ...
        case IDM_SET_MOVIE_TARGET_ID:
            // prompt the user for the desired target ID
            myString = QTTarg_GetStringFromUser(kMovieIDResIndex);
            if (myString != NULL) {
                // convert string to a long integer
                myPString = QTUtils_ConvertCToPascalString(myString);
                StringToNum(myPString, &myID);

                QTUtils_SetMovieTargetID(myMovie, myID);
                free(myString);
                free(myPString);
                (**myWindowObject).fIsDirty = true;
            }
        ...
        case IDM_GET_MOVIE_TARGET_ID:
            myID = QTUtils_GetMovieTargetID(myMovie, &myMovieHasID);
            if (myMovieHasID) {
                // convert ID to a string
                NumToString(myID, myIDString);

                // display ID to user
                QTTarg_ShowStringToUser(myIDString);
            } else {
                QTFrame_Beep();
            }
        ...
        case IDM_MAKE_TWO_VR_CONTROL_MOVIE:
            QTTarg_MakeDualVRControllerMovie();
            myIsHandled = true;
            break;
...
#pragma unused(theMenu)
    ...
    QTFrame_SetMenuItemState(myMenu, IDM_ADD_TEXT_CONTROL_TO_MOVIE, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_SET_MOVIE_TARGET_NAME, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_GET_MOVIE_TARGET_NAME, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_SET_MOVIE_TARGET_ID, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_GET_MOVIE_TARGET_ID, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
}
...
PASCAL_RTN Boolean QTApp_MCActionFilterProc (MovieController theMC, short theAction, void *theParams, long theRefCon)
{
    ...
    switch (theAction) {
        ...
        case mcActionGetExternalMovie:
            QTFrame_FindExternalMovieTarget(theMC, (QTGetExternalMoviePtr)theParams);
            break;
        ...
        case mcActionGetMovieID:
            ...
            if (myMovie != NULL) {
                ...
                // get the target ID of the current movie
                myID = QTUtils_GetMovieTargetID(myMovie, &myMovieHasID);
                if (myMovieHasID) {
                    *(long *)theParams = myID;
                    isHandled = true;
