...
source_url: https://developer.apple.com/library/archive/samplecode/CreateMovie/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-18T03:05:07.541993Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [CreateMovie](CreateMovie.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](CreateMovie.md)
...
void QTApp_Draw (WindowReference theWindow, Rect *theRefreshArea)
{
...
#pragma unused(theMenu)
    ...
    // ***insert application-specific menu adjusting here***

    if (myMC == NULL) {
        // no movie controller, so disable all the Test menu items
        QTFrame_SetMenuItemState(myMenu, IDM_CONTROLLER, kDisableMenuItem);
        QTFrame_SetMenuItemState(myMenu, IDM_SPEAKER_BUTTON, kDisableMenuItem);
    } else {
        // we've got a movie controller, so it's safe to proceed....
        QTFrame_SetMenuItemState(myMenu, IDM_CONTROLLER, kEnableMenuItem);

        // if controller bar is visible, enable Test menu items
        if (QTUtils_IsControllerBarVisible(myMC)) {
            QTFrame_SetMenuItemLabel(myMenu, IDM_CONTROLLER, "Hide &Controller");
            // ungray the other menu items
            QTFrame_SetMenuItemState(myMenu, IDM_SPEAKER_BUTTON, kEnableMenuItem);

            // handle Test menu items
            if (QTUtils_IsControllerButtonVisible(myMC, kQTVRSpeakerButton))
                QTFrame_SetMenuItemLabel(myMenu, IDM_SPEAKER_BUTTON, "Hide Spea&ker Button");
            else 
                QTFrame_SetMenuItemLabel(myMenu, IDM_SPEAKER_BUTTON, "Show Spea&ker Button");

        } else {
            // controller bar is not visible

            QTFrame_SetMenuItemLabel(myMenu, IDM_CONTROLLER, "Show &Controller");
...
void QTApp_InstallAppleEventHandlers (void)
{
    ...
    if (myErr == noErr) {
        if (myAttrs & (1L << gestaltAppleEventsPresent)) {
            ...
            gHandleOpenAppAEUPP = NewAEEventHandlerUPP((AEEventHandlerProcPtr)QTApp_HandleOpenApplicationAppleEvent);
            gHandleOpenDocAEUPP = NewAEEventHandlerUPP((AEEventHandlerProcPtr)QTApp_HandleOpenDocumentAppleEvent);
            gHandlePrintDocAEUPP = NewAEEventHandlerUPP((AEEventHandlerProcPtr)QTApp_HandlePrintDocumentAppleEvent);
            gHandleQuitAppAEUPP = NewAEEventHandlerUPP((AEEventHandlerProcPtr)QTApp_HandleQuitApplicationAppleEvent);
...
PASCAL_RTN OSErr QTApp_HandleOpenApplicationAppleEvent (const AppleEvent *theMessage, AppleEvent *theReply, unsigned long theRefcon)            
{
...
PASCAL_RTN OSErr QTApp_HandleOpenDocumentAppleEvent (const AppleEvent *theMessage, AppleEvent *theReply, unsigned long theRefcon)           
{
...
PASCAL_RTN OSErr QTApp_HandlePrintDocumentAppleEvent (const AppleEvent *theMessage, AppleEvent *theReply, unsigned long theRefcon)          
{
#pragma unused(theMessage, theReply, theRefcon)
...
PASCAL_RTN OSErr QTApp_HandleQuitApplicationAppleEvent (const AppleEvent *theMessage, AppleEvent *theReply, unsigned long theRefcon)            
{
#pragma unused(theMessage, theReply, theRefcon)
