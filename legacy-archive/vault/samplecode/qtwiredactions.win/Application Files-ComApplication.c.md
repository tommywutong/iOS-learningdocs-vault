...
source_url: https://developer.apple.com/library/archive/samplecode/qtwiredactions.win/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:53:24.610400Z'
---
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        case IDM_MAKE_HYPERTEXT_MOVIE:
            myPrompt = QTUtils_ConvertCToPascalString(kHTSavePrompt);
            myName = QTUtils_ConvertCToPascalString(kHTSaveFileName);

            // elicit a file from the user to save the new movie into
            QTFrame_PutFile(myPrompt, myName, &myFile, &myIsSelected, &myIsReplacing);

            // create a text movie with hypertext links
            if (myIsSelected) {
                myErr = QTWired_CreateTextMovie(&myFile);
                if (myErr == noErr)
...
//      case IDM_MAKE_CUSTOM_EVENT_MOVIE:
            QTWired_CreateWiredIconMovie(theMenuItem);
            myIsHandled = true;
            break;
...
#pragma unused(theMenu)
    ...
    // no special enabling/disabling required here....
}
