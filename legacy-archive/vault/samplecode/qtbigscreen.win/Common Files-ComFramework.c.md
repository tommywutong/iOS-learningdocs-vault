...
source_url: https://developer.apple.com/library/archive/samplecode/qtbigscreen.win/Listings/Common_Files_ComFramework_c.html
archived_at: '2026-07-26T19:52:45.802185Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [qtbigscreen.win](qtbigscreen.win.md)


[Next](Common%20Files-ComFramework.h.md)[Previous](Application%20Files-ComResource.h.md)
...
gotImageFile:
    ...
    // if the movie is a fullscreen movie, then request fullscreen mode
{
    ...
    if (myUserData != NULL) {
        myErr = GetUserDataItem(myUserData, &myRec, sizeof(myRec), FOUR_CHAR_CODE('ptv '), 0);
        if (myErr == noErr) {
            myRec.fSize = EndianU16_BtoN(myRec.fSize);

            if (myRec.fSize == kSizeFullScreen)
                MCDoAction(myMC, mcActionAppMessageReceived, (void *)kQTAppMessageEnterFullScreenRequested);
    ...
    if (QTFrame_IsImageWindow((**theWindowObject).fWindow)) {
        ...
        if ((**theWindowObject).fController != NULL)
            if (MCGetVisible((**theWindowObject).fController))
                if (MCIsControllerAttached((**theWindowObject).fController))
