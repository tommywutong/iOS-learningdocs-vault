---
title: SndPlayDoubleBuffer
apple_id: DTS10000371
resource_type: Sample Code
platform: macOS
topic: null
technology: null
published: '2003-03-14'
source_url: https://developer.apple.com/library/archive/samplecode/SndPlayDoubleBuffer/Listings/_headers_Interrupt_Routines_h.html
archived_at: '2026-07-18T03:24:54.646301Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [SndPlayDoubleBuffer](SndPlayDoubleBuffer.md)


[Next](headers-LDandFix.h.md)[Previous](headers-DoubleBufferFromFile.h.md)

# _headers/Interrupt_Routines.h

```c
/*
    File:       Interrupt_Routines.h

    Contains:   Headers for routines demonstrating how to manage interrupt routines for sound play.

    Written by: Mark Cookson    

    Copyright:  Copyright © 1996-1999 by Apple Computer, Inc., All Rights Reserved.

                You may incorporate this Apple sample source code into your program(s) without
                restriction. This Apple sample source code has been provided "AS IS" and the
                responsibility for its operation is yours. You are not permitted to redistribute
                this Apple sample source code as "Apple sample source code" after having made
                changes. If you're going to re-distribute the source, we require that you make
                it clear in the source that the code was descended from Apple sample source
                code, but that you've made changes.

    Change History (most recent first):
                8/31/1999   Karl Groethe    Updated for Metrowerks Codewarror Pro 2.1


*/
#ifndef __INTERRUPT_ROUTINES__
#define __INTERRUPT_ROUTINES__

#include <Sound.h>

#ifndef __SOUNDSTRUCT__
#include "SoundStruct.h"
#endif

#ifndef __DBFFERRORS__
#include "DBFF_Errors.h"
#endif

#ifndef __DOUBLEBUFFERFROMFILE__
#include "DoubleBufferFromFile.h"
#endif

#ifndef __MUNGEBUFFER__
#include "MungeBuffer.h"
#endif

#ifndef __DEFINES__
#include "Defines.h"
#endif

/* Function declarations */
#ifdef powerc
pascal  void    ASoundFileCallBack      (myParmBlkPtr mypb);
#else
pascal  void    ASoundFileCallBack      (myParmBlkPtr mypb:__a0);
#endif
pascal  void    ASoundDoubleBackProc    (SndChannelPtr chan,
                                        SndDoubleBufferPtr doubleBuffer);
pascal  void    ASoundDoneCallBack      (SndChannelPtr chan,
                                        SndCommand *cmd);

extern Boolean  gBackwards, gSoundDone;

#endif
```

[Next](headers-LDandFix.h.md)[Previous](headers-DoubleBufferFromFile.h.md)

