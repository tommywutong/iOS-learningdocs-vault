...
source_url: https://developer.apple.com/library/archive/samplecode/vrspeech/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:53:04.677073Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [vrspeech](vrspeech.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](vrspeech.md)
...
//
//     <3>      04/05/00    rtm     made changes to get things running under CarbonLib
//     <2>      10/13/97    rtm     ported VRSpeech code to ComApplication.c
...
Boolean                 gHasSpeechRec = false;                  // is the Speech Recognition Manager available?
...
void QTApp_Init (UInt32 theStartPhase)
{
    ...
    if (theStartPhase & kInitAppPhase_BeforeCreateFrameWindow) {
        ...
        gHasSpeechRec = false;                                  // assume the worst
        VRSpeech_Init();
#endif
    }
...
#if TARGET_OS_MAC
        // shutdown speech recognition capabilities, if enabled
        if (gHasSpeechRec)
            VRSpeech_Stop();
...
void QTApp_Idle (WindowReference theWindow)
{
    ...
    // handle speech-initiated periodics
    if (gHasSpeechRec)
...
#pragma unused(theMenuItem)
    // currently, the Mac application has NO app-specific menus; you could change that.
    return(false);
}
...
void QTApp_SetupWindowObject (WindowObject theWindowObject)
{
    ...
    if (myInstance != NULL) {
        ...
        // install a node entering procedure
        QTVRSetEnteringNodeProc(myInstance, NewQTVREnteringNodeUPP(VRSpeech_SpeakNameOfNode), (long)myInstance, 0);
