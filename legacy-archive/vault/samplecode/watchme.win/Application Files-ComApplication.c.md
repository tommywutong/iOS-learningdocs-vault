...
source_url: https://developer.apple.com/library/archive/samplecode/watchme.win/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:53:34.728207Z'
---
...
//
//
//  WatchMe is a simple QuickTime viewer framework that demonstrates how to handle absolute and relative
//  URL chasing in a QuickTime movie.
//
//////////
...
void QTApp_Init (UInt32 theStartPhase)
{
...
#if TARGET_OS_WIN32
    char                myTail[] = "\\Movies\\SDKWatchMe-SourceMovies\\WatchMe.mov";
#endif
#if TARGET_OS_MAC
    char                myTail[] = ":Movies:SDKWatchMe-SourceMovies:WatchMe.mov";
#endif
    ...
    if (theStartPhase & kInitAppPhase_AfterCreateFrameWindow) {
        ...
        // construct the full pathname of the target movie
        myName[0] = '\0';
        myVolName = WatchMe_GetLaunchVolumeName();
        if (myVolName != NULL) {
            strcat(myName, myVolName);
            strcat(myName, myTail);
        }

        myString = QTUtils_ConvertCToPascalString(myName);

        // open the WatchMe movie
#if TARGET_OS_MAC
        MacSetCursor(*MacGetCursor(watchCursor));
#endif

        FSMakeFSSpec(0, 0, (ConstStr255Param)myName, &myFSSpec);
...
#pragma unused(theMC, theParams)
    ...
    switch (theAction) {
        ...
        // handle link-to-URL events
        case mcActionLinkToURL:
            // the movie controller knows how to handle absolute URLs but not relative ones,
            // so we need to convert any relative URLs to absolute ones
            WatchMe_ConvertRelativeToAbsoluteURL((Handle)theParams, theRefCon);
            break;
