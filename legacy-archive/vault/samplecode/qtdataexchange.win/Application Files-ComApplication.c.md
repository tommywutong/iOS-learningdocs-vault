...
source_url: https://developer.apple.com/library/archive/samplecode/qtdataexchange.win/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:48.764177Z'
---
...
extern MovieProgressUPP             gMovieProgressProcUPP;      // UPP to our custom movie progress dialog box procedure
extern ICMProgressUPP               gImageProgressProcUPP;      // UPP to our custom image progress dialog box procedure
extern UserItemUPP                  gProgressUserItemProcUPP;   // UPP to our custom progress dialog user item procedure

extern StringPtr                    gSettingsFileName;          // the name of our settings preferences file
...
void QTApp_Init (UInt32 theStartPhase)
{
    ...
    if (theStartPhase & kInitAppPhase_AfterCreateFrameWindow) {
        ...
        gMovieProgressProcUPP = NewMovieProgressUPP(QTDX_MovieProgressProc);
        gImageProgressProcUPP = NewICMProgressUPP(QTDX_ImageProgressProc);
        gProgressUserItemProcUPP = NewUserItemUPP(QTDX_ProgressBoxUserItemProcedure);

        gSettingsFileName = QTUtils_ConvertCToPascalString(kSettingsFileName);
...
void QTApp_Stop (UInt32 theStopPhase)
{
    ...
    if (theStopPhase & kStopAppPhase_BeforeDestroyWindows) {
        DisposeMovieProgressUPP(gMovieProgressProcUPP);
        DisposeICMProgressUPP(gImageProgressProcUPP);
        DisposeUserItemUPP(gProgressUserItemProcUPP);
        free(gSettingsFileName);
    }
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_IMPORT_ANY_COMP:
            QTDX_ImportAnyNonMovie();
            myIsHandled = true;
            break;

        case IDM_EXPORT_ANY_COMP:
            QTDX_ExportMovieAsAnyTypeFile(myMovie, &(**myWindowObject).fFileFSSpec);
            myIsHandled = true;
            break;

        case IDM_EXPORT_AS_HINTED:
            QTDX_ExportMovieAsHintedMovie(myMovie, true);
            myIsHandled = true;
            break;
...
//
// Currently, the Mac application has only one app-specific menu ("Movie"); you could change that.
//
//////////
...
#pragma unused(theMenu)
...
#endif
#if TARGET_OS_MAC
    myMenu = GetMenuHandle(kMovieMenuResID);
#endif
    ...
    QTFrame_SetMenuItemState(myMenu, IDM_EXPORT_ANY_COMP, myMovie != NULL ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_EXPORT_AS_HINTED, myMovie != NULL ? kEnableMenuItem : kDisableMenuItem);
}
...
{
    // install our custom progress dialog box procedures for movies and images
