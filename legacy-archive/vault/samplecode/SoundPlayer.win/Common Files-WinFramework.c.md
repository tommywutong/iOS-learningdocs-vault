...
source_url: https://developer.apple.com/library/archive/samplecode/SoundPlayer.win/Listings/Common_Files_WinFramework_c.html
archived_at: '2026-07-18T03:25:06.835780Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [SoundPlayer.win](SoundPlayer.win.md)


[Next](Common%20Files-WinFramework.h.md)[Previous](Common%20Files-QTUtilities.h.md)
...
//
//     <11>     07/31/00    rtm     reworked QTFrame_CalcWindowMinMaxInfo to use subsystem 4.0 metrics
//     <10>     07/07/00    rtm     removed QTFrame_CreateMacEditMenu; now we use MCGetMenuString
//     <9>      07/06/00    rtm     made changes to support new parameter to QTFrame_AdjustMenus; added the calls
//                                  QTFrame_CreateMacEditMenu and QTFrame_GetKeyboardModifiers
...
int CALLBACK WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR theCmdLine, int nCmdShow)
{
    ...
    if (hPrevInstance == NULL) {
        ...
        myWC.hIconSm       = (HICON)LoadImage(hInstance, MAKEINTRESOURCE(IDI_APPICON), IMAGE_ICON, 16, 16, 0);
        ...
        myWC.hIconSm       = (HICON)LoadImage(hInstance, MAKEINTRESOURCE(IDI_CHILDICON), IMAGE_ICON, 16, 16, 0);
    ...
    while (GetMessage(&myMsg, NULL, 0, 0)) {    
        if (!TranslateMDISysAccel(ghWndMDIClient, &myMsg)) {
            if (!TranslateAccelerator(myWindowFrame, (HACCEL)myAccel, &myMsg)) {
...
{
    DialogBox((HINSTANCE)ghInst, MAKEINTRESOURCE(IDD_ABOUT), ghWnd, (DLGPROC)QTFrame_DialogProcedure);
}
...
int QTFrame_ShowCautionAlert (HWND theWnd, UINT theID, UINT theIconStyle, UINT theButtonStyle, LPSTR theTitle, LPSTR theArgument)
{
    ...
    LoadString((HINSTANCE)ghInst, theID, myTemplate, sizeof(myTemplate));
...
// Get the current keyboard modifiers; return a long word encoding those modifiers that's
// in the same format as the modifiers field of an event record.
//
// The Windows equivalent of the Option key is defined as the combination of Ctrl and Alt;
// so, if they are both down, we should only return the Option key; if only one of them is
// down, then we return that one.
//
