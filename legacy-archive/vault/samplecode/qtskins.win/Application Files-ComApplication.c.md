...
source_url: https://developer.apple.com/library/archive/samplecode/qtskins.win/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:47.577625Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [qtskins.win](qtskins.win.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](qtskins.win.md)
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    // make sure we have a valid movie
    if (myMovie == NULL)
        return(myIsHandled);

    switch (theMenuItem) {

        case IDM_ADD_SKIN_TRACK:
            QTSkin_AddSkinTrack(myMovie);
...
#pragma unused(theMenu)
    ...
    if (myMovie == NULL) {
        QTFrame_SetMenuItemState(myMenu, IDM_ADD_SKIN_TRACK, kDisableMenuItem);
    } else {
        if (QTSkin_IsSkinnedMovie(myMovie))
            QTFrame_SetMenuItemState(myMenu, IDM_ADD_SKIN_TRACK, kDisableMenuItem);
        else
            QTFrame_SetMenuItemState(myMenu, IDM_ADD_SKIN_TRACK, kEnableMenuItem);
    }
}
...
#if TARGET_OS_WIN32
    // on Windows, see whether the event is a click in the drag region; if it is, report
    // that we've handled it, so that the movie controller doesn't try to handle it; we'll
    // handle the click in the WM_LBUTTONDOWN message handling
    ApplicationDataHdl  myAppData = (ApplicationDataHdl)QTFrame_GetAppDataFromFrontWindow();
    Point               myPoint;
...
void QTApp_SetupWindowObject (WindowObject theWindowObject)
{
    ...
    // if this window holds a movie with a skin track....
    if (QTSkin_IsSkinnedMovie(myMovie)) {
        ...
        (**theWindowObject).fAppData = (Handle)QTSkin_InitWindowData(theWindowObject);
    }
}
