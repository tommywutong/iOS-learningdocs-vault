...
source_url: https://developer.apple.com/library/archive/samplecode/qtmusic.win/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:51.172894Z'
---
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_PLAY_SOME_NOTES:
            QTMusic_PlaySomeNotes();
            myIsHandled = true;
            break;

        case IDM_PLAY_BENT_NOTES:
            QTMusic_PlaySomeBentNotes();
            myIsHandled = true;
            break;

        case IDM_PLAY_SHEPARD_MELODY:
            QTMusic_PlayShepardMelody();
            myIsHandled = true;
            break;

        case IDM_PICK_THEN_PLAY:
            QTMusic_PickInstThenPlaySomeNotes();
            myIsHandled = true;
            break;

        case IDM_PICK_THEN_PLAY_CUSTOM:
            QTMusic_PickCustomInstThenPlaySomeNotes();
            myIsHandled = true;
            break;

        case IDM_BUILD_AND_PLAY_SEQUENCE:
            QTMusic_BuildSequenceAndPlay();
            myIsHandled = true;
            break;

        case IDM_BUILD_MUSIC_MOVIE:
            QTMusic_CreateMusicMovie(NULL);
            myIsHandled = true;
            break;

        case IDM_BUILD_MUSIC_MOVIE_CUSTOM: {
                ...
                if (myHandle != NULL) {
                    ...
                    if (myInstrument != NULL)
                        QTMusic_CreateMusicMovie(myInstrument);
                }
            }
            myIsHandled = true;
            break;

        case IDM_USE_MIDI_INPUT:
#if !TARGET_API_MAC_CARBON
...
#pragma unused(theMenu)
    ...
    QTFrame_SetMenuItemState(myMenu, IDM_PLAY_SOME_NOTES, kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_PLAY_BENT_NOTES, kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_PLAY_SHEPARD_MELODY, kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_PICK_THEN_PLAY, kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_PICK_THEN_PLAY_CUSTOM, kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_BUILD_AND_PLAY_SEQUENCE, kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_BUILD_MUSIC_MOVIE, kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_BUILD_MUSIC_MOVIE_CUSTOM, kEnableMenuItem);

#if !TARGET_API_MAC_CARBON
    QTFrame_SetMenuItemState(myMenu, IDM_USE_MIDI_INPUT, kEnableMenuItem);
#else
    QTFrame_SetMenuItemState(myMenu, IDM_USE_MIDI_INPUT, kDisableMenuItem);
#endif
