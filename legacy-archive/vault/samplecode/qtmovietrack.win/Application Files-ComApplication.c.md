...
source_url: https://developer.apple.com/library/archive/samplecode/qtmovietrack.win/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:34.550703Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [qtmovietrack.win](qtmovietrack.win.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](qtmovietrack.win.md)
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_ADD_FILE_AS_MOVIE_TRACK:
            myErr = QTMIM_ShowPropertiesDialogBox(myWindowObject);
            if (myErr == noErr)
                myErr = QTMIM_AddFileAsMovieTrack(myWindowObject);
            if (myErr == noErr)
                (**myWindowObject).fIsDirty = true;
            break;

        case IDM_ADD_URL_AS_MOVIE_TRACK:
            myErr = QTMIM_ShowPropertiesDialogBox(myWindowObject);
            if (myErr == noErr)
                myErr = QTMIM_AddURLAsMovieTrack(myWindowObject);
            if (myErr == noErr)
                (**myWindowObject).fIsDirty = true;
            break;
...
#pragma unused(theMenu)
    ...
    QTFrame_SetMenuItemState(myMenu, IDM_ADD_FILE_AS_MOVIE_TRACK, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_ADD_URL_AS_MOVIE_TRACK, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
}
...
void QTApp_RemoveWindowObject (WindowObject theWindowObject)
{
    ...
    if ((**theWindowObject).fAppData != NULL)
        DisposeHandle((Handle)(**theWindowObject).fAppData);
