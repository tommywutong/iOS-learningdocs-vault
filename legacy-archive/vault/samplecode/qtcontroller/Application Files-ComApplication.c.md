...
source_url: https://developer.apple.com/library/archive/samplecode/qtcontroller/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:29.815621Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [qtcontroller](qtcontroller.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](qtcontroller.md)
...
# Application Files/ComApplication.c
...
//
//     <2>      12/11/99    rtm     modifications for QTController sample
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_TOGGLE_CONTROLLER:
            if (myMC != NULL)
                QTUtils_ToggleControllerBar(myMC);
            myIsHandled = true;
            break;

        case IDM_TOGGLE_BADGE:
            if (myMC != NULL) {
                Boolean         myBadgeIsEnabled;

                MCDoAction(myMC, mcActionGetUseBadge, &myBadgeIsEnabled);
                MCDoAction(myMC, mcActionSetUseBadge, (void *)!myBadgeIsEnabled);
            }
            myIsHandled = true;
            break;

        case IDM_PUT_BAR_ON_TOP:
            if (myMC != NULL)
                QTUtils_PutControllerBarOnTop(myMC);
            myIsHandled = true;
            break;

        case IDM_TOGGLE_SPEAKER_BUTTON:
            if (myMC != NULL)
                QTUtils_ToggleControllerButton(myMC, kQTUtilsSpeakerButton);
            myIsHandled = true;
            break;

        case IDM_TOGGLE_STEP_BUTTONS:
            if (myMC != NULL)
                QTUtils_ToggleControllerButton(myMC, kQTUtilsStepButtons);
            myIsHandled = true;
            break;

        case IDM_TOGGLE_CUSTOM_BUTTON:
            if (myMC != NULL)
                QTUtils_ToggleControllerButton(myMC, kQTUtilsCustomButton);
            myIsHandled = true;
            break;

        case IDM_TOGGLE_BACK_BUTTON:
            if (myMC != NULL)
                QTUtils_ToggleControllerButton(myMC, kQTUtilsBackButton);
            myIsHandled = true;
            break;

        case IDM_TOGGLE_ZOOM_BUTTONS:
            if (myMC != NULL)
                QTUtils_ToggleControllerButton(myMC, kQTUtilsZoomButtons);
            myIsHandled = true;
            break;

        case IDM_TOGGLE_HOTSPOT_BUTTON:
            if (myMC != NULL)
                QTUtils_ToggleControllerButton(myMC, kQTUtilsHotSpotButton);
            myIsHandled = true;
            break;

        case IDM_TOGGLE_TRANSLATE_BUTTON:
            if (myMC != NULL)
                QTUtils_ToggleControllerButton(myMC, kQTUtilsTranslateButton);
            myIsHandled = true;
            break;

        case IDM_BROWSE_APPLE_DOT_COM:
            QTApp_HaveBrowserOpenURL("http://www.apple.com");
            myIsHandled = true;
            break;
...
#pragma unused(theMenu)
    ...
    Boolean             myGotMC = false;
    Boolean             myIsVRMovie = false;
    Boolean             myBarIsVisible = false;
    Boolean             myBarIsAttached = false;
    Boolean             myBadgeIsEnabled = false;
    ...
    if (myMovie != NULL)
        myIsVRMovie = QTUtils_IsQTVRMovie(myMovie);

    if (myMC != NULL) {
        myBarIsVisible = QTUtils_IsControllerBarVisible(myMC);
        myBarIsAttached = MCIsControllerAttached(myMC) == 1;
    }

    //////////
    //
    // enable or disable the menu items, according to the current state
    //
    //////////

    QTFrame_SetMenuItemState(myMenu, IDM_TOGGLE_CONTROLLER, (myGotMC && myBarIsAttached) ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_TOGGLE_BADGE, (myGotMC && myBarIsAttached && !myIsVRMovie) ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_PUT_BAR_ON_TOP, (myGotMC && myBarIsAttached && myBarIsVisible) ? kEnableMenuItem : kDisableMenuItem);

    QTFrame_SetMenuItemState(myMenu, IDM_TOGGLE_SPEAKER_BUTTON, (myGotMC && myBarIsVisible) ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_TOGGLE_CUSTOM_BUTTON, (myGotMC && myBarIsVisible) ? kEnableMenuItem : kDisableMenuItem);

    QTFrame_SetMenuItemState(myMenu, IDM_TOGGLE_STEP_BUTTONS, (myGotMC && myBarIsVisible && !myIsVRMovie) ? kEnableMenuItem : kDisableMenuItem);

    QTFrame_SetMenuItemState(myMenu, IDM_TOGGLE_BACK_BUTTON, (myGotMC && myBarIsVisible && myIsVRMovie) ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_TOGGLE_ZOOM_BUTTONS, (myGotMC && myBarIsVisible && myIsVRMovie) ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_TOGGLE_HOTSPOT_BUTTON, (myGotMC && myBarIsVisible && myIsVRMovie) ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_TOGGLE_TRANSLATE_BUTTON, (myGotMC && myBarIsVisible && myIsVRMovie) ? kEnableMenuItem : kDisableMenuItem);

    //////////
    //
    // set the menu item labels, according to the current state
    //
    //////////

    if (myBarIsVisible)
        QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_CONTROLLER, "Hide Controller Bar");
    else
        QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_CONTROLLER, "Show Controller Bar");

    if (myMC != NULL) {
        MCDoAction(myMC, mcActionGetUseBadge, &myBadgeIsEnabled);
        if (myBadgeIsEnabled)
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_BADGE, "Disable Badge Display");
        else
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_BADGE, "Enable Badge Display");

        if (QTUtils_IsControllerButtonVisible(myMC, kQTUtilsSpeakerButton))
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_SPEAKER_BUTTON, "Hide Speaker Button");
        else
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_SPEAKER_BUTTON, "Show Speaker Button");

        if (QTUtils_IsControllerButtonVisible(myMC, kQTUtilsCustomButton))
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_CUSTOM_BUTTON, "Hide Custom Button");
        else
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_CUSTOM_BUTTON, "Show Custom Button");

        if (QTUtils_IsControllerButtonVisible(myMC, kQTUtilsStepButtons))
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_STEP_BUTTONS, "Hide Step Buttons");
        else
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_STEP_BUTTONS, "Show Step Buttons");

        if (QTUtils_IsControllerButtonVisible(myMC, kQTUtilsBackButton))
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_BACK_BUTTON, "Hide Back Button");
        else
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_BACK_BUTTON, "Show Back Button");

        if (QTUtils_IsControllerButtonVisible(myMC, kQTUtilsZoomButtons))
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_ZOOM_BUTTONS, "Hide Zoom Buttons");
        else
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_ZOOM_BUTTONS, "Show Zoom Buttons");

        if (QTUtils_IsControllerButtonVisible(myMC, kQTUtilsHotSpotButton))
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_HOTSPOT_BUTTON, "Hide Hot Spot Button");
        else
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_HOTSPOT_BUTTON, "Show Hot Spot Button");

        if (QTUtils_IsControllerButtonVisible(myMC, kQTUtilsTranslateButton))
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_TRANSLATE_BUTTON, "Hide Translate Button");
        else
            QTFrame_SetMenuItemLabel(myMenu, IDM_TOGGLE_TRANSLATE_BUTTON, "Show Translate Button");
    }
}
