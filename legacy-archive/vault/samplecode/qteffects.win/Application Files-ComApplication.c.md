...
source_url: https://developer.apple.com/library/archive/samplecode/qteffects.win/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:40.528937Z'
---
...
void QTApp_Idle (WindowReference theWindow)
{
    ...
    if (myWindowObject != NULL) {
        ...
        if ((**myWindowObject).fGraphicsImporter != NULL) {
            ...
            myAppData = (ApplicationDataHdl)(**myWindowObject).fAppData;
            if (myAppData != NULL)
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_ADD_FILM_NOISE_TO_MOVIE:
            QTEffects_AddFilmNoiseToMovie(myMovie);
            if (myWindowObject != NULL)
                (**myWindowObject).fIsDirty = true;
            myIsHandled = true;
            break;

        case IDM_ADD_FILM_NOISE_TO_IMAGE:
            QTEffects_AddFilmNoiseToImage(myWindowObject);
            // no point in setting the window dirty, since we can't save the changes anyway....
        //  if (myWindowObject != NULL)
        //      (**myWindowObject).fIsDirty = true;
            myIsHandled = true;
            break;
        ...
        case IDM_ADD_EFFECT_SEGMENT:
            QTEffects_AddEffectToMovieSegment(myMovie, kFilmNoiseImageFilterType, 1, 1200, 2400);
            if (myWindowObject != NULL)
                (**myWindowObject).fIsDirty = true;
            myIsHandled = true;
            break;

        case IDM_ADD_SPRITE_EFFECT:
...
#pragma unused(theWindow)
    ...
    QTFrame_SetMenuItemState(myMenu, IDM_ADD_FILM_NOISE_TO_MOVIE, myMovie != NULL ? kEnableMenuItem : kDisableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_ADD_FILM_NOISE_TO_IMAGE, myImporter != NULL ? kEnableMenuItem : kDisableMenuItem);

    // if the effects dialog is already open, don't allow user to select more files
    QTFrame_SetMenuItemState(myMenu, IDM_MAKE_EFFECT_MOVIE, gEffectsDialog == 0 ? kEnableMenuItem : kDisableMenuItem);
    ...
    QTFrame_SetMenuItemState(myMenu, IDM_ADD_EFFECT_SEGMENT, myMovie != NULL ? kEnableMenuItem : kDisableMenuItem);
}
...
void QTApp_SetupWindowObject (WindowObject theWindowObject)
{
    ...
    (**theWindowObject).fAppData = QTEffects_InitWindowData(theWindowObject);
}
