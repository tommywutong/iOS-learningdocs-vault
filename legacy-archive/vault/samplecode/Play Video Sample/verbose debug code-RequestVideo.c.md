...
source_url: https://developer.apple.com/library/archive/samplecode/Play_Video_Sample/Listings/verbose_debug_code_RequestVideo_c.html
archived_at: '2026-07-18T03:19:14.118584Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [Play Video Sample](Play%20Video%20Sample.md)


[Next](Document%20Revision%20History.md)[Previous](verbose%20debug%20code-PlayVideo.c.md)

# verbose debug code/RequestVideo.c
...
OSErr RVSetVideoRequest (VideoRequestRecPtr requestRecPtr)
{
    ...
    if (displayMgrPresent)
    {
        if (requestRecPtr->displayMode && requestRecPtr->depthMode)
        {
            ...
            {
                printf ("DMSetDisplayMode: second monitor & DM version: %d, %d\n", aMonitor, displayMgrVersion);
...
OSErr RVSetVideoAsScreenPrefs (void)
{
    ...
    if (displayMgrPresent)
    {
        if (noErr != (err = DMBeginConfigureDisplays (&displaystate)))  // Tell the world it is about to change
            printf ("DMBeginConfigureDisplays error: %d\n", err);
        if (noErr != (err = DMUseScreenPrefs (true, displaystate)))     // Make the change
            printf ("DMBeginConfigureDisplays error: %d\n", err);
        if (noErr != (err = DMEndConfigureDisplays (displaystate)))     // Tell the world the change is over
            printf ("DMBeginConfigureDisplays error: %d\n", err);
...
OSErr RVGetCurrentVideoSetting (VideoRequestRecPtr requestRecPtr)
{
    ...
    if (requestRecPtr->screenDevice)
    {
        ...
        {   // get the info the DM 2.0 way
            ...
            printf ("RVGetCurrentVideoSetting DM2 displayMode: %d\n", requestRecPtr->displayMode);
            printf ("RVGetCurrentVideoSetting DM2 depthMode: %d\n", requestRecPtr->depthMode);
        ...
        {   // get the info the DM 1.0 way
            ...
            *(Ptr *)&pBlock.csParam[0] = (Ptr)&videoMode;
            ...
            printf ("RVGetCurrentVideoSetting DM1 displayMode: %d\n", requestRecPtr->displayMode);
            printf ("RVGetCurrentVideoSetting DM1 depthMode: %d\n", requestRecPtr->depthMode);
            return (error); // we (maybe) set the world back to a known setting
        }
    }
    return (-1);
...
OSErr RVRequestVideoSetting (VideoRequestRecPtr requestRecPtr)
{
    ...
    if (displayMgrPresent)
    {
        ...
        printf("Walk all the GDevices looking for video\n");
        ...
        if( walkDevice && myModeIteratorProc) do // start the search
        {
            ...
            printf("\n");
            printf("GDevice #%d at location %d.\n",iCount, walkDevice);
            if( noErr == DMGetDisplayIDByGDevice( walkDevice, &theDisplayID, false ) )  // DM1.0 does not need this, but it fits in the loop
            {
                ...
                if (noErr == DMNewDisplayModeList(theDisplayID, 0, 0, &theDisplayModeCount, &theDisplayModeList) )
                {
                    ...
                    printf("Find request the DM2.0 way\n");
                ...
                else
                {
                    ...
                    printf("Find request the DM1.0 way\n");
                    ...
                    )
                {
                    ...
                    if (noErr == error)                 // keep going if no errorÉ
                    {
                        ...
                        {                               // We have data! lets have a look
                            ...
                            // print screen resolution - all the data
                            printf("Mode: %d, Depth: %d, Resolution: %dH x %dV\n",
                                jCount,
                                vpData->vpPixelSize,
                            ...
                            DMListType *theDisplayModeList)
{
    ...
    for (jCount=0; jCount<theDisplayModeCount; jCount++)        // get info on all the resolution timings
    {
        ...
        if (searchData.depthBlockCount) for (kCount = 0; kCount < searchData.depthBlockCount; kCount++)
        {
                ...
                )
            {
                                    ...
                                    searchData.depthBlocks[kCount].depthVPBlock.vpBounds.bottom))
                {
                    ...
                    printf ("requestRecPtr->availBitDepth:%d\n", requestRecPtr->availBitDepth);
                }
            }
        ...
        // timing flags
        if (searchData.displayModeTimingInfo.csTimingFlags & 1<<kModeValid)
            printf("mode = %d = valid, ",searchData.displayModeTimingInfo.csTimingFlags);
        else
            printf("mode = %d = invalid, ",searchData.displayModeTimingInfo.csTimingFlags);
...
Boolean FindBestMatch (VideoRequestRecPtr requestRecPtr, short bitDepth, unsigned long horizontal, unsigned long vertical)
{
    ...
    if  (   nil == requestRecPtr->displayMode
        ...
        )
        {
            ...
            printf ("¥1 good to change - first return\n");
            return (true);
        }
    else    // can we do better than last time?
    {
        printf ("have bit priority and avail?\n");
        ...
        {
            // go ahead and set the new values
            printf ("¥2 good to change - second return\n");
            return (true);
        }
        else
        {
            printf ("match resolution: minimize Æh & Æv\n");
            ...
            {
                printf ("now we have a smaller or same delta\n");
                ...
                {
                    printf ("but only if we do not change bit depth\n");
                    ...
                    {
                        // go ahead and set the new values
                        printf ("¥3 good to change - last return\n");
                        return (true);
                    }
                }
            }
