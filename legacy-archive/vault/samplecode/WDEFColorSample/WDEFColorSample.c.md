...
apple_id: DTS10000624
resource_type: Sample Code
platform: macOS
topic: null
technology: null
published: '2003-01-30'
source_url: https://developer.apple.com/library/archive/samplecode/WDEFColorSample/Listings/WDEFColorSample_c.html
archived_at: '2026-07-18T03:28:05.438954Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [WDEFColorSample](WDEFColorSample.md)


[Next](WDEFColorSample.h.md)[Previous](WDEFColorSample.md)

# WDEFColorSample.c

```c
/*
    File:       WDEFColorSample.c

    Contains:   WDEFSample shows in high level what the system 7 WDEF does as far as
                choosing colors is concerned.
...
#include "WDEFColorSample.h"        /* bring in all the #defines for WDEFColorSample */
...
Boolean TrapAvail( short tNumber, TrapType tType );
void AlertUser( void );

extern void FakeWDEF(WindowPtr window, Boolean hilite);
...
void DoUpdate(WindowPtr window)
{
    if ( IsAppWindow(window) ) {
        ...
        if ( ! EmptyRgn(window->visRgn) )   /* draw if updating needs to be done */
            DrawWindow(window);
        if (gHilite) gHilite = false; else gHilite = true;
        EndUpdate(window);
    }
} /*DoUpdate*/
...
/*  This is called when a window is activated or deactivated.
    Since we want to show color changes when activating/deactivating
    we want to have an update in order to display the window in the
    right colors.
*/
...
/* The core shield of tWDEFSample. When an update occurs we draw another window
   using the same technique used by the WDEF.
*/
...
void Initialize()
{
    ...
    gHasWaitNextEvent = TrapAvail(_WaitNextEvent, ToolTrap);
    ...
    /* if color quickdraw is available then we use a color window,
       an old style window is aquired otherwise.
    */
    window = (WindowPtr) NewPtr(sizeof(WindowRecord));
    if ( window == nil ) AlertUser();
    if (gMac.machineType < envMacII) 
