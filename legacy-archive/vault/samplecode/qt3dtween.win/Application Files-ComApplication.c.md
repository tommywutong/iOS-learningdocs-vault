...
source_url: https://developer.apple.com/library/archive/samplecode/qt3dtween.win/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-18T03:30:00.856404Z'
---
...
extern Boolean          gAddCameraTweenData;                    // do we add camera tween data?
extern Boolean          gAddRotationTweenData;                  // do we add rotation tween data?
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_USE_CAMERA_TWEENING:
            // toggle the current setting
            gAddCameraTweenData = !gAddCameraTweenData;
            myIsHandled = true;
            break;

        case IDM_USE_ROTATION_TWEENING:
            // toggle the current setting
            gAddRotationTweenData = !gAddRotationTweenData;
            myIsHandled = true;
            break;

        case IDM_CREATE_TWEEN_MOVIE:
...
#pragma unused(theMenu)
    ...
    // allow a movie to be created only if some tween data is selected
    if (gAddCameraTweenData || gAddRotationTweenData)
        QTFrame_SetMenuItemState(myMenu, IDM_CREATE_TWEEN_MOVIE, kEnableMenuItem);
    else
        QTFrame_SetMenuItemState(myMenu, IDM_CREATE_TWEEN_MOVIE, kDisableMenuItem);

    // add check marks to selected tweenings
    QTFrame_SetMenuItemCheck(myMenu, IDM_USE_CAMERA_TWEENING, gAddCameraTweenData);
    QTFrame_SetMenuItemCheck(myMenu, IDM_USE_ROTATION_TWEENING, gAddRotationTweenData);
}
