...
source_url: https://developer.apple.com/library/archive/samplecode/RAVE_Starter_Samples/Listings/CallBack_Sample_Gouraud_Callback_cp.html
archived_at: '2026-07-18T03:21:47.916014Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [RAVE Starter Samples](RAVE%20Starter%20Samples.md)


[Next](Gouraud%20Sample-Gouraud%20Triangle.cp.md)[Previous](RAVE%20Starter%20Samples.md)

# CallBack Sample/Gouraud Callback.cp

```c
/*
    File:       Gouraud Callback.cp
...
const UInt16 kRGB16WhitePixel = 0xFFFF;
...
// This is a simple RAVE callback sample.  it sets every other pixel in a checkerboard pattern to white.
...
#pragma unused (refCon)
    ...
    UInt16                  *pixelPtr = NULL;
    Ptr                     rowPtr = NULL;
    long                    loopX, loopY, width, height, offset;

    memory = &buffer->device.memoryDevice;

    // buffer is a memory device, check to make sure we're using the right pixel type.
    if ((memory->pixelType != kQAPixel_RGB16) &&
        (memory->pixelType != kQAPixel_ARGB16))
        return;
    ...
    {
        drawPixel = !drawPixel; // This should alternate by row;
...
void DoRAVEWindow(void)
{
    ...
    // QASetNotificationMethod was barfing when I pass in RAVEComposite directly.  I think it can't
    // tell how to cast it because the notification method is a Union.  So now, I set up a variable
    // to hold the union and explictly set RAVEComposite as a buffering method.  Works fine.
    noticeMeth.bufferNoticeMethod = RAVEComposite;

    theQAErr = QASetNoticeMethod (theContext, kQAMethod_BufferComposite, noticeMeth ,NULL);
    if (theQAErr != kQANoErr) SIGNAL_ERROR("\pERROR:  Failed to set compositing completion proc")
