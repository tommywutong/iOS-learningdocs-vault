...
source_url: https://developer.apple.com/library/archive/samplecode/qdmediamaker/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-18T03:29:58.384157Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [qdmediamaker](qdmediamaker.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](qdmediamaker.md)
...
void QTApp_Init (UInt32 theStartPhase)
{
    ...
    if (theStartPhase & kInitAppPhase_AfterCreateFrameWindow) {
        ...
        MacSetRect(&myRect, 10, 40, 510, 440);
        gWindow = NewCWindow(NULL, &myRect, myTitle, true, documentProc, (WindowPtr)-1, false, 0);
        MacSetPort((GrafPtr)GetWindowPort(gWindow));
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        case IDM_ROWS:
            QDMM_MakeQDMovie(theMenuItem, 500, 400);
            myIsHandled = true;
            break;
        case IDM_LINES:
            QDMM_MakeQDMovie(theMenuItem, 256, 200);
            myIsHandled = true;
            break;
        case IDM_BOXES:
            QDMM_MakeQDMovie(theMenuItem, 300, 200);
            myIsHandled = true;
            break;
        case IDM_BALL:
            QDMM_MakeQDMovie(theMenuItem, 500, 400);
...
#pragma unused(theWindow, theMenu)
    ...
    if (FrontWindow() == gWindow)
        QTFrame_SetMenuItemState(GetMenuHandle(kFileMenuResID), IDM_FILECLOSE, kDisableMenuItem);

    // we don't allow creating new files here...
    QTFrame_SetMenuItemState(GetMenuHandle(kFileMenuResID), IDM_FILENEW, kDisableMenuItem);

    // we don't allow opening files here...
    QTFrame_SetMenuItemState(GetMenuHandle(kFileMenuResID), IDM_FILEOPEN, kDisableMenuItem);
#endif
}
