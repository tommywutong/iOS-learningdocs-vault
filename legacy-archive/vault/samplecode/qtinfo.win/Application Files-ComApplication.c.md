...
source_url: https://developer.apple.com/library/archive/samplecode/qtinfo.win/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:32.914708Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [qtinfo.win](qtinfo.win.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](qtinfo.win.md)
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    // make sure we have a valid movie and movie controller
    if ((myMovie == NULL) || (myMC == NULL))
        return(myIsHandled);

    switch (theMenuItem) {

        case IDM_SET_PREVIEW_TO_SEL:
            QTInfo_SetPreviewToSelection(myMovie, myMC);
            (**myWindowObject).fIsDirty = true;
            myIsHandled = true;
            break;

        case IDM_SET_SEL_TO_PREVIEW:
            QTInfo_SetSelectionToPreview(myMovie, myMC);
            myIsHandled = true;
            break;

        case IDM_CLEAR_PREVIEW:
            QTInfo_ClearPreview(myMovie, myMC);
            (**myWindowObject).fIsDirty = true;
            myIsHandled = true;
            break;

        case IDM_PLAY_PREVIEW:
            PlayMoviePreview(myMovie, NULL, 0L);
            myIsHandled = true;
            break;

        case IDM_GO_TO_POSTER_FRAME:
            QTInfo_GoToPosterFrame(myMovie, myMC);
            myIsHandled = true;
            break;

        case IDM_SET_POSTER_TO_FRAME:
            QTInfo_SetPosterToFrame(myMovie, myMC);
            (**myWindowObject).fIsDirty = true;
            myIsHandled = true;
            break;

        case IDM_SHOW_MOVIE_INFO:
            ShowMovieInformation(myMovie, gModalFilterUPP, 0L);
            myIsHandled = true;
            break;

        case IDM_ADD_FULL_NAME:
            myIsChanged = QTInfo_EditAnnotation(myMovie, kUserDataTextFullName);
            if (myIsChanged)
                (**myWindowObject).fIsDirty = true;
            myIsHandled = true;
            break;

        case IDM_ADD_COPYRIGHT:
            myIsChanged = QTInfo_EditAnnotation(myMovie, kUserDataTextCopyright);
            if (myIsChanged)
                (**myWindowObject).fIsDirty = true;
            myIsHandled = true;
            break;
        ...
        case IDM_ADD_INFORMATION:
            myIsChanged = QTInfo_EditAnnotation(myMovie, kUserDataTextInformation);
            if (myIsChanged)
                (**myWindowObject).fIsDirty = true;
            myIsHandled = true;
            break;
...
#pragma unused(theMenu)
    ...
    Boolean             myHasSelection = false;
    Boolean             myHasPreview = false;
    Boolean             myHasPoster = false;
    ...
    if (myMovie != NULL) {
        ...
        myHasPreview = QTInfo_MovieHasPreview(myMovie);
        myHasPoster = QTInfo_MovieHasPoster(myMovie);
    }

    QTFrame_SetMenuItemState(myMenu, IDM_SET_PREVIEW_TO_SEL, ((myMovie == NULL) || !myHasSelection) ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_SET_SEL_TO_PREVIEW, ((myMovie == NULL) || !myHasPreview) ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_CLEAR_PREVIEW, ((myMovie == NULL) || !myHasPreview) ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_PLAY_PREVIEW, ((myMovie == NULL) || !myHasPreview) ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_GO_TO_POSTER_FRAME, ((myMovie == NULL) || !myHasPoster) ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_SET_POSTER_TO_FRAME, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_SHOW_MOVIE_INFO, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_ADD_FULL_NAME, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_ADD_COPYRIGHT, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
    QTFrame_SetMenuItemState(myMenu, IDM_ADD_INFORMATION, myMovie == NULL ? kDisableMenuItem : kEnableMenuItem);
}
