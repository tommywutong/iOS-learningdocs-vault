...
source_url: https://developer.apple.com/library/archive/samplecode/qtshellCEvents.win/Listings/Common_Files_ComFramework_h.html
archived_at: '2026-07-26T19:52:37.683289Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [qtshellCEvents.win](qtshellCEvents.win.md)


[Next](Common%20Files-MacFramework.c.md)[Previous](Common%20Files-ComFramework.c.md)
...
#define USE_CARBON_EVENTS                   1               // do we use Carbon event routines or the classic event routines?
#endif

#define USE_TASK_MGMT                       0               // do we use QuickTime's task management APIs? (QT 6 and later!)

#define USE_CARBON_MOVIE_CONTROL            0               // do we use the Carbon movie control? (QT 6 and later, OS X only!)
...
#define COMMAND_ID(menuIdentifier)          ((MENU_ID(menuIdentifier)<<16)+(MENU_ITEM(menuIdentifier)))
...
typedef struct {
    ...
    EventLoopTimerRef       fTimerRef;          // window event loop timer reference
#endif
