...
source_url: https://developer.apple.com/library/archive/samplecode/MoreOSL/Listings/MoreOSL_TestMoreOSL_TestMoreOSL_c.html
archived_at: '2026-07-18T03:15:54.163080Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [MoreOSL](MoreOSL.md)


[Next](MoreOSL-TestMoreOSL-TestMoreOSLTerminology.h.md)[Previous](MoreOSL-ReadMe.html.md)
...
# MoreOSL/TestMoreOSL/TestMoreOSL.c
...
/*
    ...
    Change History (most recent first):
         ...
         <4>      9/3/00    Quinn   Fix problems caused by merging with GeorgeÕs changes.
...
// Because we use 0 as a nil value for TemplateNodeIndex, the table
...
struct Node {
    NodePtr     sibling;                // pointer to next node at this level (or nil if none)
    NodePtr     children;               // pointer to first child of this node (or nil if none)
    NodePtr     parent;                 // pointer to parent node (or nil if at top level)
    Str31       name;                   // name of this node
};
...
static OSStatus CreateNodesFromTemplate(TemplateNodeIndex templateIndex, NodePtr parent, 
    ...
    // the template) and parent is nil (items at the top of the tree
    ...
    // easy to code.  I donÕt expect to copy this part of the sample verbatim.
{
    OSStatus err;
...
static SInt32 GetSiblingIndex(NodePtr eldest, NodePtr siblingToSearchFor)
    ...
    // return 1.  Returns 0 if siblingToSearchFor isnÕt a sibling of eldest.
{
    SInt32     result;
    NodePtr    thisNode;
    Boolean    found;
...
static OSStatus FindSiblingByIndex(NodePtr eldest, SInt32 index, MOSLToken *valueTok)
    ...
    // create a token for the indexÕth sibling in the list.  Returns
...
static pascal Boolean MyAEIdleProc(EventRecord *theEvent, long *sleepTime, RgnHandle *mouseRgn)
    ...
    // ItÕs purpose is to process any incoming events by calling
    // HandleOneEvent, and return true if the application wants to quit.
{
    MoreAssertQ(theEvent  != nil);
    MoreAssertQ(sleepTime != nil);
    MoreAssertQ(mouseRgn  != nil);
    ...
    // the front before putting up the window.
{
    ...
    if (err == noErr) {
        ...
        err = NavPutFile(nil, &reply, &options, gMyNavEventUPP, fileType, fileCreator, nil);
        if (err == noErr) {
    ...
    // back to the corresponding AppleScript OSType value.
{
    ...
    if (err == noErr) {
        ...
        MoreAssertQ(gMyNavEventUPP != nil);
        err = NavAskSaveChanges(&options, action, &reply, gMyNavEventUPP, nil);
    }
    if (err == noErr) {
        switch (reply) {
...
// use the dialogÕs windowÕs refCon to store the address of a 
...
// (using the Document data type), but the about window doesnÕt.
//
// Note that the window iterators all start with GetWindowList, not
// FrontWindow, because FrontWindow wonÕt return invisible windows.
//
...
// itÕs ID property.  This ID is set when we create the window and 
// canÕt be modified.  IDs are not reused during the execution of
...
// us debug broken code and handle windows that donÕt belong to us.
// While there shouldnÕt be any windows that donÕt belong to us
...
struct DialogHeader {
    ...
    WindowRef window;                   // a pointer to the dialogÕs window (saves a call to GetDialogWindow)
};
...
struct Document {
    ...
    FSSpec       fss;                   // the documentÕs location on the disk
    NodePtr      eldest;                // a pointer to the documentÕs nodes
};
    ...
    // This routine returns the AppleScript class of the given window.
{
    ...
    if (header != nil && header->magic == kDialogHeaderMagic) {
    ...
    // returns a pointer to the associated DialogHeader.
{
    ...
    // of ours.  If the info weÕre returning belies this, we make a controlled
    // trip to MacsBug (which is better than the uncontrolled one weÕd take
    // if we returned a bogus header).

    MoreAssertQ(header != nil && header->magic == kDialogHeaderMagic && header->dialog == dialog);
...
static SInt32 GetDialogUniqueID(DialogRef dialog)
    ...
    // returns the dialogÕs unique ID.
{
    DialogHeaderPtr header;
...
static SInt32 GetIndexOfDialogOfClass(DescType theClass, DialogRef dialogToSearchFor)
    ...
    // the index property of a window is independent of the windowÕs
    // specific class.  The routine is now only used for the index property
    // of documentÕs, which are in a different 'address space' than those
    // for windows.
{
    ...
    MoreAssertQ((theClass == typeWildCard) || (GetClassOfWindow(windowToSearchFor) == theClass));
    thisWindow = GetWindowList();
    found = false;
    while (thisWindow != nil && !found) {
    ...
    // CApplicationAccessByIndex to access windows by their index.
{
    ...
    MoreAssertQ( found == (thisWindow != nil) );
    if (thisWindow == nil) {
        result = nil;
    } else {
...
static DocumentPtr GetDocumentFromDialog(DialogRef dialog)
    ...
    // 'up casts' the result to a DocumentPtr.  ItÕs called
    ...
    // get at the document-specific data.
{
    ...
    MoreAssertQ(result->header.dlgClass == cNodeWindow);
    return result;
}
...
static DocumentPtr FindDocumentForTopLevelNode(NodePtr node)
    ...
    // because the parent of all root nodes is nil, so thereÕs
    ...
    // about both of there alternatives and then decided that IÕd use
    // the simple but slow approach instead.
{
    ...
    if (!found) {
        ...
        // its parent document should also exist.  If it doesnÕt, we want to know
        // about it.
        MoreAssertQ(false);
        MoreAssertQ(thisDoc == nil);
    }
    return thisDoc;
}
...
static OSStatus GetSaveOptionsParam(const AppleEvent *theEvent, OSType *saveOptions)
    ...
    // kAEAsk if no parameter was supplied.  ItÕs common code
...
static OSStatus SaveDocument(DocumentPtr doc, ConstFSSpecPtr where)
    ...
    // a document and a pointer to a place where to save it (or nil
    ...
    // of the document in the file.
{
    ...
    // Work out exactly where weÕre saving the document.
    ...
    if (err == noErr) {
        ...
        if (err == noErr) {
            ...
            TimeString(dateTime, true, tmpStr2, nil);
            (void) PLstrcat(tmpStr, tmpStr2);
            (void) PLstrcat(tmpStr, "\p ");
            DateString(dateTime, shortDate, tmpStr2, nil);
            (void) PLstrcat(tmpStr, tmpStr2);
            PLstrcat(tmpStr, "\p\r");
...
static OSStatus CloseDocument(DocumentPtr doc, Boolean quitting, OSType saveOptions)
    ...
    // if weÕre quitting or false if weÕre just saving.  saveOptions determines
    // what to do with unsaved changes.
{
    ...
    if ( IsWindowModified(doc->header.window) ) {
        ...
        // If weÕre told to ask, put up a dialog and get the userÕs chosen action.
        ...
        // Save, or donÕt safe, or cancel.

        if (err == noErr) {
            switch (saveOptions) {
                case kAEYes:
                    err = SaveDocument(doc, nil);
                    break;
                case kAENo:
                    err = noErr;
                    break;
    ...
    // Get rid of the documentÕs data structures.
...
static OSStatus CloseAboutWindow(DialogHeaderPtr header)
    ...
    // About windows donÕt have changes to save, so there are no other
    // parameters.
{
    ...
    // CDocumentMake to create an untitled document.
{
    ...
    MoreAssertQ(dirObjTok->tokType == cApplication);
    MoreAssertQ(theEvent != nil);
...
static pascal OSStatus CApplicationReopen(const MOSLToken *dirObjTok, const AppleEvent *theEvent, AEDesc *result)
    ...
    // CDocumentMake if thereÕs no visible front window.
{
    #pragma unused(dirObjTok)
    OSStatus err;
    AEDesc   properties;
...
static pascal OSStatus CApplicationOpenDocument(const AEDesc *dirObj, const AppleEvent *theEvent, AEDesc *result)
    ...
    // goes into MOSLÕs standard class-first event dispatch.  Because
    // the direct object is a file thingy (alias, FSSSpec, cFile) and not
    // one of MOSLÕs classes, MOSL calls out to the default event handler
    // (for TestMoreOSL this is DefaultAppleEventHandler).  That recognises
    // the 'odoc' event and passes it to this routine.
    //
    // We donÕt have to handle a list of documents because MOSL has
    ...
    // windows until we error or run out of windows.
{
    ...
    if (err == noErr) {
        while (err == noErr && GetWindowList() != nil) {

            // If weÕve run out of visible windows to close,
    ...
    // through to the make routine for the specific class.
{
    ...
    MoreAssertQ(dirObjTok != nil);
    MoreAssertQ(dirObjTok->tokObjType == cApplication);
    MoreAssertQ(theEvent  != nil);
    MoreAssertQ(result    != nil);
    ...
    if (err == noErr) {
        ...
        if (err == noErr) {
            ...
            MoreAssertQ(properties.descriptorType == typeNull);
            err = noErr;
        }
    }
    if (err == noErr) {
    ...
    // The MOSL "getter" object primitive for cApplication.
{
    ...
    if (tok->tokType == typeProperty) {
        MoreAssertQ(tok->tokObjType == cApplication);
        MoreAssertQ(tok->tokData    == nil);
        ...
        MoreAssertQ(desc->descriptorType == typeNull);
        err = noErr;
    }
    return err;
}
    ...
    // The MOSL "counter" object primitive for cApplication.
{
    ...
    MoreAssertQ(containerTok->tokObjType == cApplication);
    MoreAssertQ(result != nil);
    ...
    // The MOSL "getter" object primitive for cWindow.
{
    ...
    if (tok->tokType == typeProperty) {
        ...
        InitObjectMOSLToken(&parentTok, cApplication, nil);
...
static OSStatus CDocumentMake(AEDesc *properties, AEDesc *result)
    ...
    // but I donÕt have time to fix that right now.
{
    OSStatus    err;
    DialogRef   dialog;
    MOSLToken   dialogTok;
    ...
    MoreAssertQ(properties != nil);
    MoreAssertQ(properties->descriptorType == typeNull || properties->descriptorType == typeAERecord);
    MoreAssertQ(result != nil);
    ...
    err = noErr;    
    dialog = GetNewDialog(rDocumentDialog, nil, (WindowRef) -1);
    if (dialog == nil) {
        err = memFullErr;
    }

    // Create the Document record to match the dialog, fill it out
    // and install it in the dialog windowÕs refCon.
    ...
    if (err == noErr) {
        err = CreateNodesFromTemplate(1, nil, &doc->eldest);
    }
    ...
    if (err == noErr) {
        ...
        if (properties->descriptorType != typeNull) {
            ...
            // We canÕt pass the cFile property through to the general
    ...
    // The MOSL "counter" object primitive for cNodeWindow.
{
    ...
    MoreAssertQ(containerTok != nil);
    MoreAssertQ(containerTok->tokType    != typeProperty);
    MoreAssertQ(containerTok->tokObjType == cNodeWindow);
    MoreAssertQ(result != nil);
...
static pascal OSStatus CNodeWindowCoerceToken(const MOSLToken *tok, DescType toClass, MOSLToken *coercedTok)
    ...
    // tokens.  It turns out that, given the token format, itÕs
    // very easy to convert one to the other.
{
    OSStatus err;

    MoreAssertQ(tok != nil);
    MoreAssertQ(tok->tokType == cNodeWindow);
    MoreAssertQ(tok->tokType != toClass);
    ...
    // but I donÕt have time to fix that right now.
{
    ...
    // appropriate, after weÕve finished filling out all the parameters.

    err = noErr;    
    dialog = GetNewDialog(rAboutDialog, nil, (WindowRef) -1);
    if (dialog == nil) {
        err = memFullErr;
    }
    ...
    if (err == noErr) {
        ...
        if (properties->descriptorType != typeNull) {
            ...
            // We canÕt pass the pVisible property through to the general
    ...
    // very easy to convert one to the other.
{
    ...
    MoreAssertQ(tok != nil);
    MoreAssertQ(tok->tokType == cAboutWindow);
    MoreAssertQ(tok->tokType != toClass);
    ...
    // The MOSL "counter" object primitive for cNode.
{
    ...
    MoreAssertQ(containerTok != nil);
    MoreAssertQ(containerTok->tokType    != typeProperty);
    MoreAssertQ(containerTok->tokObjType == cNode);
    MoreAssertQ(result != nil);
...
static pascal OSStatus DefaultAppleEventHandler(const AEDesc *dirObj, const AppleEvent *theEvent, MOSLEventIndex eventIndex,
    ...
    // MOSL calls this routine to handle any events whose direct objectÕs
...
static void TellMyselfToQuit(const EventRecord *event)
    ...
    // donÕt have to worry about recordability (-:  However, one
    ...
    // To handle this, we simply send a 'quit' Apple event to ourselves.
{
    ...
    if (event->modifiers & optionKey) {
        ...
        if (err == noErr) {
            err = MoreAESendEventNoReturnValue(nil, &theEvent);
        }
        MoreAssertQ((err == noErr) || (err == userCanceledErr));
    ...
    // the main event loop.
{
    ...
    err = InitMoreOSL(kEventTable, kEventTableSize, kClassTable, kClassTableSize, DefaultAppleEventHandler, nil);
    ...
    if (err == noErr) {
        ...
        gQuitNow = false;
        do {
            (void) WaitNextEvent(everyEvent, &event, 0xFFFFFFFF, nil);
