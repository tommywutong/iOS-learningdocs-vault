...
source_url: https://developer.apple.com/library/archive/samplecode/MoreOSL/Listings/MoreOSL_MoreOSL_h.html
archived_at: '2026-07-18T03:15:53.434684Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [MoreOSL](MoreOSL.md)


[Next](MoreOSL-MoreOSLHelpers.c.md)[Previous](MoreOSL-MoreOSL.c.md)
...
enum {
    ...
    kMOSLLogOSLMask         = 0x0001,           // log MOSLÕs interaction with OSL
    kMOSLLogCallbacksMask   = 0x0002,           // log MOSLÕs class callbacks
    kMOSLLogGeneralMask     = 0x0004,           // log MOSLÕs general class event handlers
    kMOSLLogDispatchMask    = 0x0008            // log MOSLÕs Apple event dispatcher
};
...
// which youÕre prepared to handle and some basic information about those events.
...
enum {
    ...
    kMOSLReturnDefault,                 // this eventÕs reply is handled in the default way, 
                                        // contrast with below
    kMOSLReturnCountList,               // when the direct object is a list, 
                                        // this eventÕs reply is the count of the list
                                        // typically used for the "count" event
    kMOSLReturnCollapseBooleanList      // when the direct object is a list of Booleans, 
                                        // this eventÕs reply collapses the list to a single Boolean by ANDing each entry
...
//   an object by iterating through the classÕs property list.  The property
...
//   applicationÕs 'aete', nor to the inheritance scheme used by your native
...
//   needed if the property table doesnÕt end with a pInherits.
...
// A classÕs event handler table is very much like the system Apple event dispatch
...
// the corresponding handler in the classÕs event handler table.
//
...
// first in your event table, you must put each classÕs "get data" handler
// first in its event handler table.  Also, the event handler array must
// have an entry for each event in the event table.
//
// If your class doesnÕt handle a specific event, you should set the corresponding
// entry in the event handlers table to nil.  MOSL will return the appropriate
// error (errAECantHandleClass) if someone tries to dispatch an event to a class
// that doesnÕt understand that event.
//
// For the common 'core' events, you can point your event handler to a generic
// implementation that callÕs your classes object primitives.  See the comment
...
typedef pascal OSStatus (*MOSLClassEventHandler)(const MOSLToken *dirObjTok, const AppleEvent *theEvent, AEDesc *result);
    // Each element of the classÕs event handler table is a pointer
...
// element of the classÕs event handler table.
...
// appropriate entry in your classÕs event handler table to point to the generic
// routine.

extern pascal OSStatus MOSLGeneralCount(const MOSLToken *dirObjTok, const AppleEvent *theEvent, AEDesc *result);
    // This routine implements the "count" Apple event based on your classÕs
...
extern pascal OSStatus MOSLGeneralGetData(const MOSLToken *dirObjTok, const AppleEvent *theEvent, AEDesc *result);
    // This routine implements the "get data" Apple event based on your classÕs
...
extern pascal OSStatus MOSLGeneralSetData(const MOSLToken *dirObjTok, const AppleEvent *theEvent, AEDesc *result);
    // This routine implements the "set data" Apple event based on your classÕs
...
extern pascal OSStatus MOSLSetObjectProperties(const MOSLToken *objTok, const AERecord *data);
    ...
    // the routine will call your classÕs "setter" object primitive to set the
...
typedef pascal OSStatus (*MOSLClassCounter)(const MOSLToken *containerTok, DescType elementType, SInt32 *result);
    ...
    // of type elementType.
    //
    // o primitive may be nil if class doesnÕt have elements
    ...
    // o you should return kClassHasNoElementsOfThisTypeErr if element type isnÕt one of your elements
    // o you should return errAENoSuchObject if the container doesnÕt contain the specified object
...
typedef pascal OSStatus (*MOSLClassAccessByUniqueID)(const MOSLToken *containerTok, DescType elementType, SInt32 uniqueID, MOSLToken *valueTok);
    ...
    //
    // o primitive may be nil if class doesnÕt support accessing objects by unique ID
    // o containerTok is always an object, never a property
    // o elementType may be cObject, which you should treat as your default element type
    // o you should return kClassHasNoElementsOfThisTypeErr if element type isnÕt one of your elements
    // o you should return errAENoSuchObject if the container doesnÕt contain the specified object\
...
typedef pascal OSStatus (*MOSLClassAccessByName)(const MOSLToken *containerTok, DescType elementType, const AEDesc *name, MOSLToken *valueTok);
    ...
    //
    // o primitive may be nil if class doesnÕt support accessing objects by name
    // o containerTok is always an object, never a property
    // o elementType may be cObject, which you should treat as your default element type
    // o you should return kClassHasNoElementsOfThisTypeErr if element type isnÕt one of your elements
    // o you should return errAENoSuchObject if the container doesnÕt contain the specified object
...
typedef pascal OSStatus (*MOSLClassGetter)(const MOSLToken *tok, AEDesc *desc);
    ...
    // o only called for properties in your classÕs property table (getter 
...
typedef pascal OSStatus (*MOSLClassSetter)(const MOSLToken *tok, const AEDesc *desc);
    ...
    // o only called for properties in your classÕs property table (setter 
    //   infrastructure checks property table and returns errAENoSuchObject)
    ...
    // o routine is optional, may be nil if no properties are read/write
...
typedef pascal OSStatus (*MOSLClassCoerceToken)(const MOSLToken *tok, DescType toClass, MOSLToken *coercedTok);
    ...
    //   to the base class
    // o coercedTok can be nil
    // o return errAECoercionFail if toClass isnÕt a superclass of tok
...
// IMPORTANT:
// The first entry of the class table *must* be the applicationÕs class
// (cApplication).
...
typedef pascal OSStatus (*MOSLDefaultEventHandler)(const AEDesc *dirObj, const AppleEvent *theEvent, MOSLEventIndex eventIndex, AEDesc *result);
    ...
    // you donÕt otherwise recognise, such as the "open" event on class
    // typeFSS.
    //
...
typedef pascal void (*MOSLErrorToStringProc)(OSStatus errNum, Str255 errStr);
    ...
    // errors that your various MOSL callbacks might return.  OTOH, you donÕt
...
extern pascal OSStatus InitMoreOSL(MOSLEventTablePtr eventTableBase, MOSLEventIndex eventTableSize,
    ...
    // the pointer must not be nil) although they may be empty (ie the size can
