...
source_url: https://developer.apple.com/library/archive/samplecode/vrscript/Listings/Feature_Files_VREffects_c.html
archived_at: '2026-07-26T19:53:02.439792Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [vrscript](vrscript.md)


[Next](Feature%20Files-VREffects.h.md)[Previous](Feature%20Files-VRActions.h.md)
...
//
//     <11>     04/15/02    rtm     added call to QDFlushPortBuffer, so we work on Mac OS X
...
OSErr VREffects_RunTransitionEffect (WindowObject theWindowObject)
{
    ...
    myPort = (CGrafPtr)QTFrame_GetPortFromWindowReference((**theWindowObject).fWindow);
    if (myPort == NULL)
        goto bail;
...
#if TARGET_OS_MAC
        // if the window is double-buffered, flush the buffer to the screen
        if (QDIsPortBuffered(myPort))
            if (QDIsPortBufferDirty(myPort))
                QDFlushPortBuffer(myPort, NULL);
#endif
...
bail:
    ...
    MCSetControllerPort((**theWindowObject).fController, myPort);
    SetMovieGWorld((**theWindowObject).fMovie, myPort, NULL);
    MCMovieChanged((**theWindowObject).fController, (**theWindowObject).fMovie);
