...
source_url: https://developer.apple.com/library/archive/samplecode/CompositedEffects.win/Listings/src_CompositedEffects_java.html
archived_at: '2026-07-18T03:04:07.978595Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [CompositedEffects.win](CompositedEffects.win.md)


[Next](src-ControlPanel.java.md)[Previous](README.txt.md)

# src/CompositedEffects.java
...
// add the background picture to the Comp -> load it into memory so it draws quicker
        ...
        GraphicsImporterDrawer if1 = new GraphicsImporterDrawer (backgroundFile);
        if1.setDisplayBounds (r);
        ImagePresenter background = ImagePresenter.fromGraphicsImporterDrawer (if1);
...
// when any modifier key is pressed when the mousePressed event is generated
        ...
        sh.addController(new SWController (new Dragger (MouseResponder.kAnyModifiersMask,
...
// resizing the movie to make it a little smaller
        QTFile movieFile = new QTFile (QTFactory.findAbsolutePath ("jumps.mov"));
