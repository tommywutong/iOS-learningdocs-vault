...
source_url: https://developer.apple.com/library/archive/samplecode/qtbigscreen/Listings/Application_Files_ComApplication_c.html
archived_at: '2026-07-26T19:52:44.811857Z'
---
> 导航：[总目录](../../README.md) · [samplecode](../../_indexes/samplecode.md) · [qtbigscreen](qtbigscreen.md)


[Next](Application%20Files-ComApplication.h.md)[Previous](qtbigscreen.md)
...
//
//     <5>      04/27/02    rtm     added fBeginFullscreenNeeded
...
extern WindowObject     gFullScreenWindowObject;
...
void QTApp_Draw (WindowReference theWindow)
{
    ...
    if (QTFrame_IsMovieWindow(theWindow))
        if ((gFullScreenWindowObject == NULL) || ((**gFullScreenWindowObject).fWindow != theWindow))
            EraseRect(&myRect);
...
Boolean QTApp_HandleKeyPress (char theCharCode)
{
    ...
    switch (theCharCode) {
        // Escape key during fullscreen display means to restore the window to its original state
        case kEscapeCharCode:
            if (gFullScreenWindowObject != NULL)
                isHandled = (QTBig_StopFullscreen(gFullScreenWindowObject) == noErr);
            break;
    }
...
Boolean QTApp_HandleMenu (UInt16 theMenuItem)
{
    ...
    switch (theMenuItem) {
        ...
        case IDM_FULLSCREEN:
            QTBig_StartFullscreen(QTFrame_GetWindowObjectFromFrontWindow());
            myIsHandled = true;
            break;
...
#pragma unused(theMenu)
    ...
    QTFrame_SetMenuItemState(myMenu, IDM_FULLSCREEN, myMC == NULL ? kDisableMenuItem : kEnableMenuItem);
}
...
Boolean QTApp_HandleEvent (EventRecord *theEvent)
{
    ...
    // see if there are any windows marked for closing or ending fullscreen mode
    ...
    while (myWindow != NULL) {
        ...
        if (myAppData != NULL) {
            ...
            if ((**myAppData).fBeginFullscreenNeeded) {
                (**myAppData).fBeginFullscreenNeeded = false;
                QTBig_StartFullscreen(QTFrame_GetWindowObjectFromWindow(myWindow));
            }

            if ((**myAppData).fEndFullscreenNeeded) {
                (**myAppData).fEndFullscreenNeeded = false;
                QTBig_StopFullscreen(QTFrame_GetWindowObjectFromWindow(myWindow));
            }
    ...
    // if the movie is playing big screen, process events for it
    if ((**myAppData).fFullScreenWindow != NULL)
        return(MCIsPlayerEvent((**myWindowObject).fController, theEvent));
...
void QTApp_SetupWindowObject (WindowObject theWindowObject)
{
    ...
    (**theWindowObject).fAppData = (Handle)QTBig_InitWindowData(theWindowObject);
}
