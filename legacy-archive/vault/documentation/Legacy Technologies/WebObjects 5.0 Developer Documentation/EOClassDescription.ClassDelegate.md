...
source_url: https://developer.apple.com/library/archive/documentation/LegacyTechnologies/WebObjects/WebObjects_5/EOControlRef/Java/Protocols/EOClassDescClassDelegate.html
archived_at: '2026-07-15T08:13:47.792124Z'
---
...
Invoked from propagateDeleteForObject. If the class delegate returns false, it prevents _anObject_ in _anEditingContext_ from propagating deletion to the objects at the destination of _key_. This can be useful if you have a large model and a small application that only deals with a subset of the model's entities. In such a case you might want to disable delete propagation to entities that will never be accessed. You should use this method with caution, however-returning false and not propagating deletion can lead to dangling references in your object graph.
