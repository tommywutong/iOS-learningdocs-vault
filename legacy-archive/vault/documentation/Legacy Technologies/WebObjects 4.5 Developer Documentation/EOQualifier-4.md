...
source_url: https://developer.apple.com/library/archive/documentation/LegacyTechnologies/WebObjects/WebObjects_4.5/System/Library/Frameworks/EOControl.framework/ObjC_classic/Classes/More/EOQualifier.html
archived_at: '2026-07-15T08:11:41.459776Z'
---
...
cases you simply create a qualifier using the EOQualifier class method __qualifierWithQualifierFormat__:,
as follows:
...
as an argument to [qualifierWithQualifierFormat:](EOQualifier-3.md#apple-f4xwc4dqnrsv64tfmyxw6y3df5rwy3jpivhvc5lbnruwm2lfoixxc5lbnruwm2lfojlws5dikf2wc3djmzuwk4sgn5zg2yluhi) returns
...
This is illustrated in [Figure 0-4](#apple-ijbesqsgincus), in which the format string "salary > 300 AND firstName
...
The __qualifierWithQualifierFormat:__ method
...
a format string by using [qualifierWithQualifierFormat:](EOQualifier-3.md#apple-f4xwc4dqnrsv64tfmyxw6y3df5rwy3jpivhvc5lbnruwm2lfoixxc5lbnruwm2lfojlws5dikf2wc3djmzuwk4sgn5zg2yluhi).
This method takes as an argument a format string somewhat like that used
with the standard C __printf()__ function.
...
(for example, %x), an NSInvalidArgumentException is raised.

For example, suppose you have an Employee entity with the
properties __empID__, __firstName__, __lastName__, __salary__,
and __department__ (representing a to-one relationship
...
accessing the given attributes: __lastName__ and __salary__ for
Employee objects, and __name__ for Department
...
as described in the section ["Using Different Data Types in Format Strings" on page 267](#apple-ijbesrcgivaum).

The operators you can use in constructing qualifiers are
=, ==, !=, <, >, <=, >=, "like", and "caseInsensitiveLike".
The __like__ and __caseInsensitiveLike__ operators
can be used with wildcards to perform pattern matching, as described
in ["Using Wildcards and the like Operator" on page 266](#apple-ijbesqsjijaue).
...
When you use the __like__ or __caseInsensitiveLike__ operator
...
in expressions that use the __like__ or __caseInsensitiveLike__ operator.
...
The format strings you use to initialize a qualifier can include selectors.
The parser recognizes an unquoted string followed by a colon (such
as __myMethod:__) as a selector. For example:
...
Selectors in a qualifier are parsed and applied only in memory;
that is, they can't be used in to qualify fetches in a database.

## Using Different Data Types in Format Strings

As stated in the section ["Constructing Format Strings" on page 264](#apple-ijbesq2ginfem),
format strings normally include only two data types: strings and
numbers. To get around this limitation, you can perform explicit casting.

For example, NSCalendarDate and NSDecimalNumber are two classes
that are likely to be used in qualifiers. You can construct format
strings for objects of these classes as follows:
...
technique only works for classes that implement __initWithString:__.

Note that to construct a date qualifier using a format string,
...
This limitation doesn't apply when you're working with
...
[filteredArrayUsingQualifier:](NSArray%20Additions.md#apple-f4xwc4dqnrsv64tfmyxw6y3df5uw443unuxu4u2bojzgc6jaifsgi2lunfxw44zpmzuwy5dfojswiqlsojqxsvltnfxgoulvmfwgsztjmvzdu) is
a method that Enterprise Objects Framework adds to NSArray. It's used
for filtering in-memory arrays.

## Creating Subclasses

EOQualifier offers extensibility across two dimensions: new
classes can be added to extend qualifier semantics, and categories
can be added to extend functionality (for example, to provide in-memory evaluation).

Subclasses used to evaluate objects in memory must implement
the EOQualifierEvaluation protocol. Subclasses used to generate
SQL queries must conform to the EOQualifierSQLGeneration protocol.
