...
source_url: https://developer.apple.com/library/archive/documentation/LegacyTechnologies/WebObjects/WebObjects_4.5/System/Library/Frameworks/SybaseEOAdaptor.framework/Java/Introduction.html
archived_at: '2026-07-15T08:11:46.481407Z'
---
...
- [HOSTNAME](SybaseAdaptor.md#apple-inceeqski5fec)
- [DATABASENAME](SybaseAdaptor.md#apple-inceeskbiveem)
- [USERNAME](SybaseAdaptor.md#apple-inceeskjircuc)
- [PASSWORD](SybaseAdaptor.md#apple-inceercjindeq)
- [LC_ALL_KEY](SybaseAdaptor.md#apple-inceerchjjbek)
- [ENCRYPTPASSWORD](SybaseAdaptor.md#apple-inceerkkizfeu)
- [PRIMITIVE_TYPE_MAP](SybaseAdaptor.md#apple-inceeqsjjfces)
...
See the [SybaseAdaptor](SybaseAdaptor.md#apple-inceer2fi5feg) class specification
...
- You send the adaptor channel a __selectAttributes__ message
  with true specified as the value for the lock keyword.
...
type (the __external__ type) and the Java type (the __internal__ type)
to which a database value will be coerced when it's fetched from
the database. The following table lists the mapping used by SybaseAdaptor.
...
to String-the Java type to which __partnumber__'s base
data type (`char`) is mapped.
SybaseAdaptor's implementation of __describeModelWithTableNames__ automatically
...
connection dictionary, the type mapping methods- [externalToInternalTypeMap](SybaseAdaptor.md#apple-f4xwc4dqnrsv64tfmyxwuylwmexwg3dnf5jxsytbonsuczdbob2g64rpmv4hizlsnzqwyvdpjfxhizlsnzqwyvdzobsu2ylq), __internalTypeForExternalType__,
...
| `city` | `varchar` | String | `columnName = CITY` `width = 50` |
| `date` | `datetime` | NSGregorianDate | `columnName = ""` |
| `longText` | `text` | String |  |
| `money` | `money` | BigNumber | `columnName = ""` |
| `phoneNumber` | `varchar` | String | `columnName = PHONE` `width = 20` |
| `rawImage` | `image` | NSData | `columnName = RAW_IMAGE` |
| `state` | `varchar` | String | `columnName = STATE` `width = 2` |
| `streetAddress` | `varchar` | String | `columnName = STREET_ADDRESS` `width = 100` |
...
| `uniqueID` | `int` | Number | `columnName = ""` `valueType = i` |
| `zipCode` | `varchar` | String | `columnName = ZIP` `width = 10` |
...
procedure from __primaryKeyForNewRowWithEntity__ and
