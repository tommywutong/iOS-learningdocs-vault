...
source_url: https://developer.apple.com/library/archive/documentation/LegacyTechnologies/WebObjects/WebObjects_4.0/System/Library/Frameworks/EOControl.framework/Resources/English.lproj/Documentation/Reference/Java/Classes/More/EOQualifier_m.html
archived_at: '2026-07-18T01:28:31.319777Z'
---
...
As described above, there are several EOQualifier subclasses, each of which represents a different semantic. However, in most cases you simplycreate a qualifier using the EOQualifier static method __qualifierWithQualifierFormat__ `:`, as follows:

> ```
> EOQualifier qual = Qualifier.qualifierWithQualifierFormat("lastName = 'Smith'", null);
> ```

The qualifier or group of qualifiers that result from such a statement is based on the contents of the format string you provide. For example, giving the format string "lastName = 'Smith'" as an argument to [__qualifierWithQualifierFormat__](EOQualifier.md)returns an EOKeyValueQualifier object. But you don't normally need to be concerned with this level of detail.
...
__Figure 4__ EOQualifier Tree for __salary > 300 AND firstName = "Angela" AND manager.name = "Fred"__ __Note:__ The [__qualifierWithQualifierFormat__](EOQualifier.md)method can't be used to create an instance of EOSQLQualifier. This is because EOSQLQualifier uses a non-structured syntax. It also requires an entity. To create an instance of EOSQLQualifier, you'd use a statement such as the following:
...
As described above, you typically create a qualifier from a format string by using [__qualifierWithQualifierFormat__](EOQualifier.md). This method takes as an argument a format string somewhat like that used with the standard C __printf()__ function. The format string can embed strings, numbers, and other objects using the conversion specification %@. The second argument to [__qualifierWithQualifierFormat__](EOQualifier.md)is an array that contains the value or result to substitute for any %@ conversion specifications. This allows qualifiers to be built dynamically. The following table lists the conversion specifications you can use in a format string and their corresponding data types.

| __Conversion Specification__ | __Expected Value or Result__ |
| %@ | It can either be an object whose __toString__ (or __description__ ) method returns a key (in other words, a java.lang.String), or a value object such as an java.lang.String, java.lang.Number, java.util.CalendarDate, and so on. |
| %% | Results in a literal __%__ character. |

```
```

__Note:__ If you use an unrecognized character in a conversion specification (for example, %x), an exception is thrown.
...
[__filteredArrayWithQualifier__](EOQualifier.md)is a method that returns an array containing objects from the provided array that match the provided qualifier.
