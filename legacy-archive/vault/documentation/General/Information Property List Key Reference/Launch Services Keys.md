...
source_url: https://developer.apple.com/library/archive/documentation/General/Reference/InfoPlistKeyReference/Articles/LaunchServicesKeys.html
archived_at: '2026-07-15T07:34:59.279384Z'
---
...
[Next](Cocoa%20Keys.md)[Previous](Core%20Foundation%20Keys.md)
...
Launch Services keys use the prefix `LS` to distinguish them from other keys. For more information about Launch Services in macOS, see _[Launch Services Programming Guide](../../Carbon/Launch%20Services%20Programming%20Guide/Introduction.md#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydsojz)_ and _[Launch Services Reference](https://developer.apple.com/documentation/coreservices/launch_services)_.
...
__Table 1__  Summary of Launch Services keys

| Key | Xcode name | Summary | Platforms |
| LSApplicationCategoryType | “Application Category” | Contains a string with the UTI that categorizes the app for the App Store. See [LSApplicationCategoryType](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfvjvooa) for details. | macOS |
| LSApplicationQueriesSchemes | None | Specifies the URL schemes the app is able to test using the [canOpenURL:](https://developer.apple.com/documentation/uikit/uiapplication/1622952-canopenurl) method. See [LSApplicationQueriesSchemes](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfvjvomju) for details. | iOS 9.0 and later |
| LSArchitecturePriority | “Architecture priority” | Contains an array of strings identifying the supported code architectures and their preferred execution priority. See [LSArchitecturePriority](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfuytanrygi2s2vcqlbjekrrrgi2a) for details. | macOS |
| LSBackgroundOnly | “Application is background only” | Specifies whether the app runs only in the background. (Mach-O apps only). See [LSBackgroundOnly](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrga4tenry) for details. | macOS |
| LSEnvironment | “Environment variables” | Contains a list of key/value pairs, representing environment variables and their values. See [LSEnvironment](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrga3dqmrv) for details. | macOS |
| LSFileQuarantineEnabled | “File quarantine enabled” | Specifies whether the files this app creates are quarantined by default. See [LSFileQuarantineEnabled](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfvjvomjq). | macOS |
| LSFileQuarantineExcludedPathPatterns | None | Specifies directories for which files should not be automatically quarantined. See [LSFileQuarantineExcludedPathPatterns](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfvjvonq). | macOS |
| LSGetAppDiedEvents | “Application should get App Died events” | Specifies whether the app is notified when a child process dies. See [LSGetAppDiedEvents](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrga3dqnzt) for details. | macOS |
| LSMinimumSystemVersion | “Minimum system version” | Specifies the minimum version of macOS required for the app to run. See [LSMinimumSystemVersion](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrgeztenjt) for details. | macOS |
| LSMinimumSystemVersionByArchitecture | “Minimum system versions, per-architecture” | Specifies the minimum version of macOS required to run a given platform architecture. See [LSMinimumSystemVersionByArchitecture](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfuytcmzsguzs2qsbjjceqqkdii) for details. | macOS |
| LSMultipleInstancesProhibited | “Application prohibits multiple instances” | Specifies whether one user or multiple users can launch an app simultaneously. See [LSMultipleInstancesProhibited](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrgeztgmbq) for details. | macOS |
| LSRequiresIPhoneOS | “Application requires iPhone environment” | Specifies whether the app is an iOS app. See [LSRequiresIPhoneOS](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfvjvomq) for details. | iOS |
| LSRequiresNativeExecution | “Application requires native environment” | Specifies whether the app must run natively on an Intel-based Mac, as opposed to under Rosetta emulation. See [LSRequiresNativeExecution](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfuytaobrgy3c2vcqlbjekrrrgi4a) for details. | macOS |
| LSSupportsOpeningDocumentsInPlace | None | Enables your app to directly open original documents, not copies, from other apps. See [LSSupportsOpeningDocumentsInPlace](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfvjvomjt) for details. | iOS |
| LSUIElement | “Application is agent (UIElement)” | Specifies whether the app is an agent app, that is, an app that should not appear in the Dock or Force Quit window. See [LSUIElement](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrga4denjw) for details. | macOS |
| LSUIPresentationMode | “Application UI Presentation Mode” | Sets the visibility of system UI elements when the app launches. See [LSUIPresentationMode](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrgeztmmjw) for details. | macOS |
| LSVisibleInClassic | “Application is visible in Classic” | Specifies whether an agent app or background-only app is visible to other apps in the Classic environment. See [LSVisibleInClassic](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrgeztqmzr) for details. | macOS |
| MinimumOSVersion | “Minimum system version” | Do not use. Use [LSMinimumSystemVersion](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrgeztenjt) instead. | iOS |
...
__Table 2__  UTIs for app categories
...
__Table 3__  UTIs for game-specific categories
...
`LSApplicationQueriesSchemes` (`Array` - iOS) Specifies the URL schemes you want the app to be able to use with the [canOpenURL:](https://developer.apple.com/documentation/uikit/uiapplication/1622952-canopenurl) method of the [UIApplication](https://developer.apple.com/documentation/uikit/uiapplication) class. For each URL scheme you want your app to use with the `canOpenURL:` method, add it as a string in this array. Read the [canOpenURL:](https://developer.apple.com/documentation/uikit/uiapplication/1622952-canopenurl) method description for important information about declaring supported schemes and using that method.

To learn about the converse operation of registering the URL schemes an app can handle, read the description of the [CFBundleURLTypes](Core%20Foundation%20Keys.md#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambrgqztcljrgazdembx) key.
...
__Table 4__  Execution architecture identifiers
...
`LSRequiresNativeExecution` (`Boolean` - macOS) specifies whether to launch the app using the subbinary for the current architecture. If this key is set to `YES`, Launch Services always runs the app using the binary compiled for the current architecture. You can use this key to prevent a universal binary from being run under Rosetta emulation on an Intel-based Mac. For more information about configuring the execution architectures, see [LSArchitecturePriority](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjqfuytanrygi2s2vcqlbjekrrrgi2a).

`LSSupportsOpeningDocumentsInPlace` (`Boolean` - iOS) When set to a value of `YES`, enables your app to open the _original_ document from a file provider, rather than a _copy_ of the document. The app can access documents from the system’s local file provider, the iCloud file provider, and any third-party File Provider extensions that support opening documents in place.

The URL for a document opened in place is security-scoped. For information about working with security-scoped URLs and bookmarks, read the overview in _[NSURL Class Reference](https://developer.apple.com/documentation/foundation/nsurl)_ and read [Document Provider](../App%20Extension%20Programming%20Guide/FileProvider.md#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqge2demjufvbuqmjy) in _[App Extension Programming Guide](https://developer.apple.com/library/archive/documentation/General/Conceptual/ExtensibilityPG/index.html#//apple_ref/doc/uid/TP40014214)_.

In iOS 11 and later, if both this key and the [UIFileSharingEnabled](iOS%20Keys.md#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga4tenjsfvjvomrq) key are `YES`, the local file provider grants access to all the documents in the app’s `Documents` directory. These documents appear in the Files app, and in a document browser. Users can open and edit these document in place.
...
`MinimumOSVersion` (`String` - iOS). When you build an iOS app, Xcode uses the iOS Deployment Target setting of the project to set the value for the `MinimumOSVersion` key. Do _not_ specify this key yourself in the `Info.plist` file; it is a system-written key. When you publish your app to the App Store, the store indicates the iOS releases on which your app can run based on this key. It is equivalent to the `LSMinimumSystemVersion` key in macOS.
