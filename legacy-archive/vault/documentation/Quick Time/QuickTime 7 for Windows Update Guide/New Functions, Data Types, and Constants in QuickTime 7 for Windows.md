...
source_url: https://developer.apple.com/library/archive/documentation/QuickTime/Conceptual/QT7Win_Update_Guide/Chapter03/03QT7_Update_Guide.html
archived_at: '2026-07-18T01:58:38.487993Z'
---
...
[Next](Document%20Revision%20History.md)[Previous](What%E2%80%99s%20New%20in%20QuickTime%207%20for%20Windows.md)
...
Parameters
lpBuffer
A pointer to the buffer to receive the null-terminated
string containing the path to the QuickTime Applications directory.
This path does not end with a backslash unless the system directory
is the root directory.
uSize
The maximum size of the buffer, in TCHARs.
This value should be set to at least MAX_PATH+1 to allow sufficient
...
this call looks up the value of
...
lpBuffer
A pointer to the buffer to receive the null-terminated
string containing the path to the QuickTime Components directory.
This path does not end with a backslash unless the system directory
is the root directory.
uSize
The maximum size of the buffer, in TCHARs.
...
lpBuffer
A pointer to the buffer to receive the null-terminated
string containing the path to the QuickTime Extensions directory.
This path does not end with a backslash unless the system directory
is the root directory.
uSize
The maximum size of the buffer, in TCHARs.
...
extensions files, whose names ends with
.qtx
and
.qtr
.
...
.qtx
file contains
the data fork of an extension––a Windows DLL, while a
.qtr
file
contains the resource fork of an extension for Macintosh-style resources).
Note that this call looks up the value of
HKEY_LOCAL_MACHINE\SOFTWARE\Apple
...
lpBuffer
A pointer to the buffer to receive the null-terminated
string containing the path to the QuickTime System directory. This
path does not end with a backslash unless the system directory is
the root directory.
uSize
The maximum size of the buffer, in TCHARs.
This value should be set to at least MAX_PATH+1 to allow sufficient
...
others. Note that this call looks up the value of
...
Begins a movie audio extraction session and will return `badCallOrderErr` if
...
StringFromGUID2
,
then wrapped in a CFStringRef using
...
```
Discussion
This routine takes the name of the QuickTime DLL and searches
all the places where it can be installed.
```
...
there is no error. See `[“Metadata Error Codes”](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqgazdinzwfvbuqmzrgqwtmmrvheyds)`.
...
See `[“Metadata Error Codes”](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqgazdinzwfvbuqmzrgqwtmmrvheyds)`.
...
or `[noErr](../../../../Networking/Conceptual/NSL/NSL3/NSL36.md#apple-f4xwc4dqnrsv64tfmyxwi33df5rv64tfmyxw432fojza)` if there is no error. See `[“Metadata Error Codes”](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqgazdinzwfvbuqmzrgqwtmmrvheyds)`.
...
```
Discussion
Note that these are variable-sized structures that must be
...
a movie property. See [New Movie Property Codes](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqgazdinzwfvbuqmzrgqwtmmrvhe3tm).
...
parse CF dictionaries returned by `kQTSGAudioPropertyID_DeviceListWithAttributes` and `kQTSGAudioPropertyID_DeviceAttributes` IDs
