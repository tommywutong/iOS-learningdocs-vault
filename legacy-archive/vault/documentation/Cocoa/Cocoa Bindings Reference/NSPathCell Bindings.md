...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSPathCell.html
archived_at: '2026-07-15T07:21:42.534803Z'
---
...
[Next](NSPathControl%20Bindings.md)[Previous](NSPanel%20Bindings.md)

# NSPathCell Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSPathCell](https://developer.apple.com/documentation/appkit/nspathcell) |
| __Availability__ | Available in OS X v10.5 and later. |

A multiple-value binding that determines if the `NSPathCell` is editable in the user interface.
...
The contents of the `NSPathCell` are editable if a logical AND operation on all the `editable` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSPathCell` is enabled in the user interface.
...
The `NSPathCell` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSPathCell`.
...
A Boolean value that determines if the NSFont used to display the `NSPathCell` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSPathCell`.
...
A Boolean value that determines if the NSFont used to display the `NSPathCell` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSPathCell`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSPathCell`.
...
An `NSString` or file `NSURL` that is displayed as the content of the `NSPathCell`.
