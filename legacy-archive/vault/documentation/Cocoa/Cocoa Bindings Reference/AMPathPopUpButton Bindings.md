...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/AMPathPopUpButton.html
archived_at: '2026-07-15T07:21:29.686917Z'
---
...
[Next](NSArrayController%20Bindings.md)[Previous](Binding%20Options.md)

# AMPathPopUpButton Bindings

|  |  |
| --- | --- |
| __Related class__ | AMPathPopUpButton |
| __Availability__ | Available in OS X v10.4 and later. |

The AMPathPopUpButton is available in the Automator palette and is part of the Automator framework.

This class is intended for use with Automator actions. Linking to the Automator framework from a Cocoa application also links against the AppleScriptKit framework which, in essence, causes an application to become an AppleScript Studio application. This is generally not desirable as AppleScript Studio has a large scripting terminology that can hide your application's scripting terminology. AppleScript Studio also declares categories that may interfere with your application.

A multiple-value binding that determines if the `AMPathPopUpButton` is enabled in the user interface.
...
The `AMPathPopUpButton` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `AMPathPopUpButton` is displayed in the user interface.
...
The `AMPathPopUpButton` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `AMPathPopUpButton`.
...
A Boolean value that determines if the NSFont used to display the `AMPathPopUpButton` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `AMPathPopUpButton`.
...
A Boolean value that determines if the NSFont used to display the `AMPathPopUpButton` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `AMPathPopUpButton`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `AMPathPopUpButton`.
...
An NSString that specifies the path of the `AMPathPopUpButton`.
...
An NSString that contains the tool tip to display for this `AMPathPopUpButton`.
...
This binding is exposed by the superclass and is not used by `AMPathPopUpButton`.
...
__Availability:__Available in OS X v10.4 and later.[Next](NSArrayController%20Bindings.md)[Previous](Binding%20Options.md)
