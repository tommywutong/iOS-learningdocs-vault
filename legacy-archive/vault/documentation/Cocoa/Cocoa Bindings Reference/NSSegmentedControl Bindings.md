...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSSegmentedControl.html
archived_at: '2026-07-15T07:21:48.558936Z'
---
...
[Next](NSSlider%20Bindings.md)[Previous](NSSegmentedCell%20Bindings.md)

# NSSegmentedControl Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSSegmentedControl](https://developer.apple.com/documentation/appkit/nssegmentedcontrol) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSSegmentedControl` is enabled in the user interface.
...
The `NSSegmentedControl` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSSegmentedControl` is displayed in the user interface.
...
The `NSSegmentedControl` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSSegmentedControl`.
...
A Boolean value that determines if the NSFont used to display the `NSSegmentedControl` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSSegmentedControl`.
...
A Boolean value that determines if the NSFont used to display the `NSSegmentedControl` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSSegmentedControl`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSSegmentedControl`.
...
An NSString that contains the tool tip to display for this `NSSegmentedControl`.
...
An integer value that specifies the index of the selected item in the `NSSegmentedControl`. When the selection changes in the `NSSegmentedControl`, this value is updated with the index of the newly selected item.
...
An NSString that specifies the label of the currently selected tab in the `NSSegmentedControl`.
...
An integer value that specifies the tag of the selected item in the `NSSegmentedControl`. When the selection changes in the `NSSegmentedControl`, this value is updated with the tag of the newly selected item.
