...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSSliderCell.html
archived_at: '2026-07-15T07:21:49.554266Z'
---
...
[Next](NSSplitView%20Bindings.md)[Previous](NSSlider%20Bindings.md)

# NSSliderCell Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSSliderCell](https://developer.apple.com/documentation/appkit/nsslidercell) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSSliderCell` is editable in the user interface.
...
The contents of the `NSSliderCell` are editable if a logical AND operation on all the `editable` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSSliderCell` is enabled in the user interface.
...
The `NSSliderCell` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A `double` value that specifies the maximum value allowed by the `NSSliderCell`.
...
A `double` value that specifies the minimum value allowed by the `NSSliderCell`.
...
A `double` value that specifies the current value of the `NSSliderCell`.
