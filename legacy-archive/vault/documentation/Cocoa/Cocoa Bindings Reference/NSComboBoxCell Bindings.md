...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSComboBoxCell.html
archived_at: '2026-07-15T07:21:34.733263Z'
---
...
[Next](NSDatePicker%20Bindings.md)[Previous](NSComboBox%20Bindings.md)

# NSComboBoxCell Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSComboBoxCell](https://developer.apple.com/documentation/appkit/nscomboboxcell) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSComboBoxCell` is editable in the user interface.
...
The contents of the `NSComboBoxCell` are editable if a logical AND operation on all the `editable` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSComboBoxCell` is enabled in the user interface.
...
The `NSComboBoxCell` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSComboBoxCell`.
...
A Boolean value that determines if the NSFont used to display the `NSComboBoxCell` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSComboBoxCell`.
...
A Boolean value that determines if the NSFont used to display the `NSComboBoxCell` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSComboBoxCell`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSComboBoxCell`.
...
An NSColor that specifies the color used to display the text of the `NSComboBoxCell`.
...
An NSArrayController instance that provides the content of the `NSComboBoxCell`.

Unless `contentValues` is also bound, the titles of the items in the `NSComboBoxCell` are derived by invoking invoking the `description`method for each of the content objects.
...
An array of strings that are displayed as the items in the `NSComboBoxCell`.
...
An NSString or NSNumber that specifies the value of the `NSComboBoxCell`.
