...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSSlider.html
archived_at: '2026-07-15T07:21:49.052643Z'
---
...
[Next](NSSliderCell%20Bindings.md)[Previous](NSSegmentedControl%20Bindings.md)

# NSSlider Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSSlider](https://developer.apple.com/documentation/appkit/nsslider) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSSlider` is enabled in the user interface.
...
The `NSSlider` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSSlider` is displayed in the user interface.
...
The `NSSlider` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
A `double` value that specifies the maximum value allowed by the `NSSlider`.
...
A `double` value that specifies the minimum value allowed by the `NSSlider`.
...
A `double` value that specifies the current value of the `NSSlider`.
