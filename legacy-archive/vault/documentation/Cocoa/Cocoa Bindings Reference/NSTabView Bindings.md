...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSTabView.html
archived_at: '2026-07-15T07:21:51.567556Z'
---
...
[Next](NSTabViewItem%20Bindings.md)[Previous](NSStepperCell%20Bindings.md)

# NSTabView Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSTabView](https://developer.apple.com/documentation/appkit/nstabview) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSTabView` is displayed in the user interface.
...
The `NSTabView` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSTabView`.
...
A Boolean value that determines if the NSFont used to display the `NSTabView` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSTabView`.
...
A Boolean value that determines if the NSFont used to display the `NSTabView` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSTabView`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSTabView`.
...
An NSString that contains the tool tip to display for this `NSTabView`.
...
An object that specifies the identifier of the currently selected tab in the `NSTabView`.
...
An NSString that specifies the label of the currently selected tab in the `NSTabView`.
