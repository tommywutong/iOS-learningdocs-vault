...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSForm.html
archived_at: '2026-07-15T07:21:37.016212Z'
---
...
[Next](NSFormCell%20Bindings.md)[Previous](NSDrawer%20Bindings.md)

# NSForm Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSForm](https://developer.apple.com/documentation/appkit/nsform) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSForm` is enabled in the user interface.
...
The `NSForm` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSForm` is displayed in the user interface.
...
The `NSForm` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSForm`.
...
A Boolean value that determines if the NSFont used to display the `NSForm` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSForm`.
...
A Boolean value that determines if the NSFont used to display the `NSForm` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSForm`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSForm`.
...
An NSString that contains the tool tip to display for this `NSForm`.
