...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/QCView.html
archived_at: '2026-07-15T07:21:58.579617Z'
---
...
[Next](Document%20Revision%20History.md)[Previous](PDFThumbnailView%20Bindings.md)

# QCView Bindings

|  |  |
| --- | --- |
| __Related class__ | [QCView](https://developer.apple.com/documentation/quartz/qcview) |
| __Availability__ | Available in OS X v10.5 and later. |

A multiple-value binding that determines if the `QCView` is displayed in the user interface.
...
The `QCView` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
A Quartz Composer patch used as the root patch by the `QCView`.
...
An NSString that contains the tool tip to display for this `QCView`.
