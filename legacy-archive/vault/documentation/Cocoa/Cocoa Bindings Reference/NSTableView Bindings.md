...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSTableView.html
archived_at: '2026-07-15T07:21:53.078271Z'
---
...
[Next](NSTextField%20Bindings.md)[Previous](NSTableColumn%20Bindings.md)

# NSTableView Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSTableView](https://developer.apple.com/documentation/appkit/nstableview) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that specifies the object passed as a parameter to the selector when `NSTableView` receives a double click.
...
The objects specified in the `doubleClickArgument` bindings are passed as parameters to the selector specified in the `doubleClickTarget` binding when the `NSTableView` is double clicked.
...
An object that receives a message corresponding to `selector name` when the `NSTableView` is double clicked.
...
A multiple-value binding that determines if the `NSTableView` is enabled in the user interface.
...
The `NSTableView` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSTableView` is displayed in the user interface.
...
The `NSTableView` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSTableView`.
...
A Boolean value that determines if the NSFont used to display the `NSTableView` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSTableView`.
...
A Boolean value that determines if the NSFont used to display the `NSTableView` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSTableView`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSTableView`.
...
An integer value that specifies the height, in pixels, of a row in the `NSTableView`.
...
An NSString that contains the tool tip to display for this `NSTableView`.
...
An NSIndexSet instance that specifies the indices of the selected rows in the `NSTableView`.
...
An indexed collection of NSSortDescriptor instances that specify the sort ordering of the contents of the `NSTableView`.
