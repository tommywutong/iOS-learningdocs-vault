...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSSearchField.html
archived_at: '2026-07-15T07:21:46.557549Z'
---
...
[Next](NSSecureTextField%20Bindings.md)[Previous](NSScrollView%20Bindings.md)

# NSSearchField Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSSearchField](https://developer.apple.com/documentation/appkit/nssearchfield) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSSearchField` is editable in the user interface.
...
The contents of the `NSSearchField` are editable if a logical AND operation on all the `editable` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSSearchField` is enabled in the user interface.
...
The `NSSearchField` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSSearchField` is displayed in the user interface.
...
The `NSSearchField` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
A Boolean value that determines if the NSFont used to display the `NSSearchField` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
A Boolean value that determines if the NSFont used to display the `NSSearchField` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSSearchField`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
The multiple-value predicate binding allows you to create a search field pop-up menu that is pre populated with menu items that correspond to predicate filters. Each of the predicate bindings correspond to an entry in the search field pop-up menu. See [Predicate Binding](Binding%20Types.md#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambsgmydkljsge3dsobv) for further information.

When `predicate` is bound, a new binding, `predicate2`, is exposed and can be bound. Binding to `predicate2` causes `predicate3` to be exposed, and so on.

If `predicate` is bound, the `recentSearches` binding is not available.
...
| Display Name. Available in OS X v10.4 and later. | [NSDisplayNameBindingOption](Binding%20Options.md#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambsgmydiljsge4dsnjz) | NSString |
| Predicate Format String. Available in OS X v10.4 and later. | [NSPredicateFormatBindingOption](Binding%20Options.md#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambsgmydiljsge4tanbv) | NSString |
...
An `NSString` or `NSNumber` that is displayed as the content of the `NSSearchField`.
