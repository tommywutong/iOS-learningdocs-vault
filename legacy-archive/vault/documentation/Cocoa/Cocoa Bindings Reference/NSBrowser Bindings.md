...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSBrowser.html
archived_at: '2026-07-15T07:21:31.111567Z'
---
...
[Next](NSButton%20Bindings.md)[Previous](NSBox%20Bindings.md)

# NSBrowser Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSBrowser](https://developer.apple.com/documentation/appkit/nsbrowser) |
| __Availability__ | Available in OS X v10.4 and later. |

A multiple-value binding that determines if the `NSBrowser` is enabled in the user interface.
...
The `NSBrowser` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSBrowser` is displayed in the user interface.
...
The `NSBrowser` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSTreeController instance that provides the content of the `NSBrowser`.

Unless `contentValues` is also bound, the titles of the items in the `NSBrowser` are derived by invoking invoking the `description`method for each of the content objects.
...
An array of strings that are displayed as the items in the `NSBrowser`.
...
An array of NSIndexPath instances that specify the current selection in the `NSBrowser`.
...
An NSFont used to display the `NSBrowser`.
...
A Boolean value that determines if the NSFont used to display the `NSBrowser` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSBrowser`.
...
A Boolean value that determines if the NSFont used to display the `NSBrowser` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSBrowser`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSBrowser`.
...
An NSString that contains the tool tip to display for this `NSBrowser`.
