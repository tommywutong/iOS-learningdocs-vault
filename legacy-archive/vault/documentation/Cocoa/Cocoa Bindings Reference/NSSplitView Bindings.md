...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSSplitView.html
archived_at: '2026-07-15T07:21:50.046254Z'
---
...
[Next](NSStepper%20Bindings.md)[Previous](NSSliderCell%20Bindings.md)

# NSSplitView Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSSplitView](https://developer.apple.com/documentation/appkit/nssplitview) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSSplitView` is displayed in the user interface.
...
The `NSSplitView` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSString that contains the tool tip to display for this `NSSplitView`.
