...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSSecureTextField.html
archived_at: '2026-07-15T07:21:47.071440Z'
---
...
[Next](NSSecureTextFieldCell%20Bindings.md)[Previous](NSSearchField%20Bindings.md)

# NSSecureTextField Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSSecureTextField](https://developer.apple.com/documentation/appkit/nssecuretextfield) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSSecureTextField` is editable in the user interface.
...
The contents of the `NSSecureTextField` are editable if a logical AND operation on all the `editable` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSSecureTextField` is enabled in the user interface.
...
The `NSSecureTextField` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSSecureTextField` is displayed in the user interface.
...
The `NSSecureTextField` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSSecureTextField`.
...
A Boolean value that determines if the NSFont used to display the `NSSecureTextField` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSSecureTextField`.
...
A Boolean value that determines if the NSFont used to display the `NSSecureTextField` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSSecureTextField`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSSecureTextField`.
...
An integer value that specifies the alignment of the content displayed in the `NSSecureTextField`.
...
An NSString that contains the tool tip to display for this `NSSecureTextField`.
...
An NSColor that specifies the color used to display the text of the `NSSecureTextField`.
...
An NSString or NSNumber that is displayed as the content of the `NSSecureTextField`.
...
A multiple-value binding that specifies a string value that replaces `%{value1}@` in a display pattern string that is used to set the value of the `NSSecureTextField`.
