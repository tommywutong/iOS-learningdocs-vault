...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSStepper.html
archived_at: '2026-07-15T07:21:50.554540Z'
---
...
[Next](NSStepperCell%20Bindings.md)[Previous](NSSplitView%20Bindings.md)

# NSStepper Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSStepper](https://developer.apple.com/documentation/appkit/nsstepper) |
| __Availability__ | Available in OS X v10.3 and later. |

A multiple-value binding that determines if the `NSStepper` is enabled in the user interface.
...
The `NSStepper` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSStepper` is displayed in the user interface.
...
The `NSStepper` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSString that contains the tool tip to display for this `NSStepper`.
...
A `double` value that specifies the maximum value allowed by the `NSStepper`.
...
A `double` value that specifies the minimum value allowed by the `NSStepper`.
...
A `double` value that specifies the current value of the `NSStepper`.
