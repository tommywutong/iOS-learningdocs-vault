...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSTokenFieldCell.html
archived_at: '2026-07-15T07:21:55.578394Z'
---
...
[Next](NSToolbarItem%20Bindings.md)[Previous](NSTokenField%20Bindings.md)

# NSTokenFieldCell Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSTokenFieldCell](https://developer.apple.com/documentation/appkit/nstokenfieldcell) |
| __Availability__ | Available in OS X v10.4 and later. |

A multiple-value binding that determines if the `NSTokenFieldCell` is editable in the user interface.
...
The contents of the `NSTokenFieldCell` are editable if a logical AND operation on all the `editable` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSTokenFieldCell` is enabled in the user interface.
...
The `NSTokenFieldCell` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSTokenFieldCell`.
...
A Boolean value that determines if the NSFont used to display the `NSTokenFieldCell` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSTokenFieldCell`.
...
A Boolean value that determines if the NSFont used to display the `NSTokenFieldCell` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSTokenFieldCell`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSTokenFieldCell`.
...
An NSColor that specifies the color used to display the text of the `NSTokenFieldCell`.
...
An NSString or NSNumber that is displayed as the content of the `NSTokenFieldCell`.
...
A multiple-value binding that specifies a string value that replaces `%{value1}@` in a display pattern string that is used to set the value of the `NSTokenFieldCell`.
