...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSPathControl.html
archived_at: '2026-07-15T07:21:43.042763Z'
---
...
[Next](NSPersistentDocument%20Bindings.md)[Previous](NSPathCell%20Bindings.md)

# NSPathControl Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSPathControl](https://developer.apple.com/documentation/appkit/nspathcontrol) |
| __Availability__ | Available in OS X v10.5 and later. |

A multiple-value binding that determines if the `NSPathControl` is enabled in the user interface.
...
The `NSPathControl` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSPathControl` is displayed in the user interface.
...
The `NSPathControl` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSPathControl`.
...
A Boolean value that determines if the NSFont used to display the `NSPathControl` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSPathControl`.
...
A Boolean value that determines if the NSFont used to display the `NSPathControl` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSPathControl`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSPathControl`.
...
An NSString that contains the tool tip to display for this `NSPathControl`.
...
An `NSString` or file `NSURL` that is displayed as the content of the `NSPathControl`.
