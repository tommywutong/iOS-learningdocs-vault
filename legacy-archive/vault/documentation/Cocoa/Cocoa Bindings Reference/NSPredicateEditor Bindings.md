...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSPredicateEditor.html
archived_at: '2026-07-15T07:21:45.042949Z'
---
...
[Next](NSProgressIndicator%20Bindings.md)[Previous](NSPopUpButtonCell%20Bindings.md)

# NSPredicateEditor Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSPredicateEditor](https://developer.apple.com/documentation/appkit/nspredicateeditor) |
| __Availability__ | Available in OS X v10.5 and later. |

A multiple-value binding that determines if the `NSPredicateEditor` is editable in the user interface.
...
The contents of the `NSPredicateEditor` are editable if a logical AND operation on all the `editable` bindings results in a Boolean value of `YES`.

__Availability:__Available in OS X v10.5 and later.__Binding is Read-Only.__
...
A multiple-value binding that determines if the `NSPredicateEditor` is enabled in the user interface.
...
The `NSPredicateEditor` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSPredicateEditor` is displayed in the user interface.
...
The `NSPredicateEditor` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSPredicateEditor`.
...
A Boolean value that determines if the NSFont used to display the `NSPredicateEditor` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSPredicateEditor`.
...
A Boolean value that determines if the NSFont used to display the `NSPredicateEditor` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSPredicateEditor`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSPredicateEditor`.
...
An array of `NSPrediateEditorRowTemplate` objects that are used to construct the predicate editor display.
...
An NSString that contains the tool tip to display for this `NSPredicateEditor`.
...
An `NSPredicate` specified by the `NSPredicateEditor`.

__Availability:__Available in OS X v10.5 and later.
