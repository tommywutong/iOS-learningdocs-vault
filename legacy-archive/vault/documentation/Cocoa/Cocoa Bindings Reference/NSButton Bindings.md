...
source_url: https://developer.apple.com/library/archive/documentation/Cocoa/Reference/CocoaBindingsRef/BindingsText/NSButton.html
archived_at: '2026-07-15T07:21:31.603447Z'
---
...
[Next](NSButtonCell%20Bindings.md)[Previous](NSBrowser%20Bindings.md)

# NSButton Bindings

|  |  |
| --- | --- |
| __Related class__ | [NSButton](https://developer.apple.com/documentation/appkit/nsbutton) |
...
An object that receives a message corresponding to `selector name` when the `NSButton` is clicked.
...
A multiple-value binding that determines if the `NSButton` is enabled in the user interface.
...
The `NSButton` is enabled if a logical AND operation on all the `enabled` bindings results in a Boolean value of `YES`.
...
A multiple-value binding that determines if the `NSButton` is displayed in the user interface.
...
The `NSButton` is hidden if a logical OR operation on all the `hidden` bindings results in a Boolean value of `YES`.
...
An NSFont used to display the `NSButton`.
...
A Boolean value that determines if the NSFont used to display the `NSButton` is bold. If `fontBold` evaluates to `YES`, the bold attribute is added to the font.
...
An NSString that specifies the family name of the NSFont used to display the contents of the `NSButton`.
...
A Boolean value that determines if the NSFont used to display the `NSButton` is italic. If `fontItalic` evaluates to `YES`, the italic attribute is added to the font.
...
An NSString that specifies the full name of the NSFont that is used to display the contents of the `NSButton`. The full font name includes the family and the style of the font — for example, "Helvetica-Bold".
...
An integer value that determines the size, in points, of the font used to display the `NSButton`.
...
An NSImage that is displayed by the `NSButton` when it is in its alternate state. Note that some button types don't display an alternate image.
...
An NSString that is displayed by the `NSButton` when it is in its alternate state. Note that some button types don't display an alternate title.
...
An NSImage that specifies the image displayed by the `NSButton`.
...
An NSString value that is displayed as the label of the `NSButton`.
...
An NSString that contains the tool tip to display for this `NSButton`.
...
An `integer` value that specifies the current state of the `NSButton`.
