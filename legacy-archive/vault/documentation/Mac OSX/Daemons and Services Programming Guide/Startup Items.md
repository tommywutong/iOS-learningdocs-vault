...
source_url: https://developer.apple.com/library/archive/documentation/MacOSX/Conceptual/BPSystemStartup/Chapters/StartupItems.html
archived_at: '2026-07-15T08:16:29.713395Z'
---
...
[Next](Customizing%20Login%20and%20Logout.md)[Previous](Scheduling%20Timed%20Jobs.md)

# Startup Items

A startup item is a specialized bundle whose code is executed during the final phase of the boot process, and at other predetermined times (see [Managing Startup Items](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambsgezteljrga3timzu)). The startup item typically contains a shell script or other executable file along with configuration information used by the system to determine the execution order for all startup items.
...
1. Create the startup item directory. The directory name should correspond to the behavior you’re providing.
   ...
   Example: `MyDBServer`
2. Add your executable to the directory. The name of your executable should be exactly the same as the directory name. For more information, see [Creating the Startup Item Executable](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambsgezteljrga2tsojw).
3. 

   Create a property list with the name `StartupParameters.plist` and add it to the directory. For information on the keys to include in this property list, see [Specifying the Startup Item Properties](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambsgezteljrga3dsmrx).
...
4. Create an installer to place your startup item in the `/Library/StartupItems` directory of the target system. (Your installer may need to create this directory first.)
   ...
   Your installer script should set the permissions of the startup item directory to prevent non-root users from modifying the startup item or its contents. For more information, see [Startup Item Permissions](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpgiydambsgezteljrge2dqobw).
...
For more information about writing shell scripts, see _[Shell Scripting Primer](../../Shell%20Scripting%20Primer/Introduction.md#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridimbqga2denry)_.
...
The value of the _key_ field must precisely match the English string printed by your startup item using `ConsoleMessage`.
