...
source_url: https://developer.apple.com/library/archive/documentation/DeveloperTools/gcc-4.2.1/gccint/Misc.html
archived_at: '2026-07-15T07:31:02.374741Z'
---
...
— Target Hook: int __TARGET_MIN_DIVISIONS_FOR_RECIP_MUL__ (enum machine_mode mode)
> When `-ffast-math` is in effect, GCC tries to optimize
> divisions by the same divisor, by turning them into multiplications by
> the reciprocal. This target hook specifies the minimum number of divisions
> that should be there for GCC to perform the optimization for a variable
> of mode mode. The default implementation returns 3 if the machine
...
> See [shift patterns](https://developer.apple.com/library/archive/documentation/DeveloperTools/gcc-4.2.1/gccint/shift-patterns.html#shift-patterns).
>
...
— Target Hook: int __TARGET_MODE_REP_EXTENDED__ (enum machine_mode mode, enum machine_mode rep_mode)
...
> mode to mode_rep so that mode_rep is not the next
...
— Target Hook: tree __TARGET_MD_ASM_CLOBBERS__ (tree outputs, tree inputs, tree clobbers)
...
— Target Hook: tree __TARGET_RESOLVE_OVERLOADED_BUILTIN__ (tree fndecl, tree arglist)
> Select a replacement for a machine specific built-in function that
> was set up by ``TARGET_INIT_BUILTINS`'. This is done
> _before_ regular type checking, and so allows the target to
...
— Target Hook: tree __TARGET_FOLD_BUILTIN__ (tree fndecl, tree arglist, bool ignore)
> Fold a call to a machine specific built-in function that was set up by
> ``TARGET_INIT_BUILTINS`'. fndecl is the declaration of the
> built-in function. arglist is the list of arguments passed to
...
— Target Hook: const __char__ \* TARGET_INVALID_WITHIN_DOLOOP (rtx insn)
...
— Target Hook: bool __TARGET_COMMUTATIVE_P__ (rtx x, outer_code)
...
— Target Hook: rtx __TARGET_ALLOCATE_INITIAL_VALUE__ (rtx hard_reg)
...
— Target Hook: const char \* __TARGET_INVALID_CONVERSION__ (tree fromtype, tree totype)
> If defined, this macro returns the diagnostic message when it is
> invalid to convert from fromtype to totype, or `NULL`
> if validity should be determined by the front end.

— Target Hook: const char \* __TARGET_INVALID_UNARY_OP__ (int op, tree type)
> If defined, this macro returns the diagnostic message when it is
> invalid to apply operation op (where unary plus is denoted by
> `CONVERT_EXPR`) to an operand of type type, or `NULL`
> if validity should be determined by the front end.

— Target Hook: const char \* __TARGET_INVALID_BINARY_OP__ (int op, tree type1, tree type2)
> If defined, this macro returns the diagnostic message when it is
