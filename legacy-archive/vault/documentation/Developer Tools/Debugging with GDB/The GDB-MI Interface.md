...
source_url: https://developer.apple.com/library/archive/documentation/DeveloperTools/gdb/gdb/gdb_25.html
archived_at: '2026-07-15T07:31:03.357320Z'
---
> 导航：[总目录](../../../README.md) · [documentation](../../../_indexes/documentation.md) · [Debugging with GDB](Debugging%20with%20GDB.md)


Go to the [first](Summary%20of%20GDB.md), [previous](Using%20GDB%20under%20GNU%20Emacs.md), [next](GDB%20Annotations.md), [last](Index.md) section, [table of contents](Debugging%20with%20GDB.md).

---

# [The GDB/MI Interface](Debugging%20with%20GDB.md#apple-krhugmrtgy)

## [Function and Purpose](Debugging%20with%20GDB.md#apple-krhugmrtg4)

GDB/MI is a line based machine oriented text interface to
GDB and is activated by specifying using the
@option{--interpreter} command line option (see section [Choosing modes](Getting%20In%20and%20Out%20of%20GDB.md#apple-kncugoi)). It
...
## [Notation and Terminology](Debugging%20with%20GDB.md#apple-krhugmrtha)
...
## [GDB/MI Command Syntax](Debugging%20with%20GDB.md#apple-krhugmrthe)

### [GDB/MI Input Syntax](Debugging%20with%20GDB.md#apple-krhugmruga)
...
### [GDB/MI Output Syntax](Debugging%20with%20GDB.md#apple-krhugmruge)
...
See section [GDB/MI Stream Records](#apple-kncugmrugy), for more
details about the various output records.

### [Simple Examples of GDB/MI Interaction](Debugging%20with%20GDB.md#apple-krhugmrugi)
...
## [GDB/MI Compatibility with CLI](Debugging%20with%20GDB.md#apple-krhugmrugm)
...
## [GDB/MI Output Records](Debugging%20with%20GDB.md#apple-krhugmrugq)

### [GDB/MI Result Records](Debugging%20with%20GDB.md#apple-krhugmrugu)
...
### [GDB/MI Stream Records](Debugging%20with%20GDB.md#apple-krhugmrugy)

GDB internally maintains a number of output streams: the console, the
target, and the log. The output intended for each of these streams is
funneled through the GDB/MI interface using __stream records__.

Each stream record begins with a unique __prefix character__ which
identifies its stream (see section [GDB/MI Output Syntax](#apple-kncugmruge)). In addition to the prefix, each stream record contains a
...
### [GDB/MI Out-of-band Records](Debugging%20with%20GDB.md#apple-krhugmrug4)

__Out-of-band__ records are used to notify the GDB/MI client of
...
## [GDB/MI Command Description Format](Debugging%20with%20GDB.md#apple-krhugmruha)
...
## [GDB/MI Breakpoint table commands](Debugging%20with%20GDB.md#apple-krhugmruhe)
...
```
 ^done,bkptno="number",func="funcname",
  file="filename",line="lineno",shlib="shlib"
```

or, if there are multiple breakpoints, and the -l option is not selected:

```
 ^done,matches=[b=[index="index",number="number",func="funcname",
  file="filename",line="lineno"], b=[]...]
```

where number is the GDB number for this breakpoint, funcname
is the name of the function where the breakpoint was inserted,
filename is the name of the source file which contains this
function, and lineno is the source line number within that file, and shlib
is the shared library in which the breakpoint was set.

If there are multiple matches, the second form is sent including the
index element, and no breakpoints are set. This will allow the
UI to put up some kind of picker. Then it can re-issue the command
with the -l option, using the index values provided by the first
...
__access__ watchpoint, i.e. a watchpoint that triggers either on a
read from or on a write to the memory location. With the `` `-r' ``
option, the watchpoint created is a __read__ watchpoint, i.e. it will
trigger only when the memory location is accessed for reading. Without
either of the options, the watchpoint created is a regular watchpoint,
i.e. it will trigger when the memory location is accessed for writing.
See section [Setting watchpoints](Stopping%20and%20Continuing.md#apple-kncugmzs).
...
## [GDB/MI Data Manipulation](Debugging%20with%20GDB.md#apple-krhugmrvga)
...
__tokens__ described in section [GDB/MI Command Syntax](#apple-kncugmrthe). Notice how GDB/MI returns the same tokens in its
output.
...
same as for GDB's `print` command (see section [Output formats](Examining%20Data.md#apple-kncugnrq)).
...
## [GDB/MI Program control](Debugging%20with%20GDB.md#apple-krhugmrvge)
...
`` `-file-exec-and-symbols' ``, the symbol table is _not_ read
...
List the globals defined in file file and image image. If
file is the empty string, then all globals in image are
listed, and if both file and image are the empty string,
then all globals are listed.

print-values has the same meaning as for `` `-stack-list-locals' ``.

filter_regex is optional. If provided, then all statics whose names match
the regex will be filtered out.

constants is also optional. If provided, and set to 1, then no const statics will be printed
...
List the globals defined in file file and image image. If
image is the empty string, then the first occurance of the file
file is used.

file also accepts the special cookie `*CURRENT FRAME*` in which
...
## [Miscellaneous GDB commands in GDB/MI](Debugging%20with%20GDB.md#apple-krhugmrvgi)
...
## [GDB/MI Stack Manipulation Commands](Debugging%20with%20GDB.md#apple-krhugmrvgm)
...
Change the selected frame. Select a different frame framenum on
the stack.

#### GDB Command

The corresponding GDB commands are `` `frame' ``, `` `up' ``,
...
## [GDB/MI Symbol Query Commands](Debugging%20with%20GDB.md#apple-krhugmrvgq)
...
## [GDB/MI Target Manipulation Commands](Debugging%20with%20GDB.md#apple-krhugmrvgu)
...
Compare data of section section on target to the exec file.
...
Each message is sent as status record (see section [GDB/MI Output Syntax](#apple-kncugmruge)).
...
**`` `parameters' ``**
: Device names, host names and the like. See section [Commands for managing targets](Specifying%20a%20Debugging%20Target.md#apple-kncugmjvgi), for more details.
...
## [GDB/MI Thread Commands](Debugging%20with%20GDB.md#apple-krhugmrvgy)
...
## [GDB/MI Tracepoint Commands](Debugging%20with%20GDB.md#apple-krhugmrvg4)

The tracepoint commands are not yet implemented.

## [GDB/MI Variable Objects](Debugging%20with%20GDB.md#apple-krhugmrvha)
...
2. It will shorten development time (needless to say how important it is
   now).
...
- `-var-update`
  ...
  __Description And Use of Operations on Variable Objects__

  __The `-var-create` Command__
  ...
  __The `-var-set-format` Command__
  ...
  __The `-var-show-format` Command__
  ...
  __The `-var-info-num-children` Command__
  ...
  __The `-var-list-children` Command__
  ...
  __The `-var-info-expression` Command__
  ...
  __The `-var-show-attributes` Command__
  ...
  __The `-var-evaluate-expression` Command__
  ...
  A `` `*' `` causes all existing variable objects to be updated. The
  option print-values determines whether names and values, or just
