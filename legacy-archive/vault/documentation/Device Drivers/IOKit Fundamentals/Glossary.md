...
source_url: https://developer.apple.com/library/archive/documentation/DeviceDrivers/Conceptual/IOKitFundamentals/Glossary/Glossary.html
archived_at: '2026-07-15T07:31:25.305261Z'
---
...
[Next](I-O%20Kit%20Family%20Reference.md)[Previous](Bibliography.md)

# Glossary

- __active driver__

  A device driver that implements advanced power management tasks, such as determining device idleness and performing pre-shutdown tasks. See also [passive driver](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvjvomq)
...
- __bundle__
  ...
  A directory in the file system that typically stores executable code and the software resources related to that code. (A bundle can store only resources.) Applications, plug-ins, frameworks, and kernel extensions are types of bundles. Except for frameworks, bundles are file packages, presented by the Finder as a single file instead of a folder. See also [kernel extension](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskginfeksq)
...
- __bus master__
  ...
  A program, usually in a separate I/O controller, that directs traffic on the computer bus or input/output paths. The bus master actually controls the bus paths on which the address and control signals flow. DMA is a simple form of bus mastering where the bus master controls I/O transfers between a device and system memory and then signals to the CPU when it has done so. See also [DMA](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskdjbcegry)

- __client__

  A driver object that consumes services of some kind supplied by its provider. In a driver stack, the client in a provider/client relationship is farther away from the Platform Expert. See also [provider](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskjifeucrq)
...
- __fault__
  ...
  In the virtual-memory system, faults are the mechanism for initiating page-in activity. They are interrupts that occur when code tries to access data at a virtual address that is not mapped to physical memory. See also [page](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskiirduoqy); [virtual memory](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskdjfcuoqi)
...
- __idle sleep__
  ...
  A sleep state that occurs when there has been no device or system activity for the period of time the user specifies in the Energy Saver pane of System Preferences. See also [system sleep](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvjvona)
...
- __matching__
  ...
  See [device matching](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskcindeqqi); [driver matching](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskdirdeusq)

- __memory cursor__

  An object that lays out the buffer ranges in a memory descriptor in physical memory, generating a scatter/gather list suitable for a particular device or DMA engine. The object is derived from the IOMemoryCursor class. See also [DMA](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskdjbcegry); [memory descriptor](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskkirceira)

- __memory descriptor__

  An object that describes how a stream of data, depending on direction, should either be laid into memory or extracted from memory. It represents a segment of memory holding the data involved in an I/O transfer and is specified as one or more physical or virtual address ranges. The object is derived from the IOMemoryDescriptor class. See also [DMA](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskdjbcegry); [memory cursor](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskjjjbussq)
...
- __nub__
  ...
  An I/O Kit object that represents a detected, controllable entity such as a device or logical service. A nub may represent a bus, disk, graphics adaptor, or any number of similar entities. A nub supports dynamic configuration by providing a bridge between two drivers (and, by extension, between two families). See also [device](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskfizbusqq); [driver](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskfirfeqry)

- __page__

  (1) The smallest unit (in bytes) of information that the virtual memory system can transfer between physical memory and backing store. In Darwin, a page is currently 4 kilobytes. (2) As a verb, page refers to the transfer of pages between physical memory and backing store. Refer to `Kernel.framework/Headers/mach/machine/vm_params.h` for specifics. See also [fault](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskcifeusrq); [virtual memory](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskdjfcuoqi)

- __passive driver__

  A device driver that performs only basic power-management tasks, such as joining the power plane and changing the device’s power state. See also [active driver](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvjvomi)
...
- __physical memory__
  ...
  Electronic circuitry contained in random-access memory (RAM) chips, used to temporarily hold information at execution time. Addresses in a process’s virtual memory are mapped to addresses in physical memory. See also [virtual memory](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskdjfcuoqi)

- __PIO__

  (Programmed Input/Output) A way to move data between a device and system memory in which each byte is transferred under control of the host processor. See also [DMA](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskdjbcegry)
...
- __power child__
  ...
  In the power plane, a driver for a device that relies on another object for its power. See also [power parent](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvjvonq); [plane](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvjvony)

- __power parent__

  In the power plane, an object that provides power for a device. See also [power child](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvjvoni); [plane](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvjvony)
...
- __system sleep__
  ...
  A sleep state that occurs when the user chooses Sleep from the Apple menu or closes the lid of a laptop computer. See also [idle sleep](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvjvomy)

- __task__

  A Mach abstraction consisting of a virtual address space and a port name space. A task itself performs no computation; rather, it is the context in which threads run. See also [process](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskiijfeuqi); [thread](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskhizeumri)

- __thread__

  In Mach, the unit of CPU utilization. A thread consists of a program counter, a set of registers, and a stack pointer. See also [task](#apple-f4xwc4dqnrsv64tfmyxwi33df52wszbpkridgmbqgaydonbufvbegskdi5becqq)
