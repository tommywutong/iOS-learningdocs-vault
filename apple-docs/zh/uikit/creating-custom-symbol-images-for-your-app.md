...
SF Symbols 提供一整套一致且高度可配置的符号图像，供你在 App 中使用。你可以应用通常与文本相关联的风格特性（trait），例如应用颜色、文本样式、字重和缩放比例。符号还包含一些额外的特性，使它们能与周围文本无缝整合，并适应动态类型（Dynamic Text，Apple 标准名为 Dynamic Type）和深色模式（Dark Mode）等平台特性。
...
重叠形状有一个常见问题：路径重叠处会透出下方内容。在颜色检查器中，每个图层右侧都有一个切换开关。如果它处于停用状态，透明图层会与下面的图层融合；如果它处于启用状态，透明图层会清除背后的内容，渲染时就好像其他图层不存在一样。
