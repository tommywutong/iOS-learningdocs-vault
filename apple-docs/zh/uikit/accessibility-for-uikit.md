...
让你的 UIKit App 对使用 iOS 和 tvOS 的每一个人都无障碍可用。

## 概述

让你的 App 可访问，就是让每个人都能使用它。在设计 App 时把辅助功能（accessibility）放在心上，就能让所有人享用你的 App。更多信息参见 [Accessibility](../accessibility.md)。

UIKit 的控件和视图自带内建辅助功能，默认即提供可访问的用户体验。通常你无需额外工作就能启用标准辅助功能特性。
...
如果你用 SwiftUI 构建 App，参见 [Accessibility modifiers](../swiftui/view-accessibility.md)。
...
- [在你的 App 中支持 VoiceOver](supporting-voiceover-in-your-app.md) — 添加 VoiceOver 支持，让你的 iOS App 对失明或低视力用户更加可用。

### 行为

- [UIAccessibilityFocus](../objectivec/uiaccessibilityfocus.md) — 一个非正式协议，提供判断辅助 App（如 VoiceOver）是否聚焦于某个可访问元素的方法。
...
- [交付卓越的辅助功能体验](../accessibility/delivering_an_exceptional_accessibility_experience.md) — 改进你的 App 的交互模型，以支持 VoiceOver 等辅助技术。
...
- [UIPickerViewAccessibilityDelegate](uipickerviewaccessibilitydelegate.md) — 你可以实现的一组方法，为选取器视图的各个组件提供辅助功能信息。
...
- [UIAccessibilityCustomRotor](uiaccessibilitycustomrotor.md) — 帮助 VoiceOver 用户找到相关元素的下一个实例的上下文相关功能。
...
- [Notification names](notification-names.md) — 辅助功能系统生成的通知的名称。
- [Notification dictionary keys](notification-dictionary-keys.md) — 用 user info 字典中的键处理通知。
...
- [UIAccessibilityRequestGuidedAccessSession](<uiaccessibility/requestguidedaccesssession(enabled_completionhandler_).md>) — 异步地把 App 切入或切出单 App 模式。
...
- [UIAccessibilityIsVoiceOverRunning](uiaccessibility/isvoiceoverrunning.md) — 表示 VoiceOver 是否处于启用状态的布尔值。
...
- [指针交互](pointer-interactions.md) — 在你的自定义控件和视图中支持指针交互。
