...
> 导航：[Technologies](../../../technologies.md) · [Swift](../../../swift.md) · [Swift Standard Library](../../swift-standard-library.md) · [Concurrency](../../concurrency.md) · [AsyncSequence](../../asyncsequence.md) · [AsyncThrowingFlatMapSequence](../../asyncthrowingflatmapsequence.md) · [Iterator](../iterator.md)
...
- [next()](<next()-9sxa4.md>) — `next()` 的默认实现，基于 `next(isolation:)` 实现，这是为了保持与现有异步迭代器的向后兼容性。
- [next(isolation:)](<next(isolation_)-1y6ga.md>) — `next(isolation:)` 的默认实现，基于 `next()` 实现，这是为了保持与现有异步迭代器的向后兼容性。
