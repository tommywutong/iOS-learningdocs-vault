...
- [next()](<next()-8ijyt.md>) — 基于 `next(isolation:)` 实现的 `next()` 默认实现，这是维持与现有异步迭代器向后兼容性所必需的。
- [next(isolation:)](<next(isolation_)-4t5g.md>) — 基于 `next()` 实现的 `next(isolation:)` 默认实现，这是维持与现有异步迭代器向后兼容性所必需的。
