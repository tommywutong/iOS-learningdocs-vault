...
- 自定义文档目录的主登录页面，以介绍技术并组织其顶层符号。
- 添加特定于符号的扩展文件，以组织方法和属性等嵌套符号。
- 使用集合对多个符号进行分组，并在文档页面的导航中引入层级结构。
...
_登录页面_概述你的技术、介绍重要术语，并组织_文档目录_中的资源；文档目录是用于丰富源文档注释的文件集合。你可以利用登录页面简化读者的学习路径、讨论技术的主要功能，并提供能让读者在需要时回来查阅的学习动机。

![](../../../attachments/a50a02fc4acb28bad8a02a49b9995d77/landing-page@2x.png)

<sub>两张并排的框架顶层页面图像。左图显示 DocC 默认生成的基本空白页面。右图显示包含额外内容和彩色图形的自定义登录页面。</sub>

向项目添加文档目录时，Xcode 会自动包含一个空白登录页面。有关更多信息，请参阅[记录 App、框架和包](documenting-apps-frameworks-and-packages.md)。

如果需要手动向文档目录添加登录页面，请按照以下步骤操作：
...
若要覆盖默认组织方式并手动排列技术中的顶层符号，请向技术的登录页面添加 Topics 部分。在 Markdown 文件的任何现有内容下方，添加两个井号（`##`）、一个空格和 `Topics` 关键字。
...
重新构建文档后，文档查看器会在导航面板和登录页面中反映这些组织更改，如上图所示。

### 在扩展文件中排列嵌套符号

并非所有符号都会出现在顶层登录页面上。例如，类和结构体会定义方法和属性；在某些情况下，嵌套类或结构体还会引入额外的层级。

与顶层登录页面一样，DocC 会根据嵌套符号的类型为其生成默认主题组。使用扩展文件覆盖这种默认组织方式，为符号提供更合适的结构。
...
与登录页面一样，使用三个井号（`###`）为每个主题组创建命名部分，并使用双反引号（``）语法向每个部分添加所需符号。
