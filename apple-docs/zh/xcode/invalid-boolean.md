...
使用此检查可以检测在布尔变量的值既不是 true 也不是 false 时对它的访问。当使用整数或指针而没有进行适当的类型转换时，可能出现这个问题。使用超出范围的布尔值属于未定义行为（undefined behavior），可能很难调试。自 Xcode 9 起可用。
...
- [无效的浮点转换](invalid-float-cast.md) — 检测浮点类型之间的、转为浮点类型，或从浮点类型转出的越界转换。
