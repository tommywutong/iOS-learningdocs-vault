...
节点的内存图会显示对该节点的所有内存引用，并高亮标记强引用。按住 Control 键点按图中的任何节点可执行更多操作，例如访问快速查看或将描述打印到控制台。选择「Focus on Node」可显示所选节点的图。点按引用可查看其详细信息，包括变量名称、引用类型以及内存中的来源对象和目标对象。
