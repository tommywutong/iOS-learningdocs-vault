...
beta: false
deprecated: false
doc_path: '/documentation/storekit/appstore/requestreview(in:)-4r0y9'
source_url: 'https://developer.apple.com/documentation/storekit/appstore/requestreview(in:)-4r0y9'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/storekit/appstore/requestreview%28in%3A%29-4r0y9.json'
content_hash: 'sha256:f1a7bfbaa10adb93'
translated: false
---
...
- `controller` — The [NSViewController](../../appkit/nsviewcontroller.md) that StoreKit uses to present the rating and review request interface.
...
> [!note] Note
> Because this method may not present an alert, don’t call [requestReview(in:)](<requestreview(in_)-4r0y9.md>) in response to a button tap or other user action.
...
- [requestReview(in:)](<requestreview(in_)-1q8qs.md>) — Tells StoreKit to request an App Store rating or review from the user, if appropriate, using the specified scene.
