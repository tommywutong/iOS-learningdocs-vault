...
beta: false
deprecated: false
doc_path: /documentation/charts/rulemark
source_url: 'https://developer.apple.com/documentation/charts/rulemark'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/charts/rulemark.json'
content_hash: 'sha256:65f5d5fc99c2ee16'
translated: false
---
...
You can annotate a chart with horizontal or vertically spanning rules. This allows viewers to easily compare values over a range to a constant value. Use the [init(xStart:xEnd:y:)](<rulemark/init(xstart_xend_y_)-444cp.md>) initializer to represent a constant `y` value or [init(x:yStart:yEnd:)](<rulemark/init(x_ystart_yend_)-6zemd.md>) for a constant `x` value. To span the plotting area of a chart with a line, omit the optional start and end parameters and plot a constant value.  The example below results in a line that spans the chart horizontally at the y position of 9000:
...
<sub>Vertical bar chart with x-axis showing department categories Production, Marketing, Finance, and R&D, and with y-axis ranging from 0 to 15000. There are 3 bars: Production 15000, Marketing 8000, Finance 10000. A rule mark at 9000 shows the break even threshold.</sub>

### RuleMark in Chart3D

To plot a rule in a 3D chart, use the [init(x:y:z:)](<rulemark/init(x_y_z_).md>) initializer.
...
- [init(x:y:z:)](<rulemark/init(x_y_z_).md>) — Creates a horizontal or vertical rule mark for a  3D chart.
- [init(x:yStart:yEnd:)](<rulemark/init(x_ystart_yend_)-5gy50.md>) — Creates a vertical rule mark with an x encoding and y interval encoding.
- [init(x:yStart:yEnd:)](<rulemark/init(x_ystart_yend_)-6zemd.md>) — Creates a vertical rule mark with value plotted with x.
- [init(x:yStart:yEnd:)](<rulemark/init(x_ystart_yend_)-8iusl.md>) — Creates a vertical rule mark with a fixed x position and y interval encoding.
- [init(xStart:xEnd:y:)](<rulemark/init(xstart_xend_y_)-27yvc.md>) — Creates a horizontal rule mark that plots values on its x interval and on y.
- [init(xStart:xEnd:y:)](<rulemark/init(xstart_xend_y_)-444cp.md>) — Creates a horizontal rule mark that plots a value on y.
- [init(xStart:xEnd:y:)](<rulemark/init(xstart_xend_y_)-6jsoi.md>) — Creates a horizontal rule mark that plots values on its x interval.
