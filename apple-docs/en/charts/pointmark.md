...
beta: false
deprecated: false
doc_path: /documentation/charts/pointmark
source_url: 'https://developer.apple.com/documentation/charts/pointmark'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/charts/pointmark.json'
content_hash: 'sha256:e578a11c030db3c0'
translated: false
---
...
|---|---|
| [foregroundStyle(by:)](<chartcontent/foregroundstyle(by_).md>) | plot an additional property with color |
| [symbol(by:)](<chartcontent/symbol(by_).md>) | plot an additional property with symbols |
| [symbolSize(by:)](<chartcontent/symbolsize(by_).md>) | plot an additional property with size |

For example, to plot the `family` property from the previous example’s `Insect` structure as a color, add the [foregroundStyle(by:)](<chartcontent/foregroundstyle(by_).md>) modifier:
...
Alternatively, you can distinguish families with different symbols by plotting the `family` property using the [symbol(by:)](<chartcontent/symbol(by_).md>) modifier:
...
3D points also offer symbols, such as  [sphere](chart3dsymbolshape/sphere.md), [cylinder](chart3dsymbolshape/cylinder.md), [cone](chart3dsymbolshape/cone.md), and [cube](chart3dsymbolshape/cube.md). Combined with the [symbolSize(_:)](<chart3dcontent/symbolsize(__).md>) and [symbolRotation(_:)](<chart3dcontent/symbolrotation(__).md>) modifiers, you can provide rich customizations for your 3D points:
...
- [init(x:y:)](<pointmark/init(x_y_)-44ke9.md>) — Creates a point mark that plots values to x and y.
- [init(x:y:)](<pointmark/init(x_y_)-9dswq.md>) — Creates a point mark with fixed x position and plots values with y.
- [init(x:y:)](<pointmark/init(x_y_)-9hppd.md>) — Creates a point mark that plots a value on x with fixed y position.
- [init(x:y:z:)](<pointmark/init(x_y_z_).md>) — Creates a 3D point mark that plots values to x, y and z.
