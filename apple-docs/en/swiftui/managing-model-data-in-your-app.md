...
beta: false
deprecated: false
doc_path: /documentation/swiftui/managing-model-data-in-your-app
source_url: 'https://developer.apple.com/documentation/swiftui/managing-model-data-in-your-app'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/swiftui/managing-model-data-in-your-app.json'
content_hash: 'sha256:a2cc180f6bacb5f3'
translated: false
---
...
SwiftUI helps keep your app’s UI up to date with changes made to the data thanks to Observation. With [Observation](../observation.md), a view in SwiftUI can form dependencies on observable data models and update the UI when data changes.

> [!note] Note
> [Observation](../observation.md) support in SwiftUI is available starting with iOS 17, iPadOS 17, macOS 14, tvOS 17, and watchOS 10. For information about adopting Observation in existing apps, see [Migrating from the Observable Object protocol to the Observable macro](migrating-from-the-observable-object-protocol-to-the-observable-macro.md).

### Make model data observable

To make data changes visible to SwiftUI, apply the [Observable()](<../observation/observable().md>) macro to your data model. This macro generates code that adds observation support to your data model at compile time, keeping your data model code focused on the properties that store data. For example, the following code defines a data model for books:
...
> [!important] Important
> The [Observable()](<../observation/observable().md>) macro, in addition to adding observation functionality, also conforms your data model type to the [Observable](../observation/observable.md) protocol, which serves as a signal to other APIs that your type supports observation. Don’t apply the `Observable` protocol by itself to your data model type, since that alone doesn’t add any observation functionality. Instead, always use the `Observable` macro when adding observation support to your type.
...
To share model data throughout a view hierarchy without needing to pass it to each view, add the model data to the view’s environment. You can add the data to the environment using either [environment(_:_:)](<view/environment(____).md>) or the [environment(_:)](<view/environment(__).md>) modifier, passing in the model data.

Before you can use the [environment(_:_:)](<view/environment(____).md>) modifier, you need to create a custom [EnvironmentKey](environmentkey.md). Then extend [EnvironmentValues](environmentvalues.md) to include a custom environment property that gets and sets the value for the custom key. For instance, the following code creates an environment key and property for `library`:
...
With the custom environment key and property in place, a view can add model data to its environment. For example, `LibraryView` adds the source of truth for a `Library` instance to its environment using the [environment(_:_:)](<view/environment(____).md>) modifier:
...
You can also store model data directly in the environment without defining a custom environment value by using the [environment(_:)](<view/environment(__).md>) modifier. For instance, the following code adds a `Library` instance to the environment using this modifier:
...
You can use the [Bindable](bindable.md) property wrapper on properties and variables to an [Observable](../observation/observable.md) object. This includes global variables, properties that exist outside of SwiftUI types, or even local variables. For example, you can create a `@Bindable` variable within a view’s [body](view/body-8kl5o.md):
...
- [ManagingModelDataSample.zip](https://docs-assets.developer.apple.com/published/6c08da4d9562/ManagingModelDataSample.zip)
