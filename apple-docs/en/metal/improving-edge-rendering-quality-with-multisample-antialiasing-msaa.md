...
beta: false
deprecated: false
doc_path: /documentation/metal/improving-edge-rendering-quality-with-multisample-antialiasing-msaa
source_url: 'https://developer.apple.com/documentation/metal/improving-edge-rendering-quality-with-multisample-antialiasing-msaa'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/metal/improving-edge-rendering-quality-with-multisample-antialiasing-msaa.json'
content_hash: 'sha256:b915ffdd92922898'
translated: false
---
...
MSAA improves the rendering of lines and edges by using several color and depth samples per pixel. An MSAA texture normally stores two or four samples per pixel. But some GPU devices support eight samples per pixel. After the app renders a scene to the MSAA texture, it _resolves_ it to a normal texture that contains one sample per pixel. The built-in resolve uses a simple average, but sometimes you want to use a custom resolve.
...
![The four multisample-antialiasing images compared. From left to right: disabled, 2x, 4x, and 8x.](../../../attachments/379a8b33971dc9876d0b08d6429e84a2/msaa-hd-comparison.png)
...
GPUs on Apple devices can support a variety of sample counts but need to support at least a sample count of four. The app checks the device’s supported sample count with [- supportsTextureSampleCount:](<mtldevice/supportstexturesamplecount(__).md>) and uses a default sample count of four.
...
- [ImprovingEdgeRenderingQualityWithMultisampleAntialiasingMSAA.zip](https://docs-assets.developer.apple.com/published/3bacf1535298/ImprovingEdgeRenderingQualityWithMultisampleAntialiasingMSAA.zip)
