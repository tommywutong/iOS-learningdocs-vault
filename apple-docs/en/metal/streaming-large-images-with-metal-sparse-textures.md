...
beta: false
deprecated: false
doc_path: /documentation/metal/streaming-large-images-with-metal-sparse-textures
source_url: 'https://developer.apple.com/documentation/metal/streaming-large-images-with-metal-sparse-textures'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/metal/streaming-large-images-with-metal-sparse-textures.json'
content_hash: 'sha256:4d9f4d08a0cb3052'
translated: false
---
...
This sample demonstrates sparse texture streaming by rendering a ground plane that samples from a 16K resolution texture. The renderer uses [Metal sparse textures](managing-sparse-texture-memory.md) to subdivide the image into regions, or _tiles_, and chooses the tiles to keep in memory. The GPU updates an access counter buffer, and the app determines the tiles it needs to load or discard. The sample shows a heat map of the available MIP levels on the lower left of the screen, where _white_ represents level 0, _yellow_ represents levels 1 to 3, _red_ represents levels 4 and 5, and _purple_ represents the remaining MIP levels. The app contains a checkbox that toggles the camera animation. When the animation runs, the app updates the sparse texture as the camera moves through the scene. Lastly, this sample demonstrates asynchronous updates using [Dispatch](../dispatch.md), or _Grand Central Dispatch (GCD)_, to update the sparse texture.

![](../../../attachments/492721f4ae4dbd9ca277e82d21e18460/sparse-textures-1-screen-capture.png)

<sub>A screen capture of the sparse textures app showing the Apple Park texture and a heat map of the mapped sparse texture tiles.</sub>
...
The sample checks if the [MTLGPUFamilyApple6](mtlgpufamily/apple6.md) feature set is available with the [- supportsFamily:](<mtldevice/supportsfamily(__).md>) method. This feature set begins with the Apple A13 GPUs. Here’s the code from `AAPLViewController:viewDidLoad`:
...
![](../../../attachments/38bd41fa07bb15ff077a3d68b7ea7cab/sparse-textures-3-texture-with-tiles@2x.png)

<sub>An illustration of a large texture and its mipmaps, showing their division into tile regions. An enlarged version of the mipmap tail is also included.</sub>
...
![](../../../attachments/9c70d68899aaa662ac4ad3cf2eb14d38/sparse-textures-4-memory-layout@2x.png)

<sub>An illustration of the layout of the residency and access counter buffers. Resident tiles are colored in. Levels 0, 1, and 2 are shown, as well as the mipmap tail, with 0 being the largest.</sub>
...
![](../../../attachments/1cfaf424d0e334e6359c736605e936d4/sparse-textures-5-tiles-sample@2x.png)

<sub>An illustration showing how the shader samples resident parent tiles if a requested tile isn’t resident. Levels 0, 1, and 2 are shown, along with the mipmap tail, with level 0 being the largest. Tile colors denote whether a tile is accessed and resident, accessed but not resident, nonresident, or resident.</sub>
...
![](../../../attachments/186f598d40e05c0e52ee513f36c4be0e/sparse-textures-6-texture-update@2x.png)

<sub>An illustration showing how the shader decides to map or unmap tiles. Levels 0, 1, and 2 are shown, along with the mipmap tail, with level 0 being the largest. Different colors denote whether a tile is discarded LRU, required, required parent, nonresident, or resident.</sub>
...
The app stores a list of mapping and unmapping requests that the `mapAndBlitTiles` method encodes using a _resource state command encoder_. The `updateTileMappingMode` method converts the sparse pixel regions to tile regions and then updates the texture mapping to reflect the highest mapped MIP level.
...
- [StreamingLargeImagesWithMetalSparseTextures.zip](https://docs-assets.developer.apple.com/published/c625ce6c1899/StreamingLargeImagesWithMetalSparseTextures.zip)
