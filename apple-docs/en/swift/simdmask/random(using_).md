---
title: 'random(using:)'
framework: Swift
symbol_kind: method
role: symbol
role_heading: Type Method
platforms: [iOS 8.0+, iPadOS 8.0+, Mac Catalyst 13.0+, macOS 10.10+, tvOS 9.0+, visionOS 1.0+, watchOS 2.0+]
languages: [swift]
beta: false
deprecated: false
doc_path: '/documentation/swift/simdmask/random(using:)'
source_url: 'https://developer.apple.com/documentation/swift/simdmask/random(using:)'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/swift/simdmask/random%28using%3A%29.json'
content_hash: 'sha256:f1bfc390c8fb94cc'
translated: false
---

> Navigation: [Technologies](../../technologies.md) · [Swift](../../swift.md) · [SIMDMask](../simdmask.md)

# random(using:)

<sub>Type Method</sub>

Returns a vector mask with `true` or `false` randomly assigned in each lane, using the given generator as a source for randomness.

<sub>iOS, iPadOS, Mac Catalyst, macOS, tvOS, visionOS, watchOS</sub>

```swift
static func random<T>(using generator: inout T) -> SIMDMask<Storage> where T : RandomNumberGenerator
```
