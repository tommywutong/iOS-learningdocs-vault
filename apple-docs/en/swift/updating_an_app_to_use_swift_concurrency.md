...
> [!note] Note
> This sample code project is associated with WWDC21 session [10194: Swift Concurrency: Update a Sample App](../https_/developer.apple.com/wwdc21/10194.md).
...
- The original version uses completion handlers to query the HealthKit SDK and to respond to `CLKComplicationDataSource` calls, and dispatch queues to isolate concurrent access to memory. For more information on the original version, see [Creating and updating a complication’s timeline](../clockkit/creating-and-updating-a-complication-s-timeline.md).
...
For more information on setting up watch faces, see [Change the watch face on your Apple Watch](../https_/support.apple.com/en-us/ht205536.md).
...
In some cases an SDK call requires using a completion handler. For example, a call to [init(type:predicate:anchor:limit:resultsHandler:)](<../healthkit/hkanchoredobjectquery/init(type_predicate_anchor_limit_resultshandler_).md>) takes a completion handler, but the call that needs to `await` is the call to [execute(_:)](<../healthkit/hkhealthstore/execute(__).md>).
...
Several methods on the [CLKComplicationDataSource](../clockkit/clkcomplicationdatasource.md) protocol used to configure the app’s timeline take completion handlers, which you can replace with their `async` equivalents:
...
- [TicTacFish: Implementing a game using distributed actors](tictacfish_implementing_a_game_using_distributed_actors.md) — Use distributed actors to take your Swift concurrency and actor-based apps beyond a single process.
