---
title: UnsignedInteger Implementations
framework: Swift
symbol_kind: article
role: collectionGroup
role_heading: API Collection
platforms: []
languages: [swift]
beta: false
deprecated: false
doc_path: /documentation/swift/uint32/unsignedinteger-implementations
source_url: 'https://developer.apple.com/documentation/swift/uint32/unsignedinteger-implementations'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/swift/uint32/unsignedinteger-implementations.json'
content_hash: 'sha256:bced25d788a76519'
translated: false
---

> Navigation: [Technologies](../../technologies.md) · [Swift](../../swift.md) · [Swift Standard Library](../swift-standard-library.md) · [Numbers and Basic Values](../numbers-and-basic-values.md) · [Special-Use Numeric Types](../special-use-numeric-types.md) · [UInt32](../uint32.md)

# UnsignedInteger Implementations

<sub>API Collection</sub>

## Topics

### Initializers

- [init(_:)](<init(__)-c1wd.md>) — Creates a new instance from the given integer.
- [init(exactly:)](<init(exactly_)-8t3ac.md>)

### Instance Properties

- [magnitude](magnitude-swift.property.md) — The magnitude of this value.

### Type Properties

- [isSigned](issigned.md) — A Boolean value indicating whether this type is a signed integer type.
- [max](max.md) — The maximum representable integer in this type.
- [min](min.md) — The minimum representable integer in this type.
