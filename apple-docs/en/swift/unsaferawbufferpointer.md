...
beta: false
deprecated: false
doc_path: /documentation/swift/unsaferawbufferpointer
source_url: 'https://developer.apple.com/documentation/swift/unsaferawbufferpointer'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/swift/unsaferawbufferpointer.json'
content_hash: 'sha256:dd63aee3a0bf7d05'
translated: false
---
...
- [init(_:)](<unsaferawbufferpointer/init(__)-24m35.md>) — Creates a new buffer over the same memory as the given buffer.
- [init(_:)](<unsaferawbufferpointer/init(__)-5ko6n.md>) — Creates a new buffer over the same memory as the given buffer.
- [init(_:)](<unsaferawbufferpointer/init(__)-9uv4.md>) — Creates a raw buffer over the contiguous bytes in the given typed buffer.
- [init(_:)](<unsaferawbufferpointer/init(__)-rrzw.md>) — Creates a raw buffer over the contiguous bytes in the given typed buffer.
- [init(rebasing:)](<unsaferawbufferpointer/init(rebasing_)-9mh4j.md>) — Creates a raw buffer over the same memory as the given raw buffer slice, with the indices rebased to zero.
- [init(rebasing:)](<unsaferawbufferpointer/init(rebasing_)-9sxid.md>) — Creates a raw buffer over the same memory as the given raw buffer slice, with the indices rebased to zero.
- [init(start:count:)](<unsaferawbufferpointer/init(start_count_).md>) — Creates a buffer over the specified number of contiguous bytes starting at the given pointer.
...
- [assumingMemoryBound(to:)](<unsaferawbufferpointer/assumingmemorybound(to_).md>) — Returns a typed buffer to the memory referenced by this buffer, assuming that the memory is already bound to the specified type.
- [bindMemory(to:)](<unsaferawbufferpointer/bindmemory(to_).md>) — Binds this buffer’s memory to the specified type and returns a typed buffer of the bound memory.
- [deallocate()](<unsaferawbufferpointer/deallocate().md>) — Deallocates the memory block previously allocated at this buffer pointer’s base address.
- [isTriviallyIdentical(to:)](<unsaferawbufferpointer/istriviallyidentical(to_).md>) — Returns a Boolean value indicating whether two instances refer to the same memory region.
- [load(fromByteOffset:as:)](<unsaferawbufferpointer/load(frombyteoffset_as_).md>) — Returns a new instance of the given type, read from the buffer pointer’s raw memory at the specified byte offset.
- [loadUnaligned(fromByteOffset:as:)](<unsaferawbufferpointer/loadunaligned(frombyteoffset_as_)-4ll4e.md>) — Returns a new instance of the given type, constructed from the raw memory at the specified offset.
- [loadUnaligned(fromByteOffset:as:)](<unsaferawbufferpointer/loadunaligned(frombyteoffset_as_)-95fln.md>)
- [withMemoryRebound(to:_:)](<unsaferawbufferpointer/withmemoryrebound(to___).md>) — Executes the given closure while temporarily binding the buffer to instances of type `T`.
