---
title: 'animateKeyframes(withDuration:delay:options:animations:completion:)'
framework: UIKit
symbol_kind: method
role: symbol
...
beta: false
deprecated: false
doc_path: '/documentation/uikit/uiview/animatekeyframes(withduration:delay:options:animations:completion:)'
source_url: 'https://developer.apple.com/documentation/uikit/uiview/animatekeyframes(withduration:delay:options:animations:completion:)'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/uikit/uiview/animatekeyframes%28withduration%3Adelay%3Aoptions%3Aanimations%3Acompletion%3A%29.json'
content_hash: 'sha256:187c7be862e8c4d8'
translated: false
---
...
- `options` — A mask of options indicating how you want to perform the animations. For a list of valid constants, see [KeyframeAnimationOptions](keyframeanimationoptions.md).

- `animations` — A block object containing the changes to commit to the views. Typically, you call the [+ addKeyframeWithRelativeStartTime:relativeDuration:animations:](<addkeyframe(withrelativestarttime_relativeduration_animations_).md>) method one or more times from inside this block. You may also change view values directly if you want those changes to animate over the full duration. This block takes no parameters and has no return value. Do not use a `nil` value for this parameter.
