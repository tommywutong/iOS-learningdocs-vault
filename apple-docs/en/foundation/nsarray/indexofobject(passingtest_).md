...
beta: false
deprecated: false
doc_path: '/documentation/foundation/nsarray/indexofobject(passingtest:)'
source_url: 'https://developer.apple.com/documentation/foundation/nsarray/indexofobject(passingtest:)'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/foundation/nsarray/indexofobject%28passingtest%3A%29.json'
content_hash: 'sha256:e2fbd0a58c453e60'
translated: false
---
...
- `predicate` — The block to apply to elements in the array. The block takes three arguments: - **obj** — The element in the array. - **idx** — The index of the element in the array. - **stop** — A reference to a Boolean value. The block can set the value to [true](../../swift/true.md) to stop further enumeration of the array. If a block stops further enumeration, that block continues to run until it’s finished. The `stop` argument is an out-only argument. You should only ever set this Boolean to [true](../../swift/true.md) within the block. The block returns a Boolean value that indicates whether `obj` passed the test. Returning [true](../../swift/true.md) will stop further processing of the array.
