---
title: 'NWListener.State.waiting(_:)'
framework: Network
symbol_kind: case
role: symbol
role_heading: Case
platforms: [iOS 12.0+, iPadOS 12.0+, Mac Catalyst 12.0+, macOS 10.14+, tvOS 12.0+, visionOS 1.0+, watchOS 5.0+]
languages: [swift]
beta: false
deprecated: false
doc_path: '/documentation/network/nwlistener/state-swift.enum/waiting(_:)'
source_url: 'https://developer.apple.com/documentation/network/nwlistener/state-swift.enum/waiting(_:)'
doc_json: 'https://developer.apple.com/tutorials/data/documentation/network/nwlistener/state-swift.enum/waiting%28_%3A%29.json'
content_hash: 'sha256:cad4df872da77a0d'
translated: false
---

> Navigation: [Technologies](../../../technologies.md) · [Network](../../../network.md) · [NWListener](../../nwlistener.md) · [State](../state-swift.enum.md)

# NWListener.State.waiting(_:)

<sub>Case</sub>

The listener is waiting for a network to become available.

<sub>iOS, iPadOS, Mac Catalyst, macOS, tvOS, visionOS, watchOS</sub>

```swift
case waiting(NWError)
```

## See Also

### States

- [NWListener.State.setup](setup.md) — The listener has been initialized but not started.
- [NWListener.State.ready](ready.md) — The listener is running and able to receive incoming connections.
- [NWListener.State.failed(_:)](<failed(__).md>) — The listener has encountered a fatal error.
- [NWListener.State.cancelled](cancelled.md) — The listener has been canceled.
