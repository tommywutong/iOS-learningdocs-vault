...
source_key: onevcat
source_url: 'https://onevcat.com/2021/07/swift-concurrency/'
original_language: zh
published: 2021-07-01
status: active
license: CC BY 4.0（页脚明示）→ 可公开，须署名并保留原文链接
archived_at: 2026-07-27
content_hash: 'sha256:f5bd2296cb7e354e'
translated: n/a
---

> 原文：[Swift 并发初步](https://onevcat.com/2021/07/swift-concurrency/)　·　onevcat (王巍/喵神)
...
> 你可以在这里找到本文中的[参考代码](https://onevcat.com/assets/samples/async-swift-chapter02.zip)。在本文写作的 Xcode 13 beta 2 环境下，你需要额外安装[最新的 Swift 5.5 toolchain](https://swift.org/download/#swift-55-development) 来运行这些代码。
...
- 的方式明确地告知并强制调用侧去进行错误处理。
- ，或者多次调用

  。
- 进行线程调度很快会使代码复杂化。特别是如果线程调度的操作被隐藏在被调用的方法中的时候，不查看源码的话，在 (调用侧的) 回调函数中，几乎无法确定代码当前运行的线程状态。
- 对于正在执行的任务，没有很好的取消机制。
...
1. 的操作，都需要使用
  ...
  包围起来，但是编译器并没有给我们任何保证。在某些时候忘了使用队列，编译器也不会进行任何提示，这种情况下内存依然存在危险。当有更多资源需要保护时，代码复杂度也将爆炸式上升。
2. 中调用另一个

  的方法，会造成线程死锁。在代码简单的时候，这很容易避免，但是随着复杂度增加，想要理解当前代码运行是由哪一个队列派发的，它又运行在哪一个线程上，往往会伴随着严重的困难。必须精心设计，避免重复派发。
