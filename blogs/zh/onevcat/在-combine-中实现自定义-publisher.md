...
source_key: onevcat
source_url: 'https://onevcat.com/2020/01/customize-publisher/'
original_language: zh
published: 2020-01-20
status: active
license: CC BY 4.0（页脚明示）→ 可公开，须署名并保留原文链接
archived_at: 2026-07-27
content_hash: 'sha256:5d8a7366ae4cbbf1'
translated: n/a
---

> 原文：[在 Combine 中实现自定义 Publisher](https://onevcat.com/2020/01/customize-publisher/)　·　onevcat (王巍/喵神)
...
3. 通过调用 2 中创建的
  ...
  协议中所定义的方法)。如果你对这个过程还不太熟悉，建议你可以参考我之前关于自定义
...
4. 还需要更多事件) 时，调用
  ...
  在大部分情况下会保持某个 buffer，并随着时间进行响应并改变值 (毕竟这正是 Combine 或者说响应式编程所要解决的问题领域)，所以一般我们会选择将
...
1. ，
  ...
  也便可以被确定。
2. 协议所规定需要实现的方法，不论你自定义的

  具体是什么，这一部分是不会改变的。
3. 里，按照 Combine 工作流程，我们创建
  ...
  类型，所以先把它留空。在后面我们会回到这个方法，并填上需要的内容。
...
1. 将记录下游订阅者还需要的值的数量，这样我们可以遵守基于 pull 的行为规则。
2. 中待用，因此这里出现了多个线程共享资源的情况。让整个操作线程安全的最简单的方法就是上锁。
3. 调用时，除了将下游告知的需求
...
1. 的接口和它需要完成的任务是相对固定的，遵循 Combine 的工作流程图，来实现其中各个职责类型的必要方法即可。
2. 定义为

  而非

  ，这可以让内存管理和多次订阅的行为更加容易预测。但是，如果一个

  有需要共享的话，应该将它定义为引用语义，比如
...
3. ，大部分有关时序的操作都被封装到了
  ...
  负责大部分逻辑，并维护 Combine 流程的正确性。一般来说，这也是在自定义

  时我们花费最多时间的地方。
4. 能在 Combine 的世界中运行良好，需要遵守基本的规则。比如尊重下游的 demand，考虑性能因素等。

## 练习

为了保持和[《SwiftUI 和 Combine 编程》](https://onevcat.com/2020/01/customize-publisher/(https:/objccn.io/products/swift-ui))这本书的形式上的类似，我也准备了一些小练习，希望能帮助读者通过实际动手练习掌握本文的内容。
