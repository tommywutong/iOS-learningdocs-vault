...
source_key: onevcat
source_url: 'https://onevcat.com/2017/07/state-based-viewcontroller/'
original_language: zh
published: 2017-07-13
status: active
license: CC BY 4.0（页脚明示）→ 可公开，须署名并保留原文链接
archived_at: 2026-07-27
content_hash: 'sha256:5506fa75bb7eb261'
translated: n/a
---

> 原文：[单向数据流动的函数式 View Controller](https://onevcat.com/2017/07/state-based-viewcontroller/)　·　onevcat (王巍/喵神)
...
1. 和设置
  ...
  的代码出现了三次，设置右上 button 的

  的代码存在于 extension 中，添加新项目时我们先获取了输入的 cell，然后再读取 cell 中的文本。这些散落在各处的 UI 操作将会成为隐患，因为你可能在代码的任意部分操作这些 UI，而它们的状态将随着代码的复杂变得“飘忽不定”。
2. 的文字正确，你可能需要手动向列表里添加一个待办事项，这涉及到调用

  ，而这个方法需要读取

  的文本，那么你可能还需要先去设置这个 cell 中
  ...
  和之后的内容提取成一个新的方法，但是无论怎么样，对于 model 的操作和对于 UI 的更新都没有分离 (因为毕竟我们写的就是“胶水代码”)。这正是你觉得 View Controller 难以测试的最主要原因。
...
3. 阻止了这一可能性。如果我们想要重用
  ...
  中同步待办事项的 model。
4. 的测试中，有一个地方我们没有覆盖到，那就是

  中用来加载待办的

  。在不引入 stub 的情况下，测试这类异步操作会非常困难，但是引入 stub 本身现在看来也不是特别方便。我们有没有什么好方法可以测试 View Controller 中的异步操作呢？
