...
2. [2. 自定义构造过程](#自定义构造过程)
3. [3. 默认构造器](#默认构造器)
4. [4. 值类型的构造器代理](#值类型的构造器代理)
5. [5. 类的继承和构造过程](#类的继承和构造过程)

    1. [5.1. 指定构造器和便利构造器](#指定构造器和便利构造器)
    2. [5.2. 构造器链](#构造器链)
    3. [5.3. 两段式构造过程](#两段式构造过程)
    4. [5.4. 构造器的继承和重载](#构造器的继承和重载)
6. [6. 可失败构造器(Failable Initializers)](#可失败构造器-Failable-Initializers)

    1. [6.1. 枚举的可失败构造器](#枚举的可失败构造器)
    2. [6.2. 带有原始值枚举的可失败构造器](#带有原始值枚举的可失败构造器)
    3. [6.3. 类的可失败构造器](#类的可失败构造器)
    4. [6.4. 可失败构造器的传播](#可失败构造器的传播)
    5. [6.5. 可失败构造器的重载](#可失败构造器的重载)
    6. [6.6. init!可失败构造器](#init-可失败构造器)
7. [7. 必需构造器(Required Initializers)](#必需构造器-Required-Initializers)
8. [8. 通过闭包和函数来设置属性的默认值](#通过闭包和函数来设置属性的默认值)
