...
source_url: 'http://yulingtianxia.com/blog/2015/11/13/Summary-of-the-first-month-in-the-internship-of-Tencent/'
original_language: zh
published: 2019-05-26
status: frozen
license: 未声明 → 仅私有归档
archived_at: 2026-07-26
content_hash: 'sha256:1b4198d26a90caae'
translated: n/a
---

> 原文：[腾讯实习第一个月工作总结](http://yulingtianxia.com/blog/2015/11/13/Summary-of-the-first-month-in-the-internship-of-Tencent/)　·　杨萧玉
...
1. 1. 让 UIAlertController 兼容 iOS7
    ...
    4. 1.4. 解决思路
          ...
          1. 1.4.1. 构建 TBAlertAction 替代 UIAlertAction
          2. 1.4.2. 构建 TBAlertController 属性
          3. 1.4.3. 构建 TBAlertController 方法
          4. 1.4.4. hook presentViewController: 方法
          5. 1.4.5. 还需要注意的地方
    5. 1.5. 感悟&总结
2. 2. LLDB 调试 UI 的技巧
...
3. 3. 项目实践管理经验
    ...
    2. 3.2. 文案管理
          ...
          1. 3.2.1. 文案是可复用的
          2. 3.2.2. 重复的文案在多处 hardcode 会降低效率
          3. 3.2.3. 我的解决方案：文案与代码分离
          4. 3.2.4. 解决方案的分析
    3. 3.3. project.pbxproj文件

来腾讯实习正好一个月了，因为项目已经进入了成熟期，接了两个需求，改了二十几个 bug。所以把这期间的一些思考写下来，发了邮件分享给组内同事们。我把一些自己觉得不合适的内容刨除出去，选取邮件的部分内容写在这里：）

# [#让-UIAlertController-兼容-iOS7](#让-UIAlertController-兼容-iOS7)让 UIAlertController 兼容 iOS7
...
### [#构建-TBAlertAction-替代-UIAlertAction](#构建-TBAlertAction-替代-UIAlertAction)构建 TBAlertAction 替代 UIAlertAction
...
### [#构建-TBAlertController-属性](#构建-TBAlertController-属性)构建 TBAlertController 属性
...
### [#构建-TBAlertController-方法](#构建-TBAlertController-方法)构建 TBAlertController 方法
...
### [#hook-presentViewController-方法](#hook-presentViewController-方法)hook presentViewController: 方法
...
### [#还需要注意的地方](#还需要注意的地方)还需要注意的地方

这里需要注意两点：

1. 时，必需在添加完所有 Action 之后才能设定它的

  属性，否则会在 iOS8 下出现问题：取消按钮与其他按钮连成一片。而在 iOS9 下面则不会出现此问题。这也是为什么我会在 hook 到
...
# [#LLDB-调试-UI-的技巧](#LLDB-调试-UI-的技巧)LLDB 调试 UI 的技巧
...
## [#用-LLDB-改变视图](#用-LLDB-改变视图)用 LLDB 改变视图
...
通过 `e` 命令，可以_在调试时改变变量的值，比如某个视图的透明度、背景颜色等，还可以临时创建新的视图或者视图控制器，甚至可以完成视图控制器的 push 操作_。虽然此时程序（某个进程）是被我们暂停了的，但是后台的渲染服务是由后台的另一个进程完成的，所以可以**强制触发渲染服务**：

```arduino
e (void)[CATransaction flush]
```

## [#使用-Chisel-简化-LLDB-命令](#使用-Chisel-简化-LLDB-命令)使用 Chisel 简化 LLDB 命令
...
图片一旦用 Asset Catalog 管理后，在程序中加载图片只需要用图片名而无需后缀名，更不用关心几倍分辨率，是用2x 还是 3x？Xcode 全都帮我们搞定了！之前碰到过一个 iOS7 上图片被拉伸的 bug，原因就是没有使用 Asset Catalog 管理图片资源，而是直接用文件夹。而在代码中却写了图片文件的全名，比如 “[pic@2x.png](mailto:pic@2x.png)”，本来贴心的 Xcode 会为我们在后缀名前自动加上 “@2x” 来寻找图片，于是去查找 “pic@[2x@2x.png](mailto:2x@2x.png)”，发现并没有这个二倍分辨率图片，Xcode 于是只好继续寻找一倍分辨率的图片，也就是 “[pic@2x.png](mailto:pic@2x.png)” 被当做了一倍分辨率的图片处理了，所以 `UIImage` 的 `size` 就变大了。

## [#文案管理](#文案管理)文案管理

### [#文案是可复用的](#文案是可复用的)文案是可复用的
...
### [#重复的文案在多处-hardcode-会降低效率](#重复的文案在多处-hardcode-会降低效率)重复的文案在多处 hardcode 会降低效率
...
### [#我的解决方案：文案与代码分离](#我的解决方案：文案与代码分离)我的解决方案：文案与代码分离
...
## [#project-pbxproj文件](#project-pbxproj文件)project.pbxproj文件
