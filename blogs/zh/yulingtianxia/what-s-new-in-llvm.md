...
source_url: 'http://yulingtianxia.com/blog/2017/07/17/What-s-New-in-LLVM-2017/'
original_language: zh
published: 2019-05-26
status: frozen
license: 未声明 → 仅私有归档
archived_at: 2026-07-26
content_hash: 'sha256:2bb1762fce8570ba'
translated: n/a
---

> 原文：[What's New in LLVM](http://yulingtianxia.com/blog/2017/07/17/What-s-New-in-LLVM-2017/)　·　杨萧玉
...
1. 1. API Availability Checking for Objective-C

    1. 1.1. Objective-C
    2. 1.2. C & C++
    3. 1.3. 适用范围
2. 2. Static Analyzer Checks

    1. 2.1. 比较 NSNumber
    2. 2.2. dispatch_once()
    3. 2.3. NSMutable 类型的属性使用 copy 内存管理语义
    4. 2.4. 一些建议
3. 3. New Warnings

    1. 3.1. ARC 中的 Block 捕获参数

          1. 3.1.1. Clang 5 中的 Objective-C ARC 方法家族
          2. 3.1.2. 探索方法返回值内存管理的奥秘

                  1. 3.1.2.1. 方法为 alloc/new/copy/mutableCopy 家族或方法声明使用 NS_RETURNS_RETAINED
                  2. 3.1.2.2. 普通方法
                  3. 3.1.2.3. 优化过程及原理
    2. 3.2. 无参数函数的声明
4. 4. C++ Refactoring
5. 5. Features from C++17

    1. 5.1. Structured Binding
    2. 5.2. 在条件判断语句中声明初始化变量
    3. 5.3. constexpr if
    4. 5.4. string_view
6. 6. Link-Time Optimization
...
## [#API-Availability-Checking-for-Objective-C](#API-Availability-Checking-for-Objective-C)API Availability Checking for Objective-C
...
## [#Static-Analyzer-Checks](#Static-Analyzer-Checks)Static Analyzer Checks
...
### [#比较-NSNumber](#比较-NSNumber)比较 NSNumber
...
### [#dispatch-once](#dispatch-once)`dispatch_once()`
...
### [#NSMutable-类型的属性使用-copy-内存管理语义](#NSMutable-类型的属性使用-copy-内存管理语义)`NSMutable` 类型的属性使用 `copy` 内存管理语义
...
3. 是否有内容时直接看
  ...
  。这跟打游戏是否有意识差不多，写代码也要有『意识流』。

## [#New-Warnings](#New-Warnings)New Warnings
...
### [#ARC-中的-Block-捕获参数](#ARC-中的-Block-捕获参数)ARC 中的 Block 捕获参数
...
ARC 会将 `(NSError **)error` 这种 “out-parameter” 隐式修饰为 `__autoreleasing`，也就是其实会被转化成 `(NSError * __autoreleasing *)error`。（PS：相关知识参考 [Indirect parameters](http://clang.llvm.org/docs/AutomaticReferenceCounting.html#indirect-parameters)）
...
#### [#Clang-5-中的-Objective-C-ARC-方法家族](#Clang-5-中的-Objective-C-ARC-方法家族)Clang 5 中的 Objective-C ARC 方法家族
...
1. 家族的方法创建的对象是自己持有的，会被 Clang 隐式标记为
...
2. 家族会被 Clang 隐式标记为
  ...
  宏也是等效的。
3. ，等效于 Foundation 中的宏

  。

一般情况下 Clang 会帮我们做这些事情，不必给方法声明手动标记这些属性。而且 Clang 会对最终生成的汇编指令进行优化，去掉多余的 `retain` 和 `autorelease`。

ARC 会**视情况**在调用方法时**可能会**添加 `retain`，在方法内部返回时**可能会**添加 `autorelease`，经过优化后很可能会抵消。
...
#### [#探索方法返回值内存管理的奥秘](#探索方法返回值内存管理的奥秘)探索方法返回值内存管理的奥秘

先做两种情况的实验，查看汇编代码，并总结：

##### [#方法为-alloc-new-copy-mutableCopy-家族或方法声明使用-NS-RETURNS-RETAINED](#方法为-alloc-new-copy-mutableCopy-家族或方法声明使用-NS-RETURNS-RETAINED)方法为 `alloc/new/copy/mutableCopy` 家族或方法声明使用 `NS_RETURNS_RETAINED`
...
## [#C-Refactoring](#C-Refactoring)C++ Refactoring
...
## [#Features-from-C-17](#Features-from-C-17)Features from C++17

其实这些都是 C++17 的新特性罢了，LLVM 9.0 积极响应支持。可以在编译选项 C++ Language Dialect 中选择使用的 C++ 标准库。GNU++17 比 C++17 多了语言扩展。

### [#Structured-Binding](#Structured-Binding)Structured Binding
...
### [#在条件判断语句中声明初始化变量](#在条件判断语句中声明初始化变量)在条件判断语句中声明初始化变量
...
### [#string-view](#string-view)string_view
...
## [#Link-Time-Optimization](#Link-Time-Optimization)Link-Time Optimization
