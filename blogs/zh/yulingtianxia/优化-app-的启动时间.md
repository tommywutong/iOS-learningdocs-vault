...
source_url: 'http://yulingtianxia.com/blog/2016/10/30/Optimizing-App-Startup-Time/'
original_language: zh
published: 2019-05-26
status: frozen
license: 未声明 → 仅私有归档
archived_at: 2026-07-26
content_hash: 'sha256:a020792f33efc2a8'
translated: n/a
---

> 原文：[优化 App 的启动时间](http://yulingtianxia.com/blog/2016/10/30/Optimizing-App-Startup-Time/)　·　杨萧玉
...
1. 1. App 运行理论
    ...
    1. 1.1. 理论速成
          ...
          1. 1.1.1. Mach-O 术语
          2. 1.1.2. Mach-O 镜像文件
          3. 1.1.3. Mach-O Universal 文件
          4. 1.1.4. 虚拟内存
          5. 1.1.5. Mach-O 镜像 加载
          6. 1.1.6. 安全
    2. 1.2. 从 exec() 到 main()
    3. 1.3. dyld 加载 dylib 文件

          1. 1.3.1. 加载 Dylib
          2. 1.3.2. Fix-ups
          3. 1.3.3. Rebasing 和 Binding
          4. 1.3.4. Rebasing
          5. 1.3.5. Binding
          6. 1.3.6. ObjC Runtime
          7. 1.3.7. Initializers
2. 2. 改善启动时间
...
#### [#Mach-O-镜像文件](#Mach-O-镜像文件)Mach-O 镜像文件
...
#### [#Mach-O-Universal-文件](#Mach-O-Universal-文件)Mach-O Universal 文件
...
#### [#Mach-O-镜像-加载](#Mach-O-镜像-加载)Mach-O 镜像 加载
...
### [#从-exec-到-main](#从-exec-到-main)从 `exec()` 到 `main()`
...
### [#dyld-加载-dylib-文件](#dyld-加载-dylib-文件)`dyld` 加载 dylib 文件
...
#### [#Rebasing-和-Binding](#Rebasing-和-Binding)Rebasing 和 Binding
...
2. 标志位为
  ...
  （也就是可以预绑定但没使用），且镜像在共享内存中，重置上下文中所有的 lazy pointer。（如果镜像在共享内存中，稍后会在 Binding 过程中绑定，所以无需重置）
3. ；否则进入第四步
4. 方法，这才是真正做 Rebasing 工作的方法。如果开启

  宏，会允许

  方法对

  段做写操作来对其进行 Fix-up。所以其实
...
#### [#ObjC-Runtime](#ObjC-Runtime)ObjC Runtime
...
C++ 会为静态创建的对象生成初始化器。而在 ObjC 中有个叫 `+load` 的方法，然而它被废弃了，现在建议使用 `+initialize`。对比详见：[http://stackoverflow.com/questions/13326435/nsobject-load-and-initialize-what-do-they-do](http://stackoverflow.com/questions/13326435/nsobject-load-and-initialize-what-do-they-do)
...
### [#优化启动时间](#优化启动时间)优化启动时间

可以针对 App 启动前的每个步骤进行相应的优化工作。

#### [#加载-Dylib-1](#加载-Dylib-1)加载 Dylib
...
#### [#Rebase-Binding](#Rebase-Binding)Rebase/Binding
...
#### [#ObjC-Setup](#ObjC-Setup)ObjC Setup
...
- 来替代
- 将方法显式标记为初始化器，而是让初始化方法调用时才执行。比如使用

  ,

  或

  。也就是在第一次使用时才初始化，推迟了一部分工作耗时。
