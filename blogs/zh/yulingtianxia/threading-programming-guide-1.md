...
source_url: 'http://yulingtianxia.com/blog/2017/08/28/Threading-Programming-Guide-1/'
original_language: zh
published: 2019-05-26
status: frozen
license: 未声明 → 仅私有归档
archived_at: 2026-07-26
content_hash: 'sha256:a3c0273273582946'
translated: n/a
---

> 原文：[Threading Programming Guide(1)](http://yulingtianxia.com/blog/2017/08/28/Threading-Programming-Guide-1/)　·　杨萧玉
...
1. 1. 线程相关概念
    ...
    4. 1.4. 设计秘籍
          ...
          3. 1.4.3. 避免共用数据结构
          4. 1.4.4. 线程与用户界面
          5. 1.4.5. 退出应用时注意线程的行为
          6. 1.4.6. 处理异常
          7. 1.4.7. 干净利落地终止线程
          8. 1.4.8. Library 中的线程安全
2. 2. 线程管理
    ...
    2. 2.2. 创建线程
          ...
          1. 2.2.1. 使用 NSThread
          2. 2.2.2. 使用 POSIX 线程
          3. 2.2.3. 使用 NSObject 产生线程
          4. 2.2.4. 在 Cocoa 应用中使用 POSIX 线程

                  1. 2.2.4.1. 保护 Cocoa 框架
                  2. 2.2.4.2. 混合使用 POSIX 和 Cocoa 锁
    3. 2.3. 配置线程属性

          1. 2.3.1. 配置线程的栈尺寸
          2. 2.3.2. 配置 TLS(Thread-Local Storage)
          3. 2.3.3. 设置线程的 Detached 状态
          4. 2.3.4. 设置线程优先级
    4. 2.4. 编写线程入口程序

          1. 2.4.1. 创建自动释放池
          2. 2.4.2. 设置 Exception Handler
          3. 2.4.3. 设置 Run Loop
    5. 2.5. 终止线程
...
## [#线程相关概念](#线程相关概念)线程相关概念

### [#概要和术语](#概要和术语)概要和术语

线程的概念在计算机组成原理中学过，在各个系统中的实现和行为也都大同小异。多线程编程有利也有弊：在提升程序运行效率和用户体验的同时也带来了线程间同步的种种问题。现在大多数 CPU 都是多核的，所以很多程序都是并发执行来提升性能。多线程编程的代码更复杂，尤其是在存取同一个数据时更需要注意。这里讲一讲 macOS 和 iOS 提供的有关线程的 API 以及同步机制。
...
- 方法可以立即抛通知，这是个同步执行的方法。其实通知会先进入一个先入先出的通知队列中，出列后才会被分发到通知中心。可以使用

  的

  `enqueueNotification:postingStyle:`

  方法异步发送通知，postingStyle 设为
...
### [#苹果系统的支持](#苹果系统的支持)苹果系统的支持

macOS 和 iOS 提供了几种创建线程的技术，以及对线程之间管理和同步任务的支持。

#### [#对线程的封装](#对线程的封装)对线程的封装
...
- Cocoa 的 API,非线程安全，必须在当前线程上下文中调用。它是对

  的封装。
- 为

  框架的 C 语言 API，线程安全。使用
...
#### [#退出应用时注意线程的行为](#退出应用时注意线程的行为)退出应用时注意线程的行为
...
#### [#使用-NSThread](#使用-NSThread)使用 `NSThread`
...
#### [#使用-POSIX-线程](#使用-POSIX-线程)使用 POSIX 线程
...
#### [#使用-NSObject-产生线程](#使用-NSObject-产生线程)使用 `NSObject` 产生线程
...
#### [#在-Cocoa-应用中使用-POSIX-线程](#在-Cocoa-应用中使用-POSIX-线程)在 Cocoa 应用中使用 POSIX 线程

在 Cocoa 中使用 POSIX 线程需要注意它们之间的交互，并遵守以下原则。

##### [#保护-Cocoa-框架](#保护-Cocoa-框架)保护 Cocoa 框架
...
##### [#混合使用-POSIX-和-Cocoa-锁](#混合使用-POSIX-和-Cocoa-锁)混合使用 POSIX 和 Cocoa 锁
...
### [#配置线程属性](#配置线程属性)配置线程属性

#### [#配置线程的栈尺寸](#配置线程的栈尺寸)配置线程的栈尺寸
...
#### [#配置-TLS-Thread-Local-Storage](#配置-TLS-Thread-Local-Storage)配置 TLS(Thread-Local Storage)
...
#### [#设置线程的-Detached-状态](#设置线程的-Detached-状态)设置线程的 Detached 状态
...
#### [#设置-Exception-Handler](#设置-Exception-Handler)设置 Exception Handler
...
#### [#设置-Run-Loop](#设置-Run-Loop)设置 Run Loop
