...
source_url: 'http://yulingtianxia.com/blog/2014/08/08/spritekitzai-ios8he-osx10-dot-10zhong-de-xin-te-xing/'
original_language: zh
published: 2019-05-26
status: frozen
license: 未声明 → 仅私有归档
archived_at: 2026-07-26
content_hash: 'sha256:6af1d00494317f2f'
translated: n/a
---

> 原文：[SpriteKit在iOS8和OSX10.10中的新特性](http://yulingtianxia.com/blog/2014/08/08/spritekitzai-ios8he-osx10-dot-10zhong-de-xin-te-xing/)　·　杨萧玉

# [SpriteKit在iOS8和OSX10.10中的新特性](http://yulingtianxia.com/blog/2014/08/08/spritekitzai-ios8he-osx10-dot-10zhong-de-xin-te-xing/)
...
1. 1. Shader
2. 2. Lighting and Shadows
3. 3. New Physics

    1. 3.1. Per-Pixel Physics
    2. 3.2. Constrains
    3. 3.3. Inverse Kinematics
    4. 3.4. Physics Fields
4. 4. Integration with SceneKit
5. 5. Tools
6. 6. Improvements
    ...
    2. 6.2. SKTexture
    3. 6.3. SKShapeNode
    4. 6.4. Physics Updates
    5. 6.5. SKTexture Atlas
...
## [#Lighting-and-Shadows](#Lighting-and-Shadows)Lighting and Shadows
...
- 光源的开关
- 环境色，默认是黑色，也就是没有环境色。它无视自身的alpha以及
...
- 光的种类，32位整型数。
  ...
  存储着光的种类，分别意味着：被何种光照亮、被何种光产生的阴影覆盖和遮挡何种光线并产生阴影。
...
## [#New-Physics](#New-Physics)New Physics

### [#Per-Pixel-Physics](#Per-Pixel-Physics)Per-Pixel Physics
...
### [#Physics-Fields](#Physics-Fields)Physics Fields
...
- 一些场对物体的影响程度跟距离有关，当物体与场的距离小于
  ...
  的默认值很小很小，但不是0
- 场的类别。当一个节点进入了场的影响区域，会根据节点的物理体属性的

  属性来判断此节点是否收到场的影响。此外

  的

  属性还会标记物理体所带的电荷量，这在跟电磁有关的场中会影响到物理体的运动。默认为 0xFFFFFFFF
- 场的“开关”
- 唯我独尊，这个场的地盘内，别的场的影响被无视。别让两个唯我独尊的场影响区域重叠，那样会出乱子的。默认为
...
## [#Integration-with-SceneKit](#Integration-with-SceneKit)Integration with SceneKit
...
### [#Physics-Updates](#Physics-Updates)Physics Updates

- 新加了

  属性来标志此物理体对应的节点是否被钉在它的父节点上。如果父节点也有物理体，那么这两个物理体被认为被一个pin连接。如果将

  设为NO并且

  设为YES，那么它相当于被焊在父节点上了，因为它不能转动了。
- 允许用

  把多个物理体组合在一起来创建一个新的物理体，还记得前面提到过的斧头么：

  ![](http://yulingtianxia.com/resources/140749497219.png)

### [#SKTexture-Atlas](#SKTexture-Atlas)SKTexture Atlas
