...
- 中复用
  ...
  进行注册，那么你就会收获一个应用崩溃，至于为什么需要先进行注册，

  register(_:forCellReuseIdentifier:)

  有简单说明；
- 。在数据源变化时我们需要更新对应的界面，而苹果本身并没提供一套相关的 API 给我们计算相关的删除，插入或者更新的位置（

  ），我们需要自己去计算，找出更新的数据源，然后删除，插入或者更新对应的

  。在大多数情况下，由于有复用机制的存在，我们直接使用

  也不会有什么问题，复用机制使得在

  时只会对显示在屏幕上的

  进行处理，需要处理视图数量并不多，但是应该有更好的办法来支持

  ，不是么？（ PS ：苹果在 WWDC19 上推出 Diffable Data Source 功能，支持局部刷新）；
- 的配置，处理埋点统计之类的功能，对于这些模块的隔离，系统本身的框架并没有提供这么一套东西给我们，所以很多时候会写在

  里，于是变成了

  。如果自行设计一套方案，很容易弄出一套不但使用起来非常难受，而且还难以迁移的框架。
