...
在 [Plausible Labs](http://plausible.coop/)，我们想把一个大项目迁到 ARC，但又需要保持对 iOS 4 的兼容。一个办法是使用 [`MAZeroingWeakRef`](https://github.com/mikeash/MAZeroingWeakRef) 这样的显式代理对象来管理零值弱引用（zeroing weak reference）。另一个办法是用 `__unsafe_unretained`，它其实并不比手动内存管理时代的老式不保留引用更不安全。
...
它从弱指针里取出值并返回，返回前会先对取出的值做保留并放入自动释放池，确保它活得够久、调用方来得及使用。凡是在表达式里用到 `__weak` 变量的地方都会用到它。
...
看样子只是为了给优化留出余地。虽然这些函数_都_能基于两个原语实现，但取决于运行时的实现方式，某些场景（比如初始化一个确定从未用过的 `__weak` 变量）可能存在更快的路径。虽然运行时目前没有利用这一点，但让编译器生成更专门的调用，就为将来留出了可能。

由于 `PLWeakCompatibility` 并不特别在意旧平台上的速度，在新平台上又只是直接转发到 Apple 的实现，我们干脆把其余调用全部基于那两个原语来实现。

**骗过编译器**  
运行时函数调用的生成方式与其他任何函数调用完全一样。也就是说，只要你的 App 里有一个名为 `objc_storeWeak` 的函数，编译器就会乐呵呵地生成调用它的代码——它并没有与 Objective-C 运行时库硬绑定。然而默认情况下，当部署目标是不官方支持 `__weak` 的操作系统时，`clang` 会拒绝编译任何含 `__weak` 的代码。好在可以通过一对编译器标志告诉 `clang`：当前目标真的支持 `__weak`：
