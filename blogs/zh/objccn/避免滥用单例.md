...
source_key: objccn
source_url: 'https://objccn.io/issue-13-2'
original_language: zh
published: ''
status: frozen
license: 页脚「© 2015 至今」→ 仅私有归档
archived_at: 2026-07-26
content_hash: 'sha256:7b8bb61e75ac5eee'
translated: n/a
---

> 原文：[避免滥用单例](https://objccn.io/issue-13-2)　·　ObjC 中国 (objccn.io)
...
尽管有些人认为单例是 '反模式'， '魔鬼' 以及 ['病态的说谎者'](http://misko.hevery.com/2008/08/17/singletons-are-pathological-liars/)，我不会去完全否认单例所带来的的好处，而是会展示一些使用单例所带来的问题，这样下一次在使用 `dispatch_once` 代码片段的自动补全功能时，你可以对它的影响进行评估，三思而行。
...
分析到这里，我希望你能够意识到，“这个缩略图 cache 从来就不应该作为一个单例！”。问题在于一个对象得生命周期可能在项目的最初阶段没有被很好得考虑清楚。举一个具体的例子，Dropbox 的 iOS 客户端曾经只支持一个账号登录。它以这样的状态存在了数年，直到有一天我们希望能够同时支持[多个用户账号](https://www.dropbox.com/business/two-dropboxes)登录 (同时登陆私人账号和工作账号)。突然之间，我们以前的的假设“只能够同时有一个用户处于登录状态”就不成立了。如果假定了一个对象的生命周期和应用的生命周期一致，那你的代码的灵活扩展就受到了限制，早晚有一天当产品的需求产生变化时，你会为当初的这个假定付出代价的。
...
我们可以显式地把经过认证的 user 对象作为参数传递给这个 view controller。这种把依赖性传递给依赖对象的技术正式的叫法是[依赖注入](http://en.wikipedia.org/wiki/Dependency_injection)，它有很多优点：

1. 头文件的读者来说，可以很清楚的知道它只有在有登录用户的情况下才会被展示。
...
![](https://objccn.io/images/issues/issue-13/Screen%20Shot%202014-06-02%20at%205.21.20%20AM.png)

view controller 自己，以及自定义的 image view 的列表，都会和 `sharedThumbnailCache` 产生交互。当用户登出后，我们想要清理 root view controller 并且退出到登录页面：

![](https://objccn.io/images/issues/issue-13/Screen%20Shot%202014-06-02%20at%205.53.45%20AM.png)

这里的问题在于这个好友列表的 view controller 可能仍然在执行代码 (由于后台操作的原因)，并且可能因此仍然有一些没有执行的涉及到 `sharedThumbnailCache` 的调用。

和使用依赖注入的解决方案对比一下：

![](https://objccn.io/images/issues/issue-13/Screen%20Shot%202014-06-02%20at%205.38.59%20AM.png)

简单起见，假设 `SPApplicationDelegate` 管理 `SPUser` 的实例 (在实践中，你可能会把这些用户状态的管理工作交给另外一个对象来做，这样可以使你的 application delegate [简化](http://www.objc.io/issue-1/lighter-view-controllers.html))。当展现好友列表 view controller 时，会传递进去一个 user 的引用。这个引用也会向下传递给 profile image views。现在，当用户登出时，我们的对象图如下所示：

![](https://objccn.io/images/issues/issue-13/Screen%20Shot%202014-06-02%20at%205.54.07%20AM.png)
...
![](https://objccn.io/images/issues/issue-13/Screen%20Shot%202014-06-02%20at%205.59.25%20AM.png)
...
![](https://objccn.io/images/issues/issue-13/Screen%20Shot%202014-06-02%20at%206.43.56%20AM.png)
