...
source_key: objccn
source_url: 'https://objccn.io/issue-19-4'
original_language: zh
published: ''
status: frozen
license: 页脚「© 2015 至今」→ 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:e1e5b8892e4e6272'
translated: n/a
---

> 原文：[DTrace](https://objccn.io/issue-19-4)　·　ObjC 中国 (objccn.io)

[返回期刊列表](https://objccn.io/issues/)

DTrace

[migrant](http://objcio.com/)
...
大概 10 年前，[Sun Microsystems](https://en.wikipedia.org/wiki/Sun_Microsystems) 创建了 DTrace，它的名字是 _Dynamic Trace_ 的缩写。2007 年底，苹果公司将它集成在自己的 [操作系统](https://en.wikipedia.org/wiki/Mac_OS_X_Leopard) 中。

DTrace 是一个提供了 _zero disable cost_ 的动态追踪框架，也就是说当代码中的探针关闭时，不会有额外的资源消耗 - 即使在生产版本中我们也可以将探针留在代码中。只有使用的时候才产生消耗。
...
探针被分组成所谓的 _providers_。参数 `int` 是正要进入的状态。在我们的 Objective-C (或 Swift) 代码中，简单的插入以下代码即可：
...
如果将 DTrace 保存进 `state.d`，接下来我们可以使用 [`dtrace(1)` 命令行工具](https://developer.apple.com/library/mac/documentation/Darwin/Reference/ManPages/man1/dtrace.1.html) 来运行它：
...
操作系统，尤其是 kernel，提供了数以千计的探针，被分成不同的提供者 (provider) 组。其中的很多在 [Oracle 的 DTrace 文档](https://docs.oracle.com/cd/E23824_01/html/E22973/gkyal.html)中可以找到。
...
这里 `198` 是进程标识 (亦称 PID)。我们可以在活动监视器这个 app 中找到这个数字，或者使用 [`ps(1)` 命令行工具](https://developer.apple.com/library/mac/documentation/Darwin/Reference/ManPages/man1/ps.1.html)。
...
断言 (_predicate_) 和动作语句 (_action statement_) 部分都是可选的。
...
当**动作语句**开始运行时我们可以使用断言来限制。当触发特定的探针时断言会被计算。如果断言结果为非 0，_action statements_ 将会运行，这和 C 语言中的 `if` 语句类似。
...
这限制了我们能做的事情。但是一旦知道了一些常见的模式，这种简单也给了我们很多灵活性，我们将在下一节详细讨论。在 [D Programming Language](https://docs.oracle.com/cd/E23824_01/html/E22973/gkwpo.html#scrolltoc) 的指南中可以查看更多的细节。

## 常见 D 语言模式

下面的例子会给让我们认识一些我们能做的事情。

这个例子统计了 _App Store_ 应用在 syscall (也就是一个系统调用，或对 kernel 中进行的调用) 中累计使用的时间。
...
如果运行这个并且开启 _App Store_ 应用，然后用 `^C` 退出 DTrace 脚本，可以得到像这样的输出:
...
在这个例子中，_App Store_ 在 `kevent64` 中花费了 3.6 秒。
...
通过 [Dynamic Tracing Guide, “Variables.”](https://docs.oracle.com/cd/E19253-01/817-6223/chp-variables/index.html) 可以查看关于变量的核心知识。
...
对于 Wire 应用，我们想要追踪 [NSURLSessionTask](https://developer.apple.com/library/ios/documentation/Foundation/Reference/NSURLSessionTask_class/index.html) 实例的往复时间。当开始一个任务时，我们触发一个[静态定义的探针](#staticProbes)，当完成时还有另一个探针。我们可以写一个简单的脚本：
...
我们传入 [`taskIdentifer`](https://developer.apple.com/library/ios/documentation/Foundation/Reference/NSURLSessionTask_class/index.html#//apple_ref/occ/instp/NSURLSessionTask/taskIdentifier) 作为 `arg1`，任务开始时 `arg0` 被设置为 4，任务完成时被设置为 6。
...
其中一些提供者是 [Dynamic Tracing Guide](https://docs.oracle.com/cd/E23824_01/html/E22973/gkyal.html) 文档中的原始集合中的一部分。让我们看看其中一些我们可用的。
...
[`profile` 提供者](https://docs.oracle.com/cd/E19253-01/817-6223/chp-profile/index.html)可以用来在某种程度上采样，这对于 Instruments 的用户来说应该非常熟悉。
...
通过查看 [`dtrace(1)` 帮助页面](https://developer.apple.com/library/mac/documentation/Darwin/Reference/ManPages/man1/dtrace.1.html)可以获得更多详细信息。

### `io` 提供者

为了追踪与磁盘输入输出相关的活动，[`io` 提供者](https://docs.oracle.com/cd/E19253-01/817-6223/chp-io/index.html) 定义了 6 个探针:
...
[Oracle 文档](https://docs.oracle.com/cd/E19253-01/817-6223/chp-io/index.html)中的例子展示了如何使用：
...
[`ip` 提供者](https://wikis.oracle.com/display/DTrace/ip+Provider)有 `send` 和 `receive` 两个探针。任何时候数据通过 IP 被发送或接收都会触发。参数 `arg0` 到 `arg5` 提供了与发送或接收的 IP 包所相关的 kernel 结构体的访问入口。

可以将二者放入非常强大的网络调试工具中。它可以使 `tcpdump(1)` 的看起来像过时的玩意。`ip` 提供者可以让我们在需要的时候精确的输出我们所需要的信息。

查看[文档](https://wikis.oracle.com/display/DTrace/ip+Provider)获得更多很棒的示例。
...
现在我们需要将 `provider.d` 加入到 Xcode 的构建目标中。确保将类型设置为 _DTrace source_，这十分重要。Xcode 现在会在构建时自动处理。在这个步骤中，DTrace 会创建一个对应的 `provider.h` 头文件，我们可以引入它。将 `provider.d` 同时加入 Xcode 工程和相应的构建目标非常重要。
...
这会生成相应的头文件。该文件最后会出现在 [`DERIVED_FILE_DIR`](https://developer.apple.com/library/mac/documentation/DeveloperTools/Reference/XcodeBuildSettingRef/1-Build_Setting_Reference/build_setting_ref.html#//apple_ref/doc/uid/TP40003931-CH3-SW43) 中。可以通过以下方式在任何工程内的源文件中引用
...
记得查看 [Dynamic Tracing Guide](https://docs.oracle.com/cd/E23824_01/html/E22973/toc.html) ([PDF version](https://docs.oracle.com/cd/E23824_01/pdf/E22973.pdf))。并且在你系统的 `/usr/share/examples/DTTk` 文件夹中获取更多的灵感。
...
![](https://objccn.io/images/issues/authors/2014/Apr/migrant.jpg)
