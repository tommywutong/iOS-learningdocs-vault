...
source_key: objccn
source_url: 'https://objccn.io/issue-9-2'
original_language: zh
published: ''
status: frozen
license: 页脚「© 2015 至今」→ 仅私有归档
archived_at: 2026-07-26
content_hash: 'sha256:3044a116656d7499'
translated: n/a
---

> 原文：[玩转字符串](https://objccn.io/issue-9-2)　·　ObjC 中国 (objccn.io)
...
排序和比较字符串比第一眼看上去要复杂得多。不只是因为字符串可以包含**代理对（surrogate pairs ）**(详见 [Ole 写的这篇关于 Unicode 的文章](http://objccn.io/issue-9-1/#peculiar-unicode-features)) ，而且比较还与字符串的本地化相关。在某些极端情况下相当棘手。

苹果文档中 _String Programming Guide_ 里有一节叫做 [**“字符与字形集群（Characters and Grapheme Clusters）”**](https://developer.apple.com/library/mac/documentation/Cocoa/Conceptual/Strings/Articles/stringsClusters.html)，里面提到一些陷阱。例如对于排序来说，一些欧洲语言将序列“ch”当作单个字母。在一些语言里，“ä”被认为等同于 ‘a’ ，而在其它语言里它却被排在 ‘z’ 后面。
...
需要注意这些格式化方法都是_非本地化_的。所以这些方法得到的字符串是不能直接拿来显示在用户界面上的。如果需要本地化，那我们需要使用下面这些方法:
...
Florian 有一篇关于[字符串的本地化](http://objccn.io/issue-9-3/#localized-format-strings)的文章更详细地讨论了这个问题。

[printf(3)](https://developer.apple.com/library/mac/documentation/Darwin/Reference/ManPages/man3/printf.3.html) 的 man 页面有关于它如何格式化字符串的全部细节。除了以 `%` 字符开始的所谓**格式转换符（conversion specification）**，格式化字符串会被逐字复制：
...
除了来自 `printf(3)` 的转换规范，我们还可以使用 `%@` 来输出一个对象。在[对象描述](#object-description)那一节中有述，如果对象响应 `-descriptionWithLocale:` 方法，则调用它，否则调用 `-description`。`%@` 被结果替换。
...
`%d` 和 `%i` 具有一样的功能，它们都打印出有符号十进制数。`%o` 就较为晦涩了：它使用[八进制](https://en.wikipedia.org/wiki/Octal)表示。`%u` 输出无符号十进制数——它是我们常用的。最后 `%x` 和 `%X` 使用十六进制表示——后者使用大写字母。
...
如有一个字符串 “bird” ，找出组成它的独立字母是很简单的。第二个字母是“i”（Unicode: LATIN SMALL LETTER I）。而对于像[Åse](https://en.wikipedia.org/wiki/Åse)这样的字符串就没那么简单了。看起来像三个字母的组合可有多种方式，例如：
...
从 [Ole 写的这篇关于 Unicode 的文章](http://objccn.io/issue-9-1/#peculiar-unicode-features) 里可以读到更多关于**联合标记（combining marks）**的信息，其他语言文字有更多复杂的**代理对（complicated surrogate pairs）**。

如果我们要在字符层面处理一个字符串，那我们就要小心翼翼。苹果的文档中 String Programming Guide 里有一节叫做 [“Characters and Grapheme Clusters”](https://developer.apple.com/library/mac/documentation/Cocoa/Conceptual/Strings/Articles/stringsClusters.html)，里面有更多关于这一点的细节。
...
再者，`NSURL` 有八个方法来访问被称为 _resource values_ 的东西。这些方法提供了一个稳定的接口，使我们可以用来获取和设置文件与目录的各种属性，例如本地化文件名（`NSURLLocalizedNameKey`）、文件大小（`NSURLFileSizeKey`），以及创建日期（`NSURLCreationDateKey`），等等。
...
`-fileSystemRepresentation` 所做的是它首先将这个字符串转换成文件系统的[规范形式](http://objccn.io/issue-9-1/#normalization-forms)然后用 UTF-8 编码。
