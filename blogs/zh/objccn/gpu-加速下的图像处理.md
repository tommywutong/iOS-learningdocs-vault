...
source_key: objccn
source_url: 'https://objccn.io/issue-21-7'
...
> 原文：[GPU 加速下的图像处理](https://objccn.io/issue-21-7)　·　ObjC 中国 (objccn.io)
...
我找到的最好的关于线性代数和矩阵是如何工作的资源是这个网站的[更好的解释](http://betterexplained.com/articles/linear-algebra-guide/)。我从这个网站~~偷来~~借鉴的一句引述就是：
...
*_`clamp()`:_ GLSL 中一个比较一致的方面就是它喜欢使用归一化的坐标。它希望收到的颜色分量或者纹理坐标的值在 0.0 和 1.0 之间。为了保证我们的值不会超出这个非常窄的区域，我们可以使用 `clamp()` 函数。 `clamp()` 会检查并确保你的值在 0.0 和 1.0 之间。如果你的值小于 0.0，它会把值设为 0.0。这样做是为了防止一些常见的错误，例如当你进行计算时意外的传入了一个负数，或者其他的完全超出了算式范围的值。
