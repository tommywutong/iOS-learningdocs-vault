...
source_key: objccn
source_url: 'https://objccn.io/issue-2-5'
original_language: zh
published: ''
status: frozen
license: 页脚「© 2015 至今」→ 仅私有归档
archived_at: 2026-07-26
content_hash: 'sha256:67ef25b9e41c7075'
translated: n/a
---

> 原文：[测试并发程序](https://objccn.io/issue-2-5)　·　ObjC 中国 (objccn.io)
...
XCode4 所使用的测试框架是基于 [OCUnit](http://www.sente.ch/software/ocunit/)。为了理解之前所提到的异步测试问题，我们需要了解一下测试包中的各个部分之间的执行顺序。下图展示了一个简化的流程。

![SenTestingKit call stack](https://objccn.io/images/issues/issue-2/SenTestingKit-call-stack.png)

在测试框架在主 run loop 开始运行之后，主要执行了以下几个步骤：

1. 配置一个包含所有相关测试的测试包 (比如可以在工程的 scheme 中配置)。
2. 开头测试用例的方法。运行结束后会返回一个包含单个测试结果的对象。
3. 退出测试。
...
[Kiwi](https://github.com/allending/Kiwi) 使用探测轮询 (probe poller)，它可以在测试方法中被调用。 [GHUnit](https://github.com/gabriel/gh-unit/) 编写了一个单独的测试类，它必须在测试的方法内初始化，并在结束时接收一个通知。以上两种方式都是通过编写相应的代码来确保异步测试方法在测试结束之前都不会返回。

## SenTestingKit的异步扩展

我们对这个问题的解决方案是对 SenTestingKit 添加一个[扩展](https://github.com/nxtbgthng/SenTestingKitAsync)，它在栈上使用同步执行，并把每个部分加入到主队列上。正如下图所见，在验证整个测试框架结果之前，报告异步测试成功或者失败的 Block 就被加入到队列。这种执行顺序允许我们开启一个测试并等待它的测试结果。

![SenTestingKitAsync call stack](https://objccn.io/images/issues/issue-2/SenTestingKitAsync-call-stack.png)

如果测试方法以 **Async** 结尾，框架就会认为该方法是异步测试。此外，在异步测试中，我们必须手动地报告测试成功，同时为了防止 Block 永远不会被调用，我们还需添加了一个超时方法。之前的错误的测试方法修改后如下所示：
...
为了运用到实际中，我们创建了一个示例框架：[PinacotecaCore](https://github.com/objcio/issue-2-async-testing)，它从一个虚拟的服务器获取图像信息。框架中包含一个资源管理器，它对外提供一个可以根据图像 Id 获取图像对象的接口。该接口的工作原理是资源管理器从虚拟服务器获取图片对象的信息，并更新到数据库。

虽然这个示例框架只是为了演示，但在我们自己开发的许多应用中也使用了这种模式。

![PinacotecaCore architecture](https://objccn.io/images/issues/issue-2/PinacotecaCore.png)
...
在这个[测试实例](https://github.com/objcio/issue-2-async-testing/blob/master/PinacotecaCore/PinacotecaCoreTests/PCModelLayerTests.m)中，我们就是在 `setUp` 方法中设置 core data 堆栈，然后检查 `PCImage` 实体的描述是否存在，如果不存在就构造一个，并更新它的值。当然这和异步测试没有关系，我们就不深入细说了。
...
因为服务器根本不存在，一个比较好的做法就是伪造一个代理服务器，正好 [OHHTTPStubs](https://github.com/AliSoftware/OHHTTPStubs) 可以解决这个问题。在它的最新版本中，可以在示例的请求响应中包含一个 bundle，发送给客户端。
...
由于资源管理器的测试不应该依赖于服务器接口控制器，所以我们可以用 [OCMock](http://ocmock.org) 来模拟，如果要做方法的部分 stub，它是一个理想的框架。如以下的 [资源管理器测试](https://github.com/objcio/issue-2-async-testing/blob/master/PinacotecaCore/PinacotecaCoreTests/PCResourceManagerTests.m) :
...
在 [nxtbgthng](http://nxtbgthng.com) 项目中，我们用 [SenTestingKitAsync](https://github.com/nxtbgthng/SenTestingKitAsync) 框架来测试。但是像 [Kiwi](https://github.com/allending/Kiwi) 和 [GHUnit](https://github.com/gabriel/gh-unit/) 也都是不错的异步测试框架。建议你都可以尝试下，然后找到合适自己的测试工具并开始使用它。
...
![](https://objccn.io/images/issues/authors/2014/Mar/riven.jpg)
