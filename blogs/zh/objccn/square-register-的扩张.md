---
title: Square Register 的扩张
source: ObjC 中国 (objccn.io)
source_key: objccn
source_url: 'https://objccn.io/issue-22-3'
original_language: zh
published: ''
status: frozen
license: 页脚「© 2015 至今」→ 仅私有归档
archived_at: 2026-07-26
content_hash: 'sha256:7030ee8f3076b3c0'
translated: n/a
---

> 原文：[Square Register 的扩张](https://objccn.io/issue-22-3)　·　ObjC 中国 (objccn.io)
...
- - 功能是逐步发展的，而不是长期的被隔离的功能分支。
- - 新功能后面都有一个服务器控制的标志。该功能标志在开发环境之外是不被启用的，直到准备发布之时。
- - 如果某个改变在现有的功能上导致了回退，那这个回退必须被立即修复。
- - 团队在一个发布期限内可以包含两到三个功能而不是在某个特定版本仅发布一个功能。

这意味着，我们的主分支保持在一个稳定的状态。这就是所说的列车的一部分。

1. - 在每个月的月初，会从主分支创建一个发布分支。
2. - 如果一个功能准备发布 (基本已经没有问题)，它的功能标志被启用。如果不是，它必须等待下一班列车。
3. - 每个月的其余时间花费在对发布分支的错误修正上。如果一个团队不需要发布任何东西，它会继续在主分支上工作。
4. - 发布分支上的变化会不断合并回主分支。
5. - 在月底我们会把发布分支发布到 App Store。
6. - 之后的每月重复上述过程。
...
- - 你不打算去实现的什么？有什么界限？
- - 你将如何衡量成功或失败？
- - 你还调查过其他的什么解决方案 (如果有的话)？你为什么没有选择它们呢？
- - 你决定了要做什么？你为什么决定这样做？
- - 你所选择的设计有些什么细节？包括 API 概述，技术细节，以及 (可能的) 一些头文件例子，再配上其他任何你认为有用的东西。这是你把你的设计卖给你自己和你的同伴工程师们的地方。
...
- - 如果我们发布了一个 bug，由于 App Store 的审查过程会使得修复延迟约一个星期给客户。
- - 查找错误很重要，因为 Register 对餐厅、零售商店这些用户来说非常关键。
- - 在一个像 Register 这样大的应用程序里在合并后再去找 bug 是很困难的。

我们的 pull requests 流程是什么呢？每次 PR 必须满足：

- - 每一个 PR 都有对应的 JIRA issue。
- - 每一个变化后面都必须有是什么以及为什么的一个清晰的描述。
- - Pull request 的 diff 文件必须是 500 行以内。大型的 diff 是不允许的。如果一个 diff 比 500 行多很多的话，评审很容易看不到错误。
- - 不要把重构或重命名跟一个新的功能放到一起做。把它们分别开来。
- - 在增加其他评审之前，所有 PR 的作者都要求对改动先做自我审查。这是为了避免杂散的 NSLog，缺失的测试，不完整的实现方式之类的事情。
- - 评审员中的一个必须是被改变组件的所有者。我们需要明确的列出评审人员，以确保工程师们知道在他们的审核队列到底有些什么。
- - 包括了改动的稳定性和正确性测试。没有测试的 pull request 会被拒绝。

同样，评审们被要求：

- - 意见必须清晰，简明。对于新的工程师，评审应包括可遵循的例子。
- - 不要只说 “把 X 改成 Y”；也要解释了为什么应该这样改。
- - 这是自动格式化干的事 (接下来会讲)。
- - 每个代码审查意见必须被标记为下列之一： — 必须
  ...
  — 问题
- - 评审必须有一个乐于帮忙的心态来进行代码审查。评审的工作是帮助代码安全地合并，而不是阻止它合并。
