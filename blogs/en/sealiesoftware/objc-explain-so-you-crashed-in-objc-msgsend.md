...
* i386 note: The receiver is in eax for most crashes, but not all. If you manage to get far into
...
My test program crashed at

. In real crashes, these values are typically more interesting.
...
1. 's value using the register name in the table above.

  ```
      ecx: 0x000a1029
  ```
2. 's value. This will often be either the application itself or

  . If no image spans that address, give up.
3. ```
  ...
  This crash matches my installed

  for i386. If it's a system library, you may need the image from the OS version listed in the crash log. If it's your application, you did keep a copy of every version you shipped, right?

  ```

  ```
4. 's offset into the image.
