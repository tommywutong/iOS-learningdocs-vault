---
title: 'Statements, messages and reducers | Cocoa with Love'
source: Cocoa with Love (Matt Gallagher)
source_key: cocoawithlove
source_url: 'https://www.cocoawithlove.com/blog/statements-messages-reducers.html'
...
> 原文：[Statements, messages and reducers | Cocoa with Love](https://www.cocoawithlove.com/blog/statements-messages-reducers.html)　·　Cocoa with Love (Matt Gallagher)
...
_The boxed question mark character is not a mistake, the code deliberately prints the Unicode replacement character (`0xfffd`) on failure to convert to a `UnicodeScalar`._
...
The concept here is the third tier of computational unit that I want to discuss: a **reducer**. A reducer gets its name because it “reduces” the incoming stream of message values into a single internal state value. The new message is combined with the previous state and the result is the next state – in this way, an entire stream of values is “reduced” down to a single state value.

A reducer is an entity with identity (a reference type in Swift) and internal state that is accessed solely by sending incoming messages and passively receiving outgoing updates (usually copies of the state following the update).
...
> This representation goes a little further than a classic reducer. A classic reducer is usually `f(message, state) -> state` where the input state is immutable and the output value is the next state. I prefer the modification in the diagram where the state is `inout` and a separate return value `notification` explains _what_ changed. If `state` and `notification` are the same, then it’s identical to the classic reducer but when they’re different, `notification` provides some extra flexibility when you want to know the displayable output following the application of the reducer (something which requires cumbersome diff-ing with a classic reducer).
...
3. messages are the glue between the two
...
It’s a situation where _limiting_ the functionality of an interface will greatly improve the ability to maintain and iterate the design.
