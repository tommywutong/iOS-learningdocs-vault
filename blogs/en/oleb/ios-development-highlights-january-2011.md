...
- Design Principles for Structuring iPhone Apps
  ...
  . Your code will be better if you follow these patterns.
- an open-source project to create an `NSAttributedString` from HTML code

  , a feature that is available on OS X and sadly absent in the iOS SDK. This should make creating attributed strings much easier.
- open-sourced MyAppSales

  , his iPhone app for scraping iTunes Connect sales reports.
- the Complete Friday Q&A Archive in e-book form

  . If you want to have some of the best Cocoa knowledge around the Web in easily accessible form or just want to support Mike, I suggest you buy his e-book, although all the content is available for free on his website. Mike also posted about

  his experience in creating the book

  .
- How To Create A Mole Whacking Game with Cocos2D

  , and Rob from mobile bros. published

  one on an Angry Birds-style game

  , also using Cocos2D in conjunction with Chipmunk and SpaceManager.
- How To Make a Simple RSS Reader iPhone App

  .
- introduces the Heapshot Analysis tool in Instruments

  and explains how to use it to find abandoned memory in your app. If you want to see it in action, I also highly recommend the video for WWDC 2010 Session 311, “Advanced Memory Analysis with Instruments”.
...
- Good UI design is all about attention to detail. I love the attention to detail these authors spent in posting about great attention to detail:
    ...
    - Little Big Details
      ...
      , a new blog from Berlin that shows us many examples of great attention to detail (not focused on iOS).
    - how the iPhone Mail app decides whether to scroll up or not when new mail arrives

      . It all depends on where you are in the list.
    - on the Mac Finder’s selection behavior
...
- Testflight
  ...
  , the new hot way to distribute your iOS betas to testers, came out of beta. I haven’t tried it yet, but the Twittersphere is raving about it.
...
- asking your customers for reviews on the App Store

  and how to do that in a Mac app.
- about promoting his iPad app, Flickpad

  .

# The Competition

- switching from the iPhone to Windows Phone 7, both from a user and a developer perspective

  . His post makes me want to try out the Windows Phone platform myself. For what it’s worth, I’d much rather explore the capabilities of Windows Phone 7 than expand to Android development at the moment (not that I’d have time for either).
- Amazon announced that they will open an Android App Store

  themselves. I haven’t heard more about this since the announcement, but I’m curious when it will launch for customers and how it will be received. Amazon certainly knows how to sell stuff and, more importantly, they already have a huge customer base who trusts them. Amazon also plans to experiment with pricing and it will be interesting how that turns out.
...
- Engadget interview with Matias Duarte

  , the guy who designed Palm’s webOS and is now at Google doing UI design for Android 3.0 (25 min video).
