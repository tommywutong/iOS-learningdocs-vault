...
- A bunch of object and binary inspection tools, which we don’t need for the finished product but which I made a lot of use of when trying to [debug mystery misbehavior](#a-week-of-mysterious-failures)
...
- An actual machine running Mac OS 9. I _have_ one, but not the charger for it, and so I did most of my testing using [SheepShaver](https://sheepshaver.cebix.net). My friend Nadine provided some testing on actual machines once things were working.
...
- was the glue code for cross-library calls trashing other data?^[11](#fn:indirect) (see the [Mac OS Runtime Architectures](https://developer.apple.com/library/archive/documentation/mac/pdf/MacOS_RT_Architectures.pdf) guide)
...
- There’s no Unicode support, so no Strings. Arguably I could make a String without Characters, or a String using [MacRoman](https://en.wikipedia.org/wiki/Mac_OS_Roman) as the native encoding, but it wouldn’t necessarily look much like today’s Swift.String.
...
- I had to mess with [linkage](https://en.wikipedia.org/wiki/Linkage_(software)) in a number of ways to make the LLVM XCOFF backend happy, so I’m not sure multi-file builds would work. I didn’t even test it.
- I’m using Carbon, which means that my program _ought_ to work on older versions of Mac OS X as well, but my friend Nadine tried and it didn’t, and it wasn’t a priority to figure out.
