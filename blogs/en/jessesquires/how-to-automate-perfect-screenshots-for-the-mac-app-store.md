...
![Screenshot images from Xcode and and macOS](https://www.jessesquires.com/img/blog/mac-screenshot-process.jpg)

<sub>From left to right: raw output from Xcode UI Test, the same image after processing, the output from taking a screenshot in macOS.</sub>
...
![Blank Mac desktop screenshot background](https://www.jessesquires.com/img/blog/mac-screenshot-background.jpg)

<sub>Blank Mac desktop screenshot background</sub>
...
![Retrobatch workflow](https://www.jessesquires.com/img/blog/mac-screenshot-retrobatch.jpg)

<sub>Retrobatch workflow</sub>

Here is what’s happening:

1. The raw screenshots from Xcode are read from the specified input folder.
2. The corners get rounded correctly.
3. It checks if the filename contains “dark”. This is part of my file naming scheme. For screenshots in dark mode, I append “-dark”. The workflow branches based on if the screenshots are in light mode or dark mode, but the steps are equivalent.
4. The 1-point border is added using the correct color based on light mode or dark mode.
5. A subtle drop shadow gets added, roughly equivalent to how macOS renders. Again through trial and error, I arrived at a `(0,-50)` offset, a 70-point blur radius, and 60% opacity.
6. Next is the overlay step. This overlays the app window screenshot over the desktop background and centers it. The light mode and dark mode branches of the workflow have their respective light and dark desktop backgrounds.
7. The metadata is deleted, to reduce file size and unnecessary data.
8. The final images get written to the specified output folder. These images are the full resolution of my 14-inch M3 MacBook Pro desktop. (I use these screenshots to embed in device frames for promotional images on my website.)
9. There is one final branch that resizes and crops all screenshots to `2880x1800 px` because of the [archaic requirements](https://www.jessesquires.com/blog/2024/01/16/app-store-screenshot-requirements/) of App Store Connect. The Mac App Store does not accept screenshots using the default resolutions of modern M-series MacBooks.
...
2. Extract the screenshots from the test results and save them to my Retrobatch input folder.
3. Run the Retrobatch workflow, which saves everything to the output folder.

Here’s an example of the final product using my app, [Taxatio](https://hexedbits.com/taxatio/).

![Finished screenshot for Taxatio](https://www.jessesquires.com/img/blog/mac-screenshot-finished.jpg)

<sub>Finished screenshot for Taxatio</sub>
