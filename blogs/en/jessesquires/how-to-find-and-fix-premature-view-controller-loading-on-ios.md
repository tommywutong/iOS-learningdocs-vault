...
A [`UIViewController`](https://developer.apple.com/documentation/uikit/uiviewcontroller) is a fundamental component of iOS development ([doc archive](https://developer.apple.com/library/archive/featuredarticles/ViewControllerPGforiPhoneOS/index.html#//apple_ref/doc/uid/TP40007457-CH2-SW1)). In order to debug our problem, we need to understand the view controller lifecycle. If you unintentionally (or deliberately) interfere with it, bad things™ can happen.

The (simplified) sequence of events and state transitions is:

1. `init()` — Initializes the view controller.
2. The initialized view controller gets presented via one of the presentation methods, like [`present(_:animated:completion:)`](https://developer.apple.com/documentation/uikit/uiviewcontroller/1621380-present).
3. `loadView()` — The view gets created and loaded into memory.
4. `viewDidLoad()` — Called after the view is loaded.
5. `viewWillAppear(_:)` — Called before the view appears.
6. `viewDidAppear(_:)` — Called after the view is presented.
7. `viewWillDisappear(_:)` — Called before the view disappears.
8. `viewDidDisappear(_:)` — Called after the view is dismissed.
...
2. `loadView()`
3. `viewDidLoad()`
4. At a later time, presentation occurs via `present(_:animated:completion:)`, etc.
5. `viewWillAppear(_:)`
6. `viewDidAppear(_:)`
7. ….
...
![Symbolic breakpoint on viewDidLoad()](https://www.jessesquires.com/img/blog/viewdidload-breakpoint.jpg)

<sub>Symbolic breakpoint on viewDidLoad()</sub>
