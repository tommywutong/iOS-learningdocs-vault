...
source_url: 'https://www.jessesquires.com/blog/2020/12/15/running-multiple-dangers/'
original_language: en
published: 2020-12-15
status: active
license: © 2014–2026 Jesse Squires → 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:835d919d224ceee0'
translated: false
---

> 原文：[Running multiple instances of Danger](https://www.jessesquires.com/blog/2020/12/15/running-multiple-dangers/)　·　Jesse Squires
...
To run `danger` multiple times, you’ll need a separate `Dangerfile` for each run. You can name these however you like, for example `Dangerfile.preCI` and `Dangerfile.postCI`. Place the appropriate rules in each file. When you run `danger` you’ll need to specify a `danger_id` — otherwise, the second run will **overwrite** the results from the first run, erasing any comments that `danger` had left on the pull request.
