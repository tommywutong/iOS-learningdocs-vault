...
source_url: 'https://www.jessesquires.com/blog/2021/10/22/recently-updated-posts/'
original_language: en
published: 2021-10-22
status: active
license: © 2014–2026 Jesse Squires → 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:70489e6e17bdb03e'
translated: false
---

> 原文：[Recently updated posts](https://www.jessesquires.com/blog/2021/10/22/recently-updated-posts/)　·　Jesse Squires
...
If you are reading on my site, updates are clearly marked and awhile back I added a new section at [`/blog/recently-updated/`](https://www.jessesquires.com/blog/recently-updated/) that you can browse. However, if not visiting that page regularly readers would otherwise be completely unaware of these updates — until today. I am now publishing an “updates only” feed, to which you can [subscribe](https://www.jessesquires.com/subscribe/) (direct links: [rss](https://www.jessesquires.com/feeds/updates.xml), [json](https://www.jessesquires.com/feeds/updates.json)).

This new update feed is also hooked up to my existing automation that tweets newly published posts, so now updates will also be tweeted as they are published. If you [follow me](https://twitter.com/jesse_squires), you’ll start seeing these updates.
...
Again, you can see my [recently updated page here](https://www.jessesquires.com/blog/recently-updated/). This page does not get much traffic, but it is nice to have some output to verify that everything is working as expected. Note that because we sorted by `date-updated`, when iterating through the posts we need to use `reversed` so that the most recently updated post displays first.

There’s one bug I encountered. I use `limit:` to only display the 30 most recent entries. (For more, you can [visit the archive](https://www.jessesquires.com/blog/archive/).) For the regular list of posts, that looks like this:
...
Using `limit:` did not work in this scenario like it does when using `site.posts`. Even though we use `reversed` to iterate through `updated_posts` in reverse order, `limit:` appears to limit the iteration count _in-order_ using the _non-reversed_ array. Thus, we end up with the first `N` updated posts rather than the `N` most recently updated posts. In other words, the “limiting” occurs first, truncating the list of posts, _and then_ the truncated list is reversed. That is not what you want. To fix it, we need to manually check the loop index against our desired limit, then break out of the loop manually.
