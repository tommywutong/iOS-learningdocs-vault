...
Until this point, the only occurrences of `@objc` in this code base were confined to normal target-action selectors. We may not be writing _Pure Swift^TM_ apps yet since it’s still [Cocoa all the way down](http://inessential.com/2016/05/25/oldie_complains_about_the_old_old_ways), but we can still take advantage of many of Swift’s powerful features — unless we start introducing `@objc` in too many places.
