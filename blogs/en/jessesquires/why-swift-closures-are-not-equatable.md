...
Now, you _could_ dive into Swift’s “unsafe” APIs, create an `UnsafePointer`, compare them, etc. But that is a very bad idea.™ Even if you end up writing something that passes a unit test, it will probably not work with optimizations turned on — or worse, it will only sometimes work. Or, it will break in the future.
