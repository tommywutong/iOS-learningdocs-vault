---
title: LLVM meets Code Property Graphs - Low Level Bits 🇺🇦
source: Low Level Bits (Alex Denisov)
source_key: lowlevelbits
source_url: 'https://lowlevelbits.org/llvm-meets-code-property-graphs/'
...
> 原文：[LLVM meets Code Property Graphs - Low Level Bits 🇺🇦](https://lowlevelbits.org/llvm-meets-code-property-graphs/)　·　Low Level Bits (Alex Denisov)

# LLVM meets Code Property Graphs

_Published on Feb 23, 2021_

_This is a cross-post from LLVM’s blog post [LLVM meets Code Property Graphs](https://blog.llvm.org/posts/2021-02-23-llvm-meets-code-property-graphs/)_
...
![Different program representations](https://lowlevelbits.org/img/cpg/different-representations.png)

into a single representation - Code Property Graph:

![Code Property Graph](https://lowlevelbits.org/img/cpg/cpg.png)
...
One of the primary interfaces to the code property graphs is a tool called [Joern](https://joern.io). It provides the mentioned DSL and allows to query the CPG to discover specific properties of a program. Here are some examples of the Joern’s DSL:
...
This happens because `llvm::StructType::isLayoutIdentical` determines equality based on the pointers. That is, if all the struct elements are identical, then the layout identical. It also meant we could not use this approach to compare types from different LLVM contexts. We had to roll out our custom solution based on the [Tree Automata](https://lowlevelbits.org/type-equality-in-llvm/) to solve this issue.
...
Once you have [Joern](https://docs.joern.io/installation) and [llvm2cpg](https://github.com/ShiftLeftSecurity/llvm2cpg/releases/latest) installed the usage is straightforward:
