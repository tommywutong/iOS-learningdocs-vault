...
source: MaskRay (宋方睿)
source_key: maskray
source_url: 'https://maskray.me/blog/2024-02-20-mmu-less-systems-and-fdpic'
original_language: en
published: 2024-02-20
status: active
license: 未声明 → 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:979d2996c1f115b4'
translated: false
---

> 原文：[MMU-less systems and FDPIC](https://maskray.me/blog/2024-02-20-mmu-less-systems-and-fdpic)　·　MaskRay (宋方睿)

[2024-02-20](https://maskray.me/blog/2024-02-20-mmu-less-systems-and-fdpic)
...
Why is the offset not known at compile time? There are primarily two reasons.

First, eXecute in Place (XIP), where code resides in ROM while the data segment is copied to RAM. Therefore, the offset between the text and data segments is often not known at compile time.
...
Then, let's see a global variable initialized by the address of a function and a C++ virtual table. 1  
2  
3  
...
If GCC implements `-fno-plt`, it can use the following code sequence: 1  
2  
3  
4  
5  
...
Unlike standard `R_*_RELATIVE` relocations that use "*loc += load_base" semantics, the load address in FDPIC mode is dependent on the containing segment. The following code adapted fro musl demonstrates the behavior: 1  
2  
3  
4  
5  
...
- A fixed [PPC64 ELFv1 function descriptors ABI](https://maskray.me/blog/2023-02-26-linker-notes-on-power-isa#ppc64-elfv1-function-descriptors). However, PPC64 ELFv1's trick of `st_value` referring to the function descriptor is better than the existing FDPIC ABIs (sh, arm).
...
You can configure binutils with `--target=arm-unknown-uclinuxfdpiceabi` to get a BFD linker that supports FDPIC emulations. 1  
2  
3  
4  
5  
...
GNU ld' arm port fails on `R_ARM_GOTOFFFUNCDESC` referencing a hidden function symbol ([PR31408](https://sourceware.org/bugzilla/show_bug.cgi?id=31408)). 1  
2  
3  
4  
5  
...
Regular [`-fno-plt`](https://maskray.me/blog/2021-09-19-all-about-procedure-linkage-table#fno-plt) code loads the `.got.plt` entry using PC-relative addressing and performs an indirect branch. The FDPIC `-fno-plt` variant needs to load both the FDPIC register and the destination address.
