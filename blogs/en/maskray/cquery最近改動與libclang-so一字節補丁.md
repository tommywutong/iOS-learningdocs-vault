...
source: MaskRay (宋方睿)
source_key: maskray
source_url: 'https://maskray.me/blog/2017-12-25-cquery-updates-and-libclang-one-byte-patch'
original_language: en
published: 2017-12-25
status: active
license: 未声明 → 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:917b3c221b0eaccf'
translated: false
---

> 原文：[cquery最近改動與libclang.so一字節補丁](https://maskray.me/blog/2017-12-25-cquery-updates-and-libclang-one-byte-patch)　·　MaskRay (宋方睿)

[2017-12-25](https://maskray.me/blog/2017-12-25-cquery-updates-and-libclang-one-byte-patch)

# cquery最近改動與libclang.so一字節補丁

## cquery最近改動

cquery介紹參看[使用cquery：C++ language server](https://maskray.me/blog/2017-12-03-c++-language-server-cquery)。

最近cquery改動比較多(~~[終於要熬過這個聖誕了](https://twitter.com/HaskRay/status/933906701563994112)~~)：
...
- VSCode使用浮動窗口，顯示多行`textDocument/hover`不成問題。但Emacs lsp-mode和LanguageClient-neovim就遇到一些困難[https://github.com/autozimu/LanguageClient-neovim/issues/224](https://github.com/autozimu/LanguageClient-neovim/issues/224)[https://github.com/emacs-lsp/lsp-ui/issues/17](https://github.com/emacs-lsp/lsp-ui/issues/17)
...
[topisani](https://github.com/topisani) raised the topic in [https://gitter.im/cquery-project/Lobby](https://gitter.im/cquery-project/Lobby) and I fianlly made up my mind to diagnose it.
...
By appending `--log-file /tmp/cq.log` to the cquery command line, we can see the error message in `/tmp/cq.log`: 1  
2  
...
https://github.com/jacobdufault/cquery/issues/219 [HighCommander4](https://github.com/HighCommander4) narrowed it down to a very simple reproduce： 1  
2  
3  
4  
5  
...
In [https://github.com/llvm-mirror/clang/blob/master/tools/libclang/CXIndexDataConsumer.cpp#L203](https://github.com/llvm-mirror/clang/blob/master/tools/libclang/CXIndexDataConsumer.cpp#L203)
...
`dyn_cast_or_null<NamedDecl>(ASTNode.Parent)` may return NULL. In some code executed later on [https://github.com/llvm-mirror/clang/blob/master/tools/libclang/CXIndexDataConsumer.cpp#L935](https://github.com/llvm-mirror/clang/blob/master/tools/libclang/CXIndexDataConsumer.cpp#L935):
...
This redundant `if` statement produces 0x47aeca `test rax,rax`. If we replace it with `if (!DC)` (because `DC` may be NULL and we should avoid null pointer dereference), since `DC` is passed in the register R8, we may use `test r8,r8`: 1  
2  
...
Sadly radare2 dropped the ball when it was in need and I had to resort to `printf+dd`... Its assembling of `test r8,r8` was incorrect [https://github.com/radare/radare2/issues/9071](https://github.com/radare/radare2/issues/9071)😢
