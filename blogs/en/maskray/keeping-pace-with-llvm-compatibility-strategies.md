...
source: MaskRay (宋方睿)
source_key: maskray
source_url: 'https://maskray.me/blog/2024-11-10-keeping-pace-with-llvm-compatibility-strategies'
original_language: en
published: 2024-11-10
status: active
license: 未声明 → 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:6c08a956f196c905'
translated: false
---

> 原文：[Keeping pace with LLVM: compatibility strategies](https://maskray.me/blog/2024-11-10-keeping-pace-with-llvm-compatibility-strategies)　·　MaskRay (宋方睿)

[2024-11-10](https://maskray.me/blog/2024-11-10-keeping-pace-with-llvm-compatibility-strategies)
...
In Oct 2018, [https://reviews.llvm.org/D52783](https://reviews.llvm.org/D52783) moved the namespace `clang::vfs` to `llvm::vfs`. To remain compatibility, I renamed `clang::vfs` uses and added a conditional namespace alias:
...
In March 2019, [https://reviews.llvm.org/D59377](https://reviews.llvm.org/D59377) removed the member variable `VirtualFileSystem` and removed `setVirtualFileSystem`. To adapt to this change, ccls employs an `#if`. 1  
2  
3  
4  
5  
...
In April 2020, the LLVM monorepo integrated a new subproject: flang. flang developers made many changes to clangDriver to reuse it for flang. [https://reviews.llvm.org/D86089](https://reviews.llvm.org/D86089) changed the constructor `clang::driver::Driver`. I added 1  
2  
3  
4  
5  
...
In November 2020, [https://reviews.llvm.org/D90890](https://reviews.llvm.org/D90890) changed an argument of `ComputePreambleBounds` from `const llvm::MemoryBuffer *Buffer` to `const llvm::MemoryBufferRef &Buffer`. 1  
2  
3  
4  
5  
...
[https://reviews.llvm.org/D91297](https://reviews.llvm.org/D91297) made a similar change and I adapted it similarly.

In Jan 2022, [https://reviews.llvm.org/D116317](https://reviews.llvm.org/D116317) added a new parameter `bool Braced` to `CodeCompleteConsumer::ProcessOverloadCandidates`.
...
In Sep 2023, [https://github.com/llvm/llvm-project/pull/65647](https://github.com/llvm/llvm-project/pull/65647) changed `CompilerInvocationRefBase` to `CompilerInvocationBase`. I duplicated the code with `.`. 1  
2  
3  
4  
5  
...
In April 2024, [https://github.com/llvm/llvm-project/pull/89548/](https://github.com/llvm/llvm-project/pull/89548/) removed `llvm::StringRef::startswith` in favor of `starts_with`. `starts_with` has been available [since Oct 2022](https://reviews.llvm.org/D136030) and `startswith` had been deprecated. I added the following snippet:
...
In November 2023, [https://github.com/llvm/llvm-project/pull/71160](https://github.com/llvm/llvm-project/pull/71160) changed an unnamed enumeration to a scoped enumeration. To keep the following snippet compiling,
...
In April 2024, [https://github.com/llvm/llvm-project/pull/89639](https://github.com/llvm/llvm-project/pull/89639) renamed an enumerator. I have made the following adaptation:
...
In Dec 2022, [https://reviews.llvm.org/D137838](https://reviews.llvm.org/D137838) added a new LLVM library LLVMTargetParser. I adjusted ccls's `CMakeLists.txt`: 1  
2  
3  
4  
...
Clang provides a C API called [libclang](https://clang.llvm.org/docs/LibClang.html). While highly stable, libclang's limited functionality makes it unsuitable for many tasks.
