...
source: MaskRay (宋方睿)
source_key: maskray
source_url: 'https://maskray.me/blog/2024-12-22-simplifying-disassembly-with-llvm-tools'
original_language: en
published: 2024-12-22
status: active
license: 未声明 → 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:925c745e86488fc3'
translated: false
---

> 原文：[Simplifying disassembly with LLVM tools](https://maskray.me/blog/2024-12-22-simplifying-disassembly-with-llvm-tools)　·　MaskRay (宋方睿)

[2024-12-22](https://maskray.me/blog/2024-12-22-simplifying-disassembly-with-llvm-tools)
...
- [`--output-asm-variant=1`](https://maskray.me/blog/2023-05-08-assemblers#x86): LLVM, like GCC, defaults to AT&T syntax for x86 assembly. This option switches to the Intel syntax. See [lhmouse/mcfgthread/wiki/Intel-syntax](https://github.com/lhmouse/mcfgthread/wiki/Intel-syntax) if you prefer the Intel syntax in compiler toolchains.
- `--cdis`: Introduced in LLVM 18, this option enables colored disassembly. In older LLVM versions, you have to use `--disassemble`.

---

I have contributed patches to [remove `.text`](https://github.com/llvm/llvm-project/pull/120185) and allow [disassembling raw bytes without the 0x prefix](https://github.com/llvm/llvm-project/pull/120185). You can now use the `--hex` option:
...
llvm-mc handles decoding failures by skipping a number of bytes, as determined by the target-specific `llvm::MCDisassembler::getInstruction`. To treat a sequence of bytes as a single unit during disassembly, enclose them within `[]`.
...
For address information, llvm-mc falls short. We need to turn to llvm-objdump to get that detail. Here is a little fish script that takes raw hex bytes as input, converts them to a binary format (`xxd -r -p`), and then creates an ELF relocatable file (`llvm-objcopy -I binary`) targeting the x86-64 architecture. Finally, llvm-objdump with the `-D` flag disassembles the data section (`.data`) containing the converted binary.
