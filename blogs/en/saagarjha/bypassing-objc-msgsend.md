...
If we’re correct, not only does this skip the cache lookup that `objc_msgSend` performs, but it also makes a _direct_ call to the method: one which the compiler knows about and can optimize around.
