...
source: objc.io
source_key: objcio
source_url: 'https://www.objc.io/issues/4-core-data/core-data-fetch-requests'
original_language: en
published: ''
status: frozen
license: 未声明（页面无版权声明，文章版权归各作者）→ 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:0d07a21bfa9b3727'
translated: false
---

> 原文：[Fetch Requests](https://www.objc.io/issues/4-core-data/core-data-fetch-requests)　·　objc.io

A way to get objects out of the store is to use an `NSFetchRequest`. Note, though, that one of the most common mistakes is to fetch data when you don't need to. Make sure you read and understand [Getting to Objects](https://www.objc.io/issues/4-core-data/core-data-overview/#getting-to-objects). Most of the time, traversing relationships is more efficient, and using an `NSFetchRequest` is often expensive.
...
We're using the `+entityName` method that we mention in [Florian's data model article](https://www.objc.io/issues/4-core-data/core-data-models-and-model-objects/#creating-objects). Next, we need to limit the results to just those close to our point.
...
This tell us that SQLite was using the `ZSTOP_ZLONGITUDE_INDEX` for the `(ZLONGITUDE>? AND ZLONGITUDE<?)` condition. We could do better by using a _compound index_ as described in the [model article](https://www.objc.io/issues/4-core-data/core-data-models-and-model-objects/#indexes). Since we'd always search for a combination of longitude and latitude that is more efficient, and we can remove the individual indexes on longitude and latitude.
...
As explained in the [SQLite Documentation](https://www.sqlite.org/eqp.html), the warning sign is a `SCAN TABLE` in the output. That basically means that SQLite needs to go through _all_ entries to see which ones are matching. Unless you store just a few objects, you'd probably want an index.

### Subqueries

Let's say we only want those stops near us that are serviced within the next twenty minutes.

We can create a predicate for the _StopTimes_ entity like this:
...
But what if what we want is a predicate that we can use to filter _Stop_ objects based on the relationship to _StopTime_ objects, not _StopTime_ objects themselves? We can do that with a `SUBQUERY` like this:
...
A common scenario is searching for text. In our case, let's look at searching for _Stop_ entities by their name.
