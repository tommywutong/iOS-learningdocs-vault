...
source: objc.io
source_key: objcio
source_url: 'https://www.objc.io/issues/15-testing/xctest'
original_language: en
published: ''
status: frozen
license: 未声明（页面无版权声明，文章版权归各作者）→ 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:c2819ca56824880d'
translated: false
---

> 原文：[Real-World Testing with XCTest](https://www.objc.io/issues/15-testing/xctest)　·　objc.io
...
We structure our tests by using the _Given-When-Then_ pattern — every test is split into three parts.
...
Our mocking framework is [OCMock](http://ocmock.org/). As described in the article about [mocking](https://www.objc.io/issue-15/mocking-stubbing.html) in this issue, a mock is an object that returns a prepared answer for method calls.
...
We also had interdependencies between objects to keep track of. If _A_ has a relationship to _B_, and _B_ has local changes, we need to wait for those local changes to be pushed first, before we can send the changes of _A_.
...
But the main problem with asynchronous testing is isolation. Isolation is the “I“ in FIRST, as mentioned by [the article about testing practices](https://www.objc.io/issue-15/bad-testing-practices.html).
...
We have a set of tests that use the API we provide to the UI, and all the interaction with the server is then channeled through a _fake_ implementation of the `TransportSession`. This transport session mimics both the behavior of the real `TransportSession` and the behavior of the server. The fake session implements the entire protocol of the `TransportSession` and adds a few methods that allow us to set up its state.
...
![](https://www.objc.io/images/issue-15/xctest-diamond-in-gutter@2x_21bd961.png)

If the test fails, it turns red:

![](https://www.objc.io/images/issue-15/xctest-red-diamon-in-gutter@2x_d65a4a1.png)

If it passes, it turns green:

![](https://www.objc.io/images/issue-15/xctest-green-diamon-in-gutter@2x_b7b2586.png)
...
![](https://www.objc.io/images/issue-15/xctest-test-navigator@2x_c89a7f3.png)
...
![](https://www.objc.io/images/issue-15/xctest-test-navigator-failing@2x_1bf300a.png)
...
We have a custom _trigger_ that runs as part of the build on Xcode Server on our release branch. This _trigger_ script uploads the generated Xcode archive to a file server. This way, we have versioned archives. The UI team can then download a specific version of the precompiled framework from the file server.

## BDD and XCTest

If you are familiar with [behavior-driven development](https://www.objc.io/issue-15/behavior-driven-development.html), you might have noticed earlier that our style of naming is heavily inspired by this way of testing. Some of us have used [Kiwi](https://github.com/kiwi-bdd/Kiwi) as a testing library before, so it felt natural to concentrate on a method or class behavior. But does this mean that XCTest can replace a good BDD library? The answer is: not quite.
