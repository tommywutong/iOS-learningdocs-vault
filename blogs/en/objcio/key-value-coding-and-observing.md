...
Let's look at a sample: Our model class `LabColor` is a color in the [Lab color space](https://en.wikipedia.org/wiki/Lab_color_space) where the components are _L_, _a_, and _b_ (instead of red, green, and blue). We want sliders to change the values and a big rectangle that shows the color.
...
Note: We will add validation to this later, as pointed out at [_Key-Value Validation_](#key-value-validation).
...
Check out the entire project on our [GitHub repository](https://github.com/objcio/issue-7-contact-editor). It also uses [_Key-Value Validation_](#key-value-validation) as discussed further below.
...
The [_Contacts Editor_ sample app](https://github.com/objcio/issue-7-contact-editor) uses the same method to wrap a C++ `std::vector` -- in a contrived example. But it illustrates how this method can be used.
...
The [_Contact Editor_ sample](https://github.com/objcio/issue-7-contact-editor) illustrates this in the `DetailViewController` and `Contact` class.
