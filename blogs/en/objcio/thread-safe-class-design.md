...
source: objc.io
source_key: objcio
source_url: 'https://www.objc.io/issues/2-concurrency/thread-safe-class-design'
original_language: en
published: ''
status: frozen
license: 未声明（页面无版权声明，文章版权归各作者）→ 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:1876be6fd8c1e0d9'
translated: false
---

> 原文：[Thread-Safe Class Design](https://www.objc.io/issues/2-concurrency/thread-safe-class-design)　·　objc.io
...
Using atomic properties alone won't make your classes thread-safe. It will only protect you against [race conditions](https://www.objc.io/issues/2-concurrency/concurrency-apis-and-pitfalls/#shared_resources) in the setter, but won't protect your application logic. Consider the following snippet:
...
Testing for the current queue might work for simple solutions, but it fails as soon as your code gets more complex, and you might have multiple queues locked at the same time. Once you are there, you almost certainly will get a [deadlock](https://www.objc.io/issues/2-concurrency/concurrency-apis-and-pitfalls/#dead_locks). Sure, one could use `dispatch_get_specific()`, which will traverse the whole queue hierarchy to test for specific queues. For that you would have to write custom queue constructors that apply this metadata. Don't go that way. There are use cases where a `NSRecursiveLock` is the better solution.
...
Of course there are some caveats; for example you can't set a target queue on your `NSOperationQueue` (like `DISPATCH_QUEUE_PRIORITY_BACKGROUND` for throttled I/O). But that's a small price for debuggability, and it also prevents you from running into problem like [priority inversion](https://www.objc.io/issues/2-concurrency/concurrency-apis-and-pitfalls/#priority_inversion). I even recommend against the nice `NSBlockOperation` API and suggest real subclasses of NSOperation, including an implementation of description. It's more work, but later on, having a way to print all running/pending operations is insanely useful.
