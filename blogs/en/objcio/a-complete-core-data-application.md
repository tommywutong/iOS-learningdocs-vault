...
source: objc.io
source_key: objcio
source_url: 'https://www.objc.io/issues/4-core-data/full-core-data-application'
original_language: en
published: ''
status: frozen
license: 未声明（页面无版权声明，文章版权归各作者）→ 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:33e239cc84c829c5'
translated: false
---

> 原文：[A Complete Core Data Application](https://www.objc.io/issues/4-core-data/full-core-data-application)　·　objc.io
...
It's important to check the error, because this will probably fail a lot during development. When you change your data model, Core Data detects this and will not continue. You can also pass in options to instruct Core Data about what to do in this case, which Martin explains thoroughly in his article about [migrations](https://www.objc.io/issues/4-core-data/core-data-migration/). Note that the last line adds an undo manager; we will need this later. On iOS, you need to explicitly add an undo manager, whereas on Mac it is there by default.

This code creates a really simple Core Data Stack: one managed object context, which has a persistent store coordinator, which has one persistent store. [More complicated setups](https://www.objc.io/issues/4-core-data/core-data-overview/#complicated-stacks) are possible; the most common is to have multiple managed object contexts (each on a separate queue).
