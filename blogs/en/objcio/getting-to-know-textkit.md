...
source: objc.io
source_key: objcio
source_url: 'https://www.objc.io/issues/5-ios7/getting-to-know-textkit'
original_language: en
published: ''
status: frozen
license: 未声明（页面无版权声明，文章版权归各作者）→ 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:3c8dae41352f4e0f'
translated: false
---

> 原文：[Getting to Know TextKit](https://www.objc.io/issues/5-ios7/getting-to-know-textkit)　·　objc.io
...
**iOS 3.2**: The introduction of the iPad brings CoreText, the aforementioned low-level text layout and rendering engine (which was ported from Mac OS X 10.5), and `UITextInput`, the also-mentioned keyboard access. Apple demos Pages as the new light tower of text editing on mobile devices ^[1](#fnref1). However, due to the framework gap I talked about earlier, only very few apps follow suit.
...
![](https://www.objc.io/images/issue-5/kerning_8feecaa.png)
...
![](https://www.objc.io/images/issue-5/ligature_3133f72.png)
...
![](https://www.objc.io/images/issue-5/Screen%20Shot%202013-09-29%20at%2022.19.58.png)
...
![](https://www.objc.io/images/issue-5/TextKit_395e9e4.png)
...
2. Starting with the text provided by the text storage, it translates all characters into glyphs ^[2](#fnref2).
...
Here’s a number: out of the [131 public classes in UIKit](https://developer.apple.com/library/ios/documentation/uikit/reference/uikit_framework/index.html), all but nine have the `UI` prefix in their name. These nine classes carry the legacy, old-world (read: Mac OS) prefix `NS`. And out of these nine classes, seven deal with text. Coincidence? Well…

Here’s a schematic of the Cocoa Text System. Feel free to compare it to the one for TextKit above.

![](https://www.objc.io/images/issue-5/CocoaTextSystem_da41d59.png)
...
To illustrate what is possible with TextKit, I created a little demo project that [can be found on GitHub](https://github.com/objcio/issue-5-textkit). In this demo I did only things that were not easily possible before. I must admit that coding it up took only a Sunday morning; something similar would probably have taken me days or weeks before.
...
For a working example of this configuration see the “Configuration” tab in the aforementioned [TextKitDemo](https://github.com/objcio/issue-5-textkit).
...
![](https://www.objc.io/images/issue-5/SyntaxHighlighting_6b7d848.png)
...
For a working example, see the “Highlighting” tab in the aforementioned [TextKitDemo](https://github.com/objcio/issue-5-textkit).
...
Enough philosophy, let’s customize something. For example, how about adjusting line heights? Sounds crazy, but adjusting line heights has been [at least hacky or required using private API](http://stackoverflow.com/questions/3760924/set-line-height-in-uitextview/3914228) on previous releases of iOS. Fortunately, this is — again — now a no-brainer. Set the layout manager’s delegate and implement a single method:
...
For a working example, see the “Layout” tab in the aforementioned [TextKitDemo](https://github.com/objcio/issue-5-textkit). Here’s a screenshot:

![](https://www.objc.io/images/issue-5/LineBreaking_cd28f16.png)
...
![](https://www.objc.io/images/issue-5/ReflowingTextAndClippy_96efeb7.png)
...
For a working example, see the “Interaction” tab in the [TextKitDemo](https://github.com/objcio/issue-5-textkit). As a little gimmick, it also includes a view that follows the current text selection. Because, you know, what would a good text editor demo be without a little ugly annoying paper clip getting in your way?

---

1. Pages did — according to Apple — use absolutely no private API. *cough*  
   My theory: it either used an extremely early version of TextKit or copied half of UIKit’s private sources. Or a mix thereof. [↩](#fnref-rev1)
2. _Glyphs_: While characters are the “semantical” representation of a letter, glyphs are the visual representation thereof. Depending on the font used, glyphs are either bezier paths or bitmap images defining the shape that should be drawn. See also the excellent [Wikipedia article](http://en.wikipedia.org/wiki/Glyph) about glyphs. [↩](#fnref-rev2)
3. In a class cluster, only an abstract superclass is public. Allocating an instance actually creates an object of a private subclass. As such, subclassing always happens on an abstract class and requires all methods to be implemented. See also the [class cluster documentation](https://developer.apple.com/library/ios/documentation/general/conceptual/CocoaEncyclopedia/ClassClusters/ClassClusters.html). [↩](#fnref-rev3)
