...
source: objc.io
source_key: objcio
source_url: 'https://www.objc.io/issues/13-architecture/singletons'
original_language: en
published: ''
status: frozen
license: 未声明（页面无版权声明，文章版权归各作者）→ 仅私有归档
archived_at: 2026-07-27
content_hash: 'sha256:11afe51de681bfa0'
translated: false
---

> 原文：[Avoiding Singleton Abuse](https://www.objc.io/issues/13-architecture/singletons)　·　objc.io
...
While others have called singletons an 'anti-pattern,' 'evil,' and ['pathological liars'](http://misko.hevery.com/2008/08/17/singletons-are-pathological-liars/), I won't completely rule out the merit of singletons. Instead, I want to demonstrate a few problems with singletons so that the next time you're about to auto-complete that `dispatch_once` snippet, you think twice about the implications.
...
So how does this example relate to singletons? In the words of Miško Hevery, ["Singletons are global state in sheep’s clothing."](http://misko.hevery.com/2008/08/25/root-cause-of-singletons/) A singleton can be used anywhere, without explicitly declaring the dependency. Just like `_a` and `_b` were used in `computeSum` without the dependency being made explicit, any module of the program can call `[SPMySingleton sharedInstance]` and get access to the singleton. This means any side effects of interacting with the singleton can affect arbitrary code elsewhere in the program.
...
We can explicitly pass the authenticated user object into the view controller. This technique of passing a dependency into a dependent object is more formally referred to as [dependency injection,](http://en.wikipedia.org/wiki/Dependency_injection) and it has ton of advantages:
...
![](https://www.objc.io/images/issue-13/Screen%20Shot%202014-06-02%20at%205.21.20%20AM.png)
...
![](https://www.objc.io/images/issue-13/Screen%20Shot%202014-06-02%20at%205.53.45%20AM.png)
...
![](https://www.objc.io/images/issue-13/Screen%20Shot%202014-06-02%20at%205.38.59%20AM.png)

Suppose, for simplicity, that the `SPApplicationDelegate` manages the `SPUser` instance (in practice, you probably want to offload such user state management to another object to keep your application delegate [lighter](https://www.objc.io/issues/1-view-controllers/lighter-view-controllers/)). When the friend list view controller is installed in the window, a reference to the user is passed. This reference can be funneled down the object graph to the profile image views as well. Now, when the user logs out, our object graph looks like this:

![](https://www.objc.io/images/issue-13/Screen%20Shot%202014-06-02%20at%205.54.07%20AM.png)
...
![](https://www.objc.io/images/issue-13/Screen%20Shot%202014-06-02%20at%205.59.25%20AM.png)
...
![](https://www.objc.io/images/issue-13/Screen%20Shot%202014-06-02%20at%206.43.56%20AM.png)
