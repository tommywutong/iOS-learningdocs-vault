...
[POSIX](https://en.wikipedia.org/wiki/POSIX) conformant systems provide the pthreads library and it contains a mutex ([lock](https://en.wikipedia.org/wiki/Lock_(computer_science))) implementation: `pthread_mutex_t`. The API creates a blocking lock (that can optionally be configured as a recursive lock) and must be initialized and destroyed using specific functions. Since the functions take in a reference to the mutex, you can’t pass in the mutex’s property address, so you need to pass in the iVar directly. Therefore, most instances of these mutexes are defined in some global scope, rather than a property/iVar of an Objective-C class. However, that doesn’t mean that you can’t store the mutex as a property[^1](https://stackoverflow.com/questions/9086736/why-would-you-use-an-ivar), you’ll just need to dereference the backing iVar (which introduces `NULL` dereference issues if you aren’t careful: `NULL` behaves differently than `nil`).
