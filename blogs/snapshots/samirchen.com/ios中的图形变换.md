---
title: iOS中的图形变换
source_url: 'http://www.samirchen.com/graphic-transform-in-ios/'
source_domain: samirchen.com
source_group: single-site
original_language: zh
published: ''
archived_at: 2026-07-27
content_hash: 'sha256:128fb01adf2d2396'
...
> 原文：[iOS中的图形变换](http://www.samirchen.com/graphic-transform-in-ios/)

# [iOS中的图形变换](http://www.samirchen.com/graphic-transform-in-ios/)
...
[Quartz 2D](https://developer.apple.com/library/ios/documentation/GraphicsImaging/Conceptual/drawingwithquartz2d/dq_overview/dq_overview.html)是 iPhone OS 和 Mac OS X 环境下的二维绘图引擎。借用 Quartz 2D API，你可以接触到这样一些特性：基于路径的绘图，透明度绘图，遮盖，阴影，透明层，颜色管理，防锯齿渲染，生成 PDF，以及 PDF 元数据相关处理。可能的时候，Quartz 2D 会借助硬件的力量。
...
[`graphics context`](https://developer.apple.com/library/ios/documentation/GraphicsImaging/Conceptual/drawingwithquartz2d/dq_overview/dq_overview.html#//apple_ref/doc/uid/TP30001066-CH202-TPXREF131) 是 Quartz 2D 中一个十分重要的概念，它对应的数据结构即 `CGContextRef`，其中包含了Quartz 将图形绘制到输出设备（比如：PDF文件、bitmap、显示窗口）需要的信息。所有 Quartz 中的对象都是画到或是包含在 `graphics context` 中。
...
这些类型针对不同的输出目标而定，详细信息参见 [苹果文档：Drawing Destinations: The Graphics Context](https://developer.apple.com/library/ios/documentation/GraphicsImaging/Conceptual/drawingwithquartz2d/dq_overview/dq_overview.html#//apple_ref/doc/uid/TP30001066-CH202-TPXREF131)。
...
想要绘制不同的对象，则根据需求选择不同的 API 来使用，详细信息参见：[苹果文档：Quartz 2D Opaque Data Types](https://developer.apple.com/library/ios/documentation/GraphicsImaging/Conceptual/drawingwithquartz2d/dq_overview/dq_overview.html#//apple_ref/doc/uid/TP30001066-CH202-BBCGCGBA)。
...
详细信息参见 [苹果文档：Graphics State](https://developer.apple.com/library/ios/documentation/GraphicsImaging/Conceptual/drawingwithquartz2d/dq_overview/dq_overview.html#//apple_ref/doc/uid/TP30001066-CH202-TPXREF132)。

#### Quartz 2D Coordinate Systems

Quartz 2D 默认的坐标系是这样的，原点 (0, 0) 在左下角：

![](http://www.samirchen.com/images/graphic-transform-in-ios/quartz-coordinates.gif)

但并不是所有的API里都是使用相同的坐标系统，比如 UIKit 中图像绘制常用到的就是 `modified coordinate system` ，其原点 (0, 0) 在左下角：

![](http://www.samirchen.com/images/graphic-transform-in-ios/flipped-coordinates.jpg)
...
- `@property CGRect bounds;` // your view’s internal drawing space’s origin and size
    ...
    - The bounds property is what you use inside your view’s own implementation. It is up to your implementation as to how to interpret the meaning of bounds.origin. bounds 这个属性是 UIView 自己内部的实际边界。在自己的坐标系统内，99%的情况下都是从origin为 (0, 0) 作为起始位置，当然你也可以自己定义这个origin不为 (0, 0)，你可以自己来解释它。
...
![](http://www.samirchen.com/images/graphic-transform-in-ios/uiview-frame-bounds.png)
...
![](http://www.samirchen.com/images/graphic-transform-in-ios/identify-transform.png)
...
[一个使用CGAffineTransform的Demo。](https://github.com/samirchen/TransformDemo)

### CGAffineTransform的理解

`CGAffineTransform` 的数据结构表示的是用于仿射变换的矩阵。详细内容可以参考：[苹果文档：CGAffineTransform](https://developer.apple.com/library/ios/documentation/graphicsimaging/reference/CGAffineTransform/Reference/reference.html)
...
![](http://www.samirchen.com/images/graphic-transform-in-ios/affine-transform-matrix.png)

由于在该矩阵中，第三列将总是 `0 0 1`，所以 CGAffineTransform 数据结构只返回前两列的值。

一个点 (x, y) 经过一个仿射变换得到的点 (x’, y’) 的计算过程和计算表达式为：

![](http://www.samirchen.com/images/graphic-transform-in-ios/affine-transform-matrix-calcu-1.png)

![](http://www.samirchen.com/images/graphic-transform-in-ios/affine-transform-matrix-calcu-2.png)
...
这个方法的参数 tx，ty 分别是在X轴和Y轴的移动位移。 CGAffineTransformMakeTranslation(tx, ty) 得到的矩阵是：

![](http://www.samirchen.com/images/graphic-transform-in-ios/affine-transform-matrix-translation.png)

那么对 (x, y) 使用上面的变换后得到的结果 (x’, y’) 的计算表达式为：

![](http://www.samirchen.com/images/graphic-transform-in-ios/affine-transform-matrix-translation-calcu.png)
...
![](http://www.samirchen.com/images/graphic-transform-in-ios/affine-transform-matrix-rotation.png)

那么对 (x, y) 使用上面的变换后得到的结果 (x’, y’) 的计算表达式为：

![](http://www.samirchen.com/images/graphic-transform-in-ios/affine-transform-matrix-rotation-calcu.png)

上面的计算过程可以很容易的用几何知识就行证明。参见：

![](http://www.samirchen.com/images/graphic-transform-in-ios/rotate-point.gif)
...
在原坐标系xoy中, 绕原点沿逆时针方向旋转θ度， 变成座标系 sot。 设有某点p，在原坐标系中的坐标为 (x, y)，旋转后的新坐标为(s, t)。

![](http://www.samirchen.com/images/graphic-transform-in-ios/rotate-axes.gif)
...
![](http://www.samirchen.com/images/graphic-transform-in-ios/affine-transform-matrix-scale.png)

那么对 (x, y) 使用上面的变换后得到的结果 (x’, y’) 的计算表达式为：

![](http://www.samirchen.com/images/graphic-transform-in-ios/affine-transform-matrix-scale-calcu.png)
...
In iOS, if the layer is associated with a UIView object, this property must be set to the view that owns the layer. Because the view is the layer’s delegate, never make the view the delegate of another CALayer object. Additionally, never change the delegate of this layer object.
...
图1 ![](http://www.samirchen.com/images/graphic-transform-in-ios/layer-coords-anchorpoint-position.png)

图2 ![](http://www.samirchen.com/images/graphic-transform-in-ios/layer-coords-anchorpoint-position-2.jpg)
...
- [FaceXSwap 支持换脸生成 GIF 表情包，我给老板做了一套表情包，在同事小群里玩疯了](http://www.samirchen.com/generate-meme-with-facexswap/)
- [AI 魔法提示词集锦第 10 期：创意人物创作](http://www.samirchen.com/aigc-prompts-collection-10/)
- [AI 魔法提示词集锦第 9 期：超现实的人物创作](http://www.samirchen.com/aigc-prompts-collection-9/)
- [AI 魔法提示词集锦第 8 期：Midjourney 日系复古胶片风](http://www.samirchen.com/aigc-prompts-collection-8/)
- [AI 魔法提示词集锦第 7 期：创意融合与场景构建](http://www.samirchen.com/aigc-prompts-collection-7/)

## Opinion

- [育儿之道](http://www.samirchen.com/education-of-children/)
- [聊聊抖音](http://www.samirchen.com/talk-about-douyin/)
- [聊聊直播和短视频](http://www.samirchen.com/live-vs-video/)
- [用微信红包弹一首「两只老虎」](http://www.samirchen.com/wechat-new-version-6-5-4/)
- [动态图带你体验微信小程序](http://www.samirchen.com/wechat-little-program/)

## Project

- [AppStore 2019 年 5 月排行榜](http://www.samirchen.com/app-store-monthly-rank-5/)
- [AppStore 2019 年 4 月排行榜](http://www.samirchen.com/app-store-monthly-rank-4/)
- [AppStore 2019 年 3 月排行榜](http://www.samirchen.com/app-store-monthly-rank-3/)
- [AppStore 2019 年 2 月排行榜](http://www.samirchen.com/app-store-monthly-rank-2/)
- [AppStore 2019 年 1 月排行榜](http://www.samirchen.com/app-store-monthly-rank-1/)
